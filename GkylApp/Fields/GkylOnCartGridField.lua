...
function GkField:alloc(nRkFields)
   ...
   -- create fields for charge and current densities
...
function GkField:forwardEuler(tCurr, dt, potIn, momIn, potOut)
   if self.evolve then
      ...
      local chargeDens = momIn[1]
      local phi = potOut[1]
      local mys, mydt = self.phiSlvr:advance(tCurr, dt, {chargeDens}, {phi})
      if self.isElectromagnetic then
        local currentDens = momIn[2]
        local apar = potOut[2]
        mys2, mydt2 = self.aparSlvr:advance(tCurr, dt, {currentDens}, {apar})
      end
...
--      local mys2 = true
--      local mydt2 = GKYL_MAX_DOUBLE
--      chargeDens:clear(0.0)
--      for nm, s in pairs(species) do
--        chargeDens:accumulate(s:charge(), s:getNumDensity())
--      end
--      local phi = potOut[1]
--      local mys, mydt = self.phiSlvr:advance(tCurr, dt, {chargeDens}, {phi})
--      if self.isElectromagnetic then
--        currentDens:clear(0.0)
--        for nm, s in pairs(species) do
--          currentDens:accumulate(s:charge(), s:getMomDensity())
--        end
--        mys2, mydt2 = self.aparSlvr:advance(tCurr, dt, {currentDens}, {apar})
--      end
--      return mys and mys2, math.min(mydt,mydt2)
