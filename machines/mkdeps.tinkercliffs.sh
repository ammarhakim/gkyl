...
export MACHINE_NAME='tinkercliffs'
cd install-deps
# gcc
...
# intel
./mkdeps.sh CC=icc CXX=icpc MPICC=mpicc MPICXX=mpicxx --prefix=$GKYLSOFT --build-gkylzero=yes --build-luajit=yes --build-adios=yes --build-openmpi=no --build-luarocks=yes
