...
local function newton(f, xl, xu, stop)
  ...
  while true do
    local x1 = x0 - y0/f10
    if diff.eq(x1, x0) then
      if diff.eq(y0, 0) then
        return x0, y0, x0, x0, y0, y0
      else
        error("x1 == x0, f(x0)="..y0..", f1(x0)="..f10)
        return nil
      end
    end
    if not diff.le(xl, x1) and diff.le(x1, xu) then 
      error("x1 outside bracket, f(x0)="..tostring(y0)..", f1(x0)="..tostring(f10))
      return nil
    end
    local y1, f11 = f(x1)
...
local function halley(f, xl, xu, stop)
  ...
  while true do
    ...
    if not (diff.le(xl,x1) and diff.le(x1,xu)) then 
      return nil, "x1 outside bracket, f(x0)="..tostring(y0)..", f1(x0)="..tostring(f10)
                ..", f2(x0)="..tostring(f20)
    end
