...
{ 
  ...
  alphaDrag[8] = (2.0*u[2]-2.828427124746191*w[2])*rdvy2nu; 
  alphaDrag[9] = 2.0*u[3]*rdvy2nu; 
  alphaDrag[11] = -0.8164965809277261*dxv[2]*rdvy2nu; 
  ...
  return std::abs(0.1767766952966368*alphaDrag[0])+std::abs(0.1767766952966368*alphaDrag[8])+std::abs(0.9428090415820636*vtSq[0]*rdvxSq4nu)+std::abs(0.9428090415820636*vtSq[0]*rdvySq4nu); 
...
{ 
  ...
  // Expand rdv2nu*(vy-uy) in phase basis.
  alphaDrag[20] = (2.0*u[3]-2.828427124746191*w[2])*rdvy2nu; 
  alphaDrag[21] = 2.0*u[4]*rdvy2nu; 
  alphaDrag[23] = -0.8164965809277261*dxv[2]*rdvy2nu; 
  alphaDrag[27] = 2.0*u[5]*rdvy2nu; 
  ...
  out[8] += 4.743416490252569*(facDiff[2]*f[7]+f[1]*facDiff[1]+f[0]*facDiff[0])*rdvxSq4nu+1.369306393762915*alphaDrag[7]*f[11]+1.224744871391589*alphaDrag[2]*f[8]+1.369306393762915*(alphaDrag[1]*f[4]+alphaDrag[0]*f[2]+f[0]*alphaDrag[2]); 
  out[9] += 4.743416490252569*(facDiff[2]*f[7]+f[1]*facDiff[1]+f[0]*facDiff[0])*rdvySq4nu+1.369306393762915*f[13]*alphaDrag[27]+1.224744871391589*f[9]*alphaDrag[23]+1.369306393762915*(f[0]*alphaDrag[23]+f[5]*alphaDrag[21]+f[3]*alphaDrag[20]); 
  ...
  out[12] += (4.242640687119286*(facDiff[1]*f[7]+f[1]*facDiff[2])+4.743416490252569*(f[0]*facDiff[1]+facDiff[0]*f[1]))*rdvxSq4nu+1.224744871391589*(alphaDrag[2]*f[12]+alphaDrag[1]*f[11]+f[4]*alphaDrag[7])+1.369306393762915*(alphaDrag[0]*f[4]+alphaDrag[1]*f[2]+f[1]*alphaDrag[2]); 
  ...
  out[14] += 4.743416490252569*(facDiff[2]*f[13]+facDiff[1]*f[5]+facDiff[0]*f[3])*rdvxSq4nu+0.6123724356957944*(f[14]*alphaDrag[23]+f[12]*alphaDrag[21]+f[8]*alphaDrag[20])+1.369306393762915*alphaDrag[7]*f[17]+1.224744871391589*alphaDrag[2]*f[14]+1.369306393762915*(alphaDrag[1]*f[10]+alphaDrag[0]*f[6]+alphaDrag[2]*f[3]); 
  out[15] += (4.242640687119286*(facDiff[1]*f[7]+f[1]*facDiff[2])+4.743416490252569*(f[0]*facDiff[1]+facDiff[0]*f[1]))*rdvySq4nu+1.224744871391589*f[5]*alphaDrag[27]+(1.224744871391589*f[15]+1.369306393762915*f[1])*alphaDrag[23]+1.224744871391589*f[13]*alphaDrag[21]+1.369306393762915*(f[3]*alphaDrag[21]+f[5]*alphaDrag[20]); 
  out[16] += 4.743416490252569*(facDiff[2]*f[11]+facDiff[1]*f[4]+facDiff[0]*f[2])*rdvySq4nu+1.369306393762915*f[17]*alphaDrag[27]+1.224744871391589*f[16]*alphaDrag[23]+1.369306393762915*(f[2]*alphaDrag[23]+f[10]*alphaDrag[21]+f[6]*alphaDrag[20])+0.6123724356957944*(alphaDrag[2]*f[16]+alphaDrag[1]*f[15]+alphaDrag[0]*f[9]); 
  ...
  out[18] += (4.242640687119286*facDiff[1]*f[13]+4.743416490252569*(facDiff[0]*f[5]+facDiff[1]*f[3])+4.242640687119286*facDiff[2]*f[5])*rdvxSq4nu+0.5477225575051661*f[12]*alphaDrag[27]+0.6123724356957944*(f[18]*alphaDrag[23]+f[8]*alphaDrag[21]+f[12]*alphaDrag[20])+1.224744871391589*(alphaDrag[2]*f[18]+alphaDrag[1]*f[17]+alphaDrag[7]*f[10])+1.369306393762915*(alphaDrag[0]*f[10]+alphaDrag[1]*f[6]+alphaDrag[2]*f[5]); 
  out[19] += (4.242640687119286*facDiff[1]*f[11]+4.743416490252569*(facDiff[0]*f[4]+facDiff[1]*f[2])+4.242640687119286*facDiff[2]*f[4])*rdvySq4nu+1.224744871391589*f[10]*alphaDrag[27]+(1.224744871391589*f[19]+1.369306393762915*f[4])*alphaDrag[23]+1.224744871391589*f[17]*alphaDrag[21]+1.369306393762915*(f[6]*alphaDrag[21]+f[10]*alphaDrag[20])+0.6123724356957944*alphaDrag[2]*f[19]+0.5477225575051661*alphaDrag[7]*f[15]+0.6123724356957944*(alphaDrag[0]*f[15]+alphaDrag[1]*f[9]); 

  return std::abs(0.1767766952966368*alphaDrag[0]-0.1976423537605236*alphaDrag[7])+std::abs(0.1767766952966368*alphaDrag[20]-0.1976423537605236*alphaDrag[27])+std::abs((1.272792206135785*facDiff[0]-1.42302494707577*facDiff[2])*rdvxSq4nu)+std::abs((1.272792206135785*facDiff[0]-1.42302494707577*facDiff[2])*rdvySq4nu); 
...
{ 
  ...
  // Expand rdv2nu*(vy-uy) in phase basis.
  alphaDrag[32] = (2.0*u[4]-2.828427124746191*w[2])*rdvy2nu; 
  alphaDrag[33] = 2.0*u[5]*rdvy2nu; 
  alphaDrag[35] = -0.8164965809277261*dxv[2]*rdvy2nu; 
  alphaDrag[39] = 2.0*u[6]*rdvy2nu; 
  alphaDrag[49] = 2.0*u[7]*rdvy2nu; 
  ...
  out[8] += 4.743416490252569*(facDiff[3]*f[17]+facDiff[2]*f[7]+f[1]*facDiff[1]+f[0]*facDiff[0])*rdvxSq4nu+1.369306393762915*(alphaDrag[17]*f[23]+alphaDrag[7]*f[11])+1.224744871391589*alphaDrag[2]*f[8]+1.369306393762915*(alphaDrag[1]*f[4]+alphaDrag[0]*f[2]+f[0]*alphaDrag[2]); 
  out[9] += 4.743416490252569*(facDiff[3]*f[17]+facDiff[2]*f[7]+f[1]*facDiff[1]+f[0]*facDiff[0])*rdvySq4nu+1.369306393762915*(f[25]*alphaDrag[49]+f[13]*alphaDrag[39])+1.224744871391589*f[9]*alphaDrag[35]+1.369306393762915*(f[0]*alphaDrag[35]+f[5]*alphaDrag[33]+f[3]*alphaDrag[32]); 
  ...
  out[14] += 4.743416490252569*(facDiff[3]*f[25]+facDiff[2]*f[13]+facDiff[1]*f[5]+facDiff[0]*f[3])*rdvxSq4nu+0.6123724356957944*(f[14]*alphaDrag[35]+f[12]*alphaDrag[33]+f[8]*alphaDrag[32])+1.369306393762915*(alphaDrag[17]*f[29]+alphaDrag[7]*f[20])+1.224744871391589*alphaDrag[2]*f[14]+1.369306393762915*(alphaDrag[1]*f[10]+alphaDrag[0]*f[6]+alphaDrag[2]*f[3]); 
  ...
  out[26] += 16.20185174601965*(facDiff[3]*f[29]+facDiff[2]*f[20]+facDiff[1]*f[10]+facDiff[0]*f[6])*rdvxSq4nu+0.6123724356957944*(f[26]*alphaDrag[35]+f[24]*alphaDrag[33]+f[18]*alphaDrag[32])+1.837117307087383*alphaDrag[2]*f[26]+0.9354143466934851*alphaDrag[17]*f[25]+2.091650066335188*(alphaDrag[1]*f[21]+alphaDrag[0]*f[14])+0.9354143466934851*alphaDrag[7]*f[13]+2.806243040080455*alphaDrag[2]*f[6]+0.9354143466934851*(alphaDrag[1]*f[5]+alphaDrag[0]*f[3]); 
  out[27] += (14.23024947075771*facDiff[2]*f[25]+(14.23024947075771*facDiff[3]+14.49137674618944*facDiff[1])*f[13]+16.20185174601965*(facDiff[0]*f[5]+facDiff[1]*f[3])+14.49137674618944*facDiff[2]*f[5])*rdvySq4nu+0.8215838362577489*f[7]*alphaDrag[49]+(0.8215838362577489*f[17]+1.870828693386971*f[15]+0.8366600265340755*f[1])*alphaDrag[39]+(1.837117307087383*f[27]+2.806243040080455*f[5])*alphaDrag[35]+(2.091650066335188*f[9]+0.8366600265340755*f[7]+0.9354143466934851*f[0])*alphaDrag[33]+(2.091650066335188*f[15]+0.9354143466934851*f[1])*alphaDrag[32]; 
  out[28] += 16.20185174601965*(facDiff[3]*f[29]+facDiff[2]*f[20]+facDiff[1]*f[10]+facDiff[0]*f[6])*rdvySq4nu+0.9354143466934851*(f[23]*alphaDrag[49]+f[11]*alphaDrag[39])+(1.837117307087383*f[28]+2.806243040080455*f[6])*alphaDrag[35]+(2.091650066335188*f[22]+0.9354143466934851*f[4])*alphaDrag[33]+(2.091650066335188*f[16]+0.9354143466934851*f[2])*alphaDrag[32]+0.6123724356957944*(alphaDrag[2]*f[28]+alphaDrag[1]*f[27]+alphaDrag[0]*f[19]); 
