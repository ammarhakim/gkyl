...
double VmLBOconstNuVol2x3vMaxP2(const double *w, const double *dxv, const double nuSum, const double *nuUSum, const double *nuVtSqSum, const double *f, double *out) 
{ 
  ...
  alphaDrag[16] = 2.828427124746191*nuUSum[4]*rdvx2; 
  alphaDrag[17] = 2.828427124746191*nuUSum[5]*rdvx2; 

  // Expand rdv2*(nu*vy-nuUSumy) in phase basis.
  alphaDrag[21] = (2.828427124746191*nuUSum[6]-5.656854249492382*w[3]*nuSum)*rdvy2; 
  alphaDrag[22] = 2.828427124746191*nuUSum[7]*rdvy2; 
  alphaDrag[23] = 2.828427124746191*nuUSum[8]*rdvy2; 
  alphaDrag[25] = -1.632993161855453*dxv[3]*nuSum*rdvy2; 
  alphaDrag[27] = 2.828427124746191*nuUSum[9]*rdvy2; 
  alphaDrag[37] = 2.828427124746191*nuUSum[10]*rdvy2; 
  alphaDrag[38] = 2.828427124746191*nuUSum[11]*rdvy2; 

  // Expand rdv2*(nu*vz-nuUSumz) in phase basis.
  alphaDrag[42] = (2.828427124746191*nuUSum[12]-5.656854249492382*w[4]*nuSum)*rdvz2; 
  alphaDrag[43] = 2.828427124746191*nuUSum[13]*rdvz2; 
  alphaDrag[44] = 2.828427124746191*nuUSum[14]*rdvz2; 
  alphaDrag[47] = -1.632993161855453*dxv[4]*nuSum*rdvz2; 
  alphaDrag[48] = 2.828427124746191*nuUSum[15]*rdvz2; 
  alphaDrag[58] = 2.828427124746191*nuUSum[16]*rdvz2; 
  alphaDrag[59] = 2.828427124746191*nuUSum[17]*rdvz2; 
  ...
  out[18] += 3.354101966249685*(facDiff[5]*f[17]+facDiff[4]*f[16]+facDiff[3]*f[6]+f[2]*facDiff[2]+f[1]*facDiff[1]+f[0]*facDiff[0])*rdvxSq4+0.6123724356957944*alphaDrag[3]*f[18]+0.6846531968814573*(alphaDrag[2]*f[8]+alphaDrag[1]*f[7]+alphaDrag[0]*f[3]+f[0]*alphaDrag[3]); 
  out[19] += 3.354101966249685*(facDiff[5]*f[17]+facDiff[4]*f[16]+facDiff[3]*f[6]+f[2]*facDiff[2]+f[1]*facDiff[1]+f[0]*facDiff[0])*rdvySq4+0.6123724356957944*f[19]*alphaDrag[25]+0.6846531968814573*(f[0]*alphaDrag[25]+f[10]*alphaDrag[23]+f[9]*alphaDrag[22]+f[4]*alphaDrag[21]); 
  out[20] += 3.354101966249685*(facDiff[5]*f[17]+facDiff[4]*f[16]+facDiff[3]*f[6]+f[2]*facDiff[2]+f[1]*facDiff[1]+f[0]*facDiff[0])*rdvzSq4+0.6123724356957944*f[20]*alphaDrag[47]+0.6846531968814573*(f[0]*alphaDrag[47]+f[13]*alphaDrag[44]+f[12]*alphaDrag[43]+f[5]*alphaDrag[42]); 

  return std::abs(0.0883883476483184*alphaDrag[0]-0.09882117688026182*(alphaDrag[17]+alphaDrag[16]))+std::abs(0.0883883476483184*alphaDrag[21]-0.09882117688026182*(alphaDrag[38]+alphaDrag[37]))+std::abs(0.0883883476483184*alphaDrag[42]-0.09882117688026182*(alphaDrag[59]+alphaDrag[58]))+std::abs((0.9*facDiff[0]-1.006230589874905*(facDiff[5]+facDiff[4]))*rdvxSq4)+std::abs((0.9*facDiff[0]-1.006230589874905*(facDiff[5]+facDiff[4]))*rdvySq4)+std::abs((0.9*facDiff[0]-1.006230589874905*(facDiff[5]+facDiff[4]))*rdvzSq4); 
