...
double VmLBOconstNuUpwindSurf1x1vTensor_VX_P1(const double *wl, const double *wr, const double *dxvl, const double *dxvr, const double nuSum, const double vMuMidMax, const double *nuUSum, const double *nuVtSqSum, const double *fl, const double *fr, double *outl, double *outr) 
{ 
  ...
  Gdiff[0] = -0.0625*(2.449489742783178*(5.0*nuVtSqSum[1]*fr[3]+5.0*nuVtSqSum[1]*fl[3]+5.0*nuVtSqSum[0]*fr[2]+5.0*nuVtSqSum[0]*fl[2])+1.414213562373095*((9.0*fl[1]-9.0*fr[1])*nuVtSqSum[1]+(9.0*fl[0]-9.0*fr[0])*nuVtSqSum[0])); 
  Gdiff[1] = -0.0625*(2.449489742783178*(5.0*nuVtSqSum[0]*fr[3]+5.0*nuVtSqSum[0]*fl[3]+5.0*nuVtSqSum[1]*fr[2]+5.0*nuVtSqSum[1]*fl[2])+1.414213562373095*((9.0*fl[0]-9.0*fr[0])*nuVtSqSum[1]-9.0*nuVtSqSum[0]*fr[1]+9.0*nuVtSqSum[0]*fl[1])); 
