...
{ 
  ...
  const double rdvzSq4 = 4/(dxv[3]*dxv[3]); 
  ...
  alpha_drag[0] = (2.828427124746191*nuU[0]-4.0*w[1]*nu)*rdvx2; 
  alpha_drag[1] = 2.828427124746191*nuU[1]*rdvx2; 
  alpha_drag[2] = -1.154700538379252*dxv[1]*nu*rdvx2; 
  ...
  alpha_drag[5] = (2.828427124746191*nuU[2]-4.0*w[2]*nu)*rdvy2; 
  alpha_drag[6] = 2.828427124746191*nuU[3]*rdvy2; 
  alpha_drag[8] = -1.154700538379252*dxv[2]*nu*rdvy2; 
  ...
  alpha_drag[10] = (2.828427124746191*nuU[4]-4.0*w[3]*nu)*rdvz2; 
  alpha_drag[11] = 2.828427124746191*nuU[5]*rdvz2; 
  alpha_drag[14] = -1.154700538379252*dxv[3]*nu*rdvz2; 
  ...
  alpha_mid += std::abs(0.3333333333333333*alpha_diffusion[0]*rdvzSq4); 
...
{ 
  ...
  alpha_drag[1] = 2.828427124746191*nuU[1]*rdvx2; 
  alpha_drag[2] = -1.154700538379252*dxv[1]*nu*rdvx2; 
  alpha_drag[11] = 2.828427124746191*nuU[2]*rdvx2; 

  // cvard[1]-drag contribution to the midpoint value used for CFL.
  alpha_mid += std::abs(0.125*alpha_drag[0]-0.1397542485937369*alpha_drag[11]); 

  // Expand rdv2*nu*(vy-uy) in phase basis.
  alpha_drag[15] = (2.828427124746191*nuU[3]-4.0*w[2]*nu)*rdvy2; 
  alpha_drag[16] = 2.828427124746191*nuU[4]*rdvy2; 
  alpha_drag[18] = -1.154700538379252*dxv[2]*nu*rdvy2; 
  alpha_drag[26] = 2.828427124746191*nuU[5]*rdvy2; 

  // cvard[2]-drag contribution to the midpoint value used for CFL.
  alpha_mid += std::abs(0.125*alpha_drag[15]-0.1397542485937369*alpha_drag[26]); 

  // Expand rdv2*nu*(vz-uz) in phase basis.
  alpha_drag[30] = (2.828427124746191*nuU[6]-4.0*w[3]*nu)*rdvz2; 
  alpha_drag[31] = 2.828427124746191*nuU[7]*rdvz2; 
  alpha_drag[34] = -1.154700538379252*dxv[3]*nu*rdvz2; 
  alpha_drag[41] = 2.828427124746191*nuU[8]*rdvz2; 
  ...
  // Diffusion contribution to the midpoint value used for CFL.
  alpha_mid += std::abs((0.45*alpha_diffusion[0]-0.5031152949374527*alpha_diffusion[11])*rdvxSq4); 
  alpha_mid += std::abs((0.45*alpha_diffusion[0]-0.5031152949374527*alpha_diffusion[11])*rdvySq4); 
  alpha_mid += std::abs((0.45*alpha_diffusion[0]-0.5031152949374527*alpha_diffusion[11])*rdvzSq4); 
  ...
  out[12] += 1.677050983124842*(alpha_diffusion[11]*f[11]+alpha_diffusion[1]*f[1]+alpha_diffusion[0]*f[0])*rdvxSq4+0.8660254037844386*alpha_drag[2]*f[12]+0.9682458365518543*(alpha_drag[1]*f[5]+alpha_drag[0]*f[2]+f[0]*alpha_drag[2]); 
  out[13] += 1.677050983124842*(alpha_diffusion[11]*f[11]+alpha_diffusion[1]*f[1]+alpha_diffusion[0]*f[0])*rdvySq4+0.8660254037844386*f[13]*alpha_drag[18]+0.9682458365518543*(f[0]*alpha_drag[18]+f[6]*alpha_drag[16]+f[3]*alpha_drag[15]); 
  out[14] += 1.677050983124842*(alpha_diffusion[11]*f[11]+alpha_diffusion[1]*f[1]+alpha_diffusion[0]*f[0])*rdvzSq4+0.8660254037844386*f[14]*alpha_drag[34]+0.9682458365518543*(f[0]*alpha_drag[34]+f[8]*alpha_drag[31]+f[4]*alpha_drag[30]); 
...
{ 
  ...
  alpha_drag[2] = -1.154700538379252*dxv[1]*nu*rdvx2; 
  alpha_drag[11] = 2.828427124746191*nuU[2]*rdvx2; 
  alpha_drag[31] = 2.828427124746191*nuU[3]*rdvx2; 

  // cvard[1]-drag contribution to the midpoint value used for CFL.
  alpha_mid += std::abs(0.125*alpha_drag[0]-0.1397542485937369*alpha_drag[11]); 

  // Expand rdv2*nu*(vy-uy) in phase basis.
  alpha_drag[35] = (2.828427124746191*nuU[4]-4.0*w[2]*nu)*rdvy2; 
  alpha_drag[36] = 2.828427124746191*nuU[5]*rdvy2; 
  alpha_drag[38] = -1.154700538379252*dxv[2]*nu*rdvy2; 
  alpha_drag[46] = 2.828427124746191*nuU[6]*rdvy2; 
  alpha_drag[66] = 2.828427124746191*nuU[7]*rdvy2; 

  // cvard[2]-drag contribution to the midpoint value used for CFL.
  alpha_mid += std::abs(0.125*alpha_drag[35]-0.1397542485937369*alpha_drag[46]); 

  // Expand rdv2*nu*(vz-uz) in phase basis.
  alpha_drag[70] = (2.828427124746191*nuU[8]-4.0*w[3]*nu)*rdvz2; 
  alpha_drag[71] = 2.828427124746191*nuU[9]*rdvz2; 
  alpha_drag[74] = -1.154700538379252*dxv[3]*nu*rdvz2; 
  alpha_drag[81] = 2.828427124746191*nuU[10]*rdvz2; 
  alpha_drag[101] = 2.828427124746191*nuU[11]*rdvz2; 
  ...
  // Diffusion contribution to the midpoint value used for CFL.
  alpha_mid += std::abs((0.5714285714285714*alpha_diffusion[0]-0.6388765649999399*alpha_diffusion[11])*rdvxSq4); 
  alpha_mid += std::abs((0.5714285714285714*alpha_diffusion[0]-0.6388765649999399*alpha_diffusion[11])*rdvySq4); 
  alpha_mid += std::abs((0.5714285714285714*alpha_diffusion[0]-0.6388765649999399*alpha_diffusion[11])*rdvzSq4); 
  ...
  out[12] += 1.677050983124842*(alpha_diffusion[31]*f[31]+alpha_diffusion[11]*f[11]+alpha_diffusion[1]*f[1]+alpha_diffusion[0]*f[0])*rdvxSq4+0.9682458365518543*alpha_drag[11]*f[19]+0.8660254037844386*alpha_drag[2]*f[12]+0.9682458365518543*(alpha_drag[1]*f[5]+alpha_drag[0]*f[2]+f[0]*alpha_drag[2]); 
  out[13] += 1.677050983124842*(alpha_diffusion[31]*f[31]+alpha_diffusion[11]*f[11]+alpha_diffusion[1]*f[1]+alpha_diffusion[0]*f[0])*rdvySq4+0.9682458365518543*f[21]*alpha_drag[46]+0.8660254037844386*f[13]*alpha_drag[38]+0.9682458365518543*(f[0]*alpha_drag[38]+f[6]*alpha_drag[36]+f[3]*alpha_drag[35]); 
  out[14] += 1.677050983124842*(alpha_diffusion[31]*f[31]+alpha_diffusion[11]*f[11]+alpha_diffusion[1]*f[1]+alpha_diffusion[0]*f[0])*rdvzSq4+0.9682458365518543*f[25]*alpha_drag[81]+0.8660254037844386*f[14]*alpha_drag[74]+0.9682458365518543*(f[0]*alpha_drag[74]+f[8]*alpha_drag[71]+f[4]*alpha_drag[70]); 
  ...
  out[22] += 1.677050983124842*(alpha_diffusion[11]*f[21]+alpha_diffusion[1]*f[6]+alpha_diffusion[0]*f[3])*rdvxSq4+0.4330127018922193*(f[22]*alpha_drag[38]+f[20]*alpha_drag[36]+f[12]*alpha_drag[35])+0.8660254037844386*alpha_drag[2]*f[22]+0.9682458365518543*(alpha_drag[1]*f[15]+alpha_drag[0]*f[7]+alpha_drag[2]*f[3]); 
  out[23] += (1.472970759092948*(alpha_diffusion[11]*f[31]+f[11]*alpha_diffusion[31])+1.5*(alpha_diffusion[1]*f[11]+f[1]*alpha_diffusion[11])+1.677050983124842*(alpha_diffusion[0]*f[1]+f[0]*alpha_diffusion[1]))*rdvySq4+0.8504200642707612*f[21]*alpha_drag[66]+0.8660254037844386*f[6]*alpha_drag[46]+(0.8660254037844386*f[23]+0.9682458365518543*f[1])*alpha_drag[38]+0.8660254037844386*f[21]*alpha_drag[36]+0.9682458365518543*(f[3]*alpha_drag[36]+f[6]*alpha_drag[35]); 
  out[24] += 1.677050983124842*(alpha_diffusion[11]*f[19]+alpha_diffusion[1]*f[5]+alpha_diffusion[0]*f[2])*rdvySq4+0.8660254037844386*f[24]*alpha_drag[38]+0.9682458365518543*(f[2]*alpha_drag[38]+f[15]*alpha_drag[36]+f[7]*alpha_drag[35])+0.4330127018922193*(alpha_drag[2]*f[24]+alpha_drag[1]*f[23]+alpha_drag[0]*f[13]); 
  ...
  out[26] += 1.677050983124842*(alpha_diffusion[11]*f[25]+alpha_diffusion[1]*f[8]+alpha_diffusion[0]*f[4])*rdvxSq4+0.4330127018922193*(f[26]*alpha_drag[74]+f[20]*alpha_drag[71]+f[12]*alpha_drag[70])+0.8660254037844386*alpha_drag[2]*f[26]+0.9682458365518543*(alpha_drag[1]*f[16]+alpha_drag[0]*f[9]+alpha_drag[2]*f[4]); 
  out[27] += 1.677050983124842*(alpha_diffusion[11]*f[25]+alpha_diffusion[1]*f[8]+alpha_diffusion[0]*f[4])*rdvySq4+0.4330127018922193*(f[27]*alpha_drag[74]+f[23]*alpha_drag[71]+f[13]*alpha_drag[70])+0.8660254037844386*f[27]*alpha_drag[38]+0.9682458365518543*(f[4]*alpha_drag[38]+f[17]*alpha_drag[36]+f[10]*alpha_drag[35]); 
  out[28] += (1.472970759092948*(alpha_diffusion[11]*f[31]+f[11]*alpha_diffusion[31])+1.5*(alpha_diffusion[1]*f[11]+f[1]*alpha_diffusion[11])+1.677050983124842*(alpha_diffusion[0]*f[1]+f[0]*alpha_diffusion[1]))*rdvzSq4+0.8504200642707612*f[25]*alpha_drag[101]+0.8660254037844386*f[8]*alpha_drag[81]+(0.8660254037844386*f[28]+0.9682458365518543*f[1])*alpha_drag[74]+0.8660254037844386*f[25]*alpha_drag[71]+0.9682458365518543*(f[4]*alpha_drag[71]+f[8]*alpha_drag[70]); 
  out[29] += 1.677050983124842*(alpha_diffusion[11]*f[19]+alpha_diffusion[1]*f[5]+alpha_diffusion[0]*f[2])*rdvzSq4+0.8660254037844386*f[29]*alpha_drag[74]+0.9682458365518543*(f[2]*alpha_drag[74]+f[16]*alpha_drag[71]+f[9]*alpha_drag[70])+0.4330127018922193*(alpha_drag[2]*f[29]+alpha_drag[1]*f[28]+alpha_drag[0]*f[14]); 
  out[30] += 1.677050983124842*(alpha_diffusion[11]*f[21]+alpha_diffusion[1]*f[6]+alpha_diffusion[0]*f[3])*rdvzSq4+0.8660254037844386*f[30]*alpha_drag[74]+0.9682458365518543*(f[3]*alpha_drag[74]+f[17]*alpha_drag[71]+f[10]*alpha_drag[70])+0.4330127018922193*(f[30]*alpha_drag[38]+f[28]*alpha_drag[36]+f[14]*alpha_drag[35]); 
  out[32] += 5.7282196186948*(alpha_diffusion[11]*f[19]+alpha_diffusion[1]*f[5]+alpha_diffusion[0]*f[2])*rdvxSq4+1.299038105676658*alpha_drag[2]*f[32]+0.6614378277661477*alpha_drag[31]*f[31]+1.479019945774904*(alpha_drag[1]*f[20]+alpha_drag[0]*f[12])+0.6614378277661477*alpha_drag[11]*f[11]+1.984313483298443*alpha_drag[2]*f[2]+0.6614378277661477*(alpha_drag[1]*f[1]+alpha_drag[0]*f[0]); 
  out[33] += 5.7282196186948*(alpha_diffusion[11]*f[21]+alpha_diffusion[1]*f[6]+alpha_diffusion[0]*f[3])*rdvySq4+0.6614378277661477*(f[31]*alpha_drag[66]+f[11]*alpha_drag[46])+(1.299038105676658*f[33]+1.984313483298443*f[3])*alpha_drag[38]+(1.479019945774904*f[23]+0.6614378277661477*f[1])*alpha_drag[36]+(1.479019945774904*f[13]+0.6614378277661477*f[0])*alpha_drag[35]; 
  out[34] += 5.7282196186948*(alpha_diffusion[11]*f[25]+alpha_diffusion[1]*f[8]+alpha_diffusion[0]*f[4])*rdvzSq4+0.6614378277661477*(f[31]*alpha_drag[101]+f[11]*alpha_drag[81])+(1.299038105676658*f[34]+1.984313483298443*f[4])*alpha_drag[74]+(1.479019945774904*f[28]+0.6614378277661477*f[1])*alpha_drag[71]+(1.479019945774904*f[14]+0.6614378277661477*f[0])*alpha_drag[70]; 
