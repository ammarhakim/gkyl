...
double VmLBOconstNuSurfNonUniform2x3vMax_VX_P1(const double *wl, const double *wr, const double *dxvl, const double *dxvr, const double nuSum, const double vMuMidMax, const double *nuUSum, const double *nuVtSqSum, const double *fl, const double *fr, double *outl, double *outr) 
{ 
  ...
  Ghat[5] = (9.0*nuVtSqSum[0]*dxvl[2]*dxvr[2]*fr[5]-9.0*nuVtSqSum[0]*dxvl[2]*dxvr[2]*fl[5])/(dxvr2R3+3.0*dxvl[2]*dxvr2R2+3.0*dxvl2R2*dxvr[2]+dxvl2R3)-0.5*fjump[5]+0.25*alphaDrag[0]*favg[5]; 
...
double VmLBOconstNuSurfNonUniform2x3vMax_VY_P1(const double *wl, const double *wr, const double *dxvl, const double *dxvr, const double nuSum, const double vMuMidMax, const double *nuUSum, const double *nuVtSqSum, const double *fl, const double *fr, double *outl, double *outr) 
{ 
  // w[5]:          Cell-center coordinates. 
  // dxv[5]:        Cell spacing. 
  // nuSum:         collisionalities added (self and cross species collisionalities). 
  ...
  Ghat[5] = (9.0*nuVtSqSum[0]*dxvl[3]*dxvr[3]*fr[5]-9.0*nuVtSqSum[0]*dxvl[3]*dxvr[3]*fl[5])/(dxvr3R3+3.0*dxvl[3]*dxvr3R2+3.0*dxvl3R2*dxvr[3]+dxvl3R3)-0.5*fjump[5]+0.25*alphaDrag[0]*favg[5]; 
...
double VmLBOconstNuSurfNonUniform2x3vMax_VZ_P1(const double *wl, const double *wr, const double *dxvl, const double *dxvr, const double nuSum, const double vMuMidMax, const double *nuUSum, const double *nuVtSqSum, const double *fl, const double *fr, double *outl, double *outr) 
{ 
  // w[5]:          Cell-center coordinates. 
  // dxv[5]:        Cell spacing. 
  // nuSum:         collisionalities added (self and cross species collisionalities). 
  ...
  Ghat[0] = (-(1.0*((8.660254037844386*nuVtSqSum[0]*dxvl[4]*dxvr4R2+1.732050807568877*nuVtSqSum[0]*dxvl4R2*dxvr[4]-1.732050807568877*nuVtSqSum[0]*dxvl4R3)*fr[5]+((-1.732050807568877*nuVtSqSum[0]*dxvr4R3)+1.732050807568877*nuVtSqSum[0]*dxvl[4]*dxvr4R2+8.660254037844386*nuVtSqSum[0]*dxvl4R2*dxvr[4])*fl[5]+((18.0*fl[2]-18.0*fr[2])*nuVtSqSum[2]+(18.0*fl[1]-18.0*fr[1])*nuVtSqSum[1]+(18.0*fl[0]-18.0*fr[0])*nuVtSqSum[0])*dxvl[4]*dxvr[4]))/(2.0*dxvr4R3+6.0*dxvl[4]*dxvr4R2+6.0*dxvl4R2*dxvr[4]+2.0*dxvl4R3))-0.5*(1.732050807568877*fjump[5]+fjump[0])+0.25*(1.732050807568877*alphaDrag[0]*favg[5]+alphaDrag[2]*favg[2]+alphaDrag[1]*favg[1]+alphaDrag[0]*favg[0]); 
  Ghat[1] = (-(1.0*((8.660254037844386*nuVtSqSum[1]*dxvl[4]*dxvr4R2+1.732050807568877*nuVtSqSum[1]*dxvl4R2*dxvr[4]-1.732050807568877*nuVtSqSum[1]*dxvl4R3)*fr[5]+((-1.732050807568877*nuVtSqSum[1]*dxvr4R3)+1.732050807568877*nuVtSqSum[1]*dxvl[4]*dxvr4R2+8.660254037844386*nuVtSqSum[1]*dxvl4R2*dxvr[4])*fl[5]+((18.0*fl[0]-18.0*fr[0])*nuVtSqSum[1]-18.0*nuVtSqSum[0]*fr[1]+18.0*nuVtSqSum[0]*fl[1])*dxvl[4]*dxvr[4]))/(2.0*dxvr4R3+6.0*dxvl[4]*dxvr4R2+6.0*dxvl4R2*dxvr[4]+2.0*dxvl4R3))+0.25*(1.732050807568877*alphaDrag[1]*favg[5]+alphaDrag[0]*favg[1]+favg[0]*alphaDrag[1])-0.5*fjump[1]; 
  Ghat[2] = (-(1.0*((8.660254037844386*nuVtSqSum[2]*dxvl[4]*dxvr4R2+1.732050807568877*nuVtSqSum[2]*dxvl4R2*dxvr[4]-1.732050807568877*nuVtSqSum[2]*dxvl4R3)*fr[5]+((-1.732050807568877*nuVtSqSum[2]*dxvr4R3)+1.732050807568877*nuVtSqSum[2]*dxvl[4]*dxvr4R2+8.660254037844386*nuVtSqSum[2]*dxvl4R2*dxvr[4])*fl[5]+((18.0*fl[0]-18.0*fr[0])*nuVtSqSum[2]-18.0*nuVtSqSum[0]*fr[2]+18.0*nuVtSqSum[0]*fl[2])*dxvl[4]*dxvr[4]))/(2.0*dxvr4R3+6.0*dxvl[4]*dxvr4R2+6.0*dxvl4R2*dxvr[4]+2.0*dxvl4R3))+0.25*(1.732050807568877*alphaDrag[2]*favg[5]+alphaDrag[0]*favg[2]+favg[0]*alphaDrag[2])-0.5*fjump[2]; 
  Ghat[3] = ((9.0*nuVtSqSum[0]*fr[3]-9.0*nuVtSqSum[0]*fl[3])*dxvl[4]*dxvr[4])/(dxvr4R3+3.0*dxvl[4]*dxvr4R2+3.0*dxvl4R2*dxvr[4]+dxvl4R3)-0.5*fjump[3]+0.25*alphaDrag[0]*favg[3]; 
  Ghat[4] = (9.0*nuVtSqSum[0]*dxvl[4]*dxvr[4]*fr[4]-9.0*nuVtSqSum[0]*dxvl[4]*dxvr[4]*fl[4])/(dxvr4R3+3.0*dxvl[4]*dxvr4R2+3.0*dxvl4R2*dxvr[4]+dxvl4R3)-0.5*fjump[4]+0.25*alphaDrag[0]*favg[4]; 
  ...
  incr2[5] = -(1.0*((5.0*nuVtSqSum[0]*dxvl4R2*dxvr4R2+3.0*nuVtSqSum[0]*dxvl4R3*dxvr[4])*fr[5]+((-3.0*nuVtSqSum[0]*dxvl[4]*dxvr4R3)-5.0*nuVtSqSum[0]*dxvl4R2*dxvr4R2)*fl[5]+((-3.464101615137754*fl[2]*nuVtSqSum[2])-3.464101615137754*fl[1]*nuVtSqSum[1]-3.464101615137754*fl[0]*nuVtSqSum[0])*dxvr4R3+((-10.39230484541326*fl[2]*nuVtSqSum[2])-10.39230484541326*fl[1]*nuVtSqSum[1]-10.39230484541326*fl[0]*nuVtSqSum[0])*dxvl[4]*dxvr4R2+((-10.39230484541326*fr[2]*nuVtSqSum[2])-10.39230484541326*fr[1]*nuVtSqSum[1]-10.39230484541326*fr[0]*nuVtSqSum[0])*dxvl4R2*dxvr[4]+((-3.464101615137754*fr[2]*nuVtSqSum[2])-3.464101615137754*fr[1]*nuVtSqSum[1]-3.464101615137754*fr[0]*nuVtSqSum[0])*dxvl4R3))/(8.0*dxvr4R3+24.0*dxvl[4]*dxvr4R2+24.0*dxvl4R2*dxvr[4]+8.0*dxvl4R3); 
