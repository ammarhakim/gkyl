...
function _M.selectVol(basisNm, CDIM, VDIM, polyOrder, isElectromagnetic)
   ...
   local funcNm = string.format("%sGyrokineticGenGeoVol%dx%dv%sP%d", emString, CDIM, VDIM, basisNmMap[basisNm], polyOrder) .. bvarString
...
function _M.selectSurf(basisNm, CDIM, VDIM, polyOrder, isElectromagnetic, positivity)
   ...
   if CDIM == 1 and VDIM <= 2 then
      funcNm[1] = string.format("%sGyrokineticGenGeoSurf%s%dx%dv%s_x_P%d", emString, posString, CDIM, VDIM, basisNmMap[basisNm], polyOrder) .. bvarString
      funcNm[2] = string.format("%sGyrokineticGenGeoSurf%s%dx%dv%s_vpar_P%d", emString, posString, CDIM, VDIM, basisNmMap[basisNm], polyOrder) .. bvarString
   elseif CDIM == 2 and VDIM == 2 then
      funcNm[1] = string.format("%sGyrokineticGenGeoSurf%s%dx%dv%s_x_P%d", emString, posString, CDIM, VDIM, basisNmMap[basisNm], polyOrder) .. bvarString
      funcNm[2] = string.format("%sGyrokineticGenGeoSurf%s%dx%dv%s_y_P%d", emString, posString, CDIM, VDIM, basisNmMap[basisNm], polyOrder) .. bvarString
      funcNm[3] = string.format("%sGyrokineticGenGeoSurf%s%dx%dv%s_vpar_P%d", emString, posString, CDIM, VDIM, basisNmMap[basisNm], polyOrder) .. bvarString
   elseif CDIM == 3 and VDIM == 2 then
      funcNm[1] = string.format("%sGyrokineticGenGeoSurf%s%dx%dv%s_x_P%d", emString, posString, CDIM, VDIM, basisNmMap[basisNm], polyOrder) .. bvarString
      funcNm[2] = string.format("%sGyrokineticGenGeoSurf%s%dx%dv%s_y_P%d", emString, posString, CDIM, VDIM, basisNmMap[basisNm], polyOrder) .. bvarString
      funcNm[3] = string.format("%sGyrokineticGenGeoSurf%s%dx%dv%s_z_P%d", emString, posString, CDIM, VDIM, basisNmMap[basisNm], polyOrder) .. bvarString
      funcNm[4] = string.format("%sGyrokineticGenGeoSurf%s%dx%dv%s_vpar_P%d", emString, posString, CDIM, VDIM, basisNmMap[basisNm], polyOrder) .. bvarString
...
function _M.selectStep2Vol(basisNm, CDIM, VDIM, polyOrder)
   local funcType = "double"
   local funcNm = string.format("EmGyrokineticGenGeoStep2Vol%dx%dv%sP%d", CDIM, VDIM, basisNmMap[basisNm], polyOrder)
...
function _M.selectStep2Surf(basisNm, CDIM, VDIM, polyOrder, positivity)
   ...
   local funcNm
   if polyOrder > 1 then
      funcNm = string.format("EmGyrokineticGenGeoSurf%s%dx%dv%s_vpar_P%d", posString, CDIM, VDIM, basisNmMap[basisNm], polyOrder) .. bvarString
   else
      funcNm = string.format("EmGyrokineticGenGeoSurf%s%dx%dv%sStep2_vpar_P%d", posString, CDIM, VDIM, basisNmMap[basisNm], polyOrder) .. bvarString
   end
