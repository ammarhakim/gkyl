...
// w[NDIM]: Cell-center coordinates. dxv[NDIM]: Cell spacing. H/f: Input Hamiltonian/distribution function. out: Incremented output 
  ...
  double alpha[3]; 
  alpha[0] = -0.3535533905932737*geoZ[0]*(3.872983346207417*Phi[6]-3.0*Phi[3]+1.732050807568877*Phi[2])*dfac_y; 
  alpha[1] = 0.3535533905932737*geoZ[0]*(6.708203932499369*Phi[7]-3.872983346207417*Phi[5])*dfac_y; 
  double amax = 0.0; 
  bool upwind = false; 
  if(upwind) 
    amax = std::abs(alpha0); 
...
//
// //
  Ghat[0] = -0.25*(1.414213562373095*((fr[0]-1.0*fl[0])*amax-1.0*(alpha[1]*favg[1]+alpha[0]*favg[0]))+(3.16227766016838*(fr[4]-1.0*fl[4])-2.449489742783178*(fr[1]+fl[1]))*amax); 
  Ghat[1] = -0.01666666666666667*(1.414213562373095*((15.0*fr[2]-15.0*fl[2])*amax-15.0*(alpha[0]*favg[1]+favg[0]*alpha[1]))+(9.48683298050514*(5.0*fr[6]-5.0*fl[6])-36.74234614174767*(fr[3]+fl[3]))*amax-18.97366596101028*alpha[1]*favg[2]); 
  Ghat[2] = 0.05*(1.414213562373095*((5.0*fl[5]-5.0*fr[5])*amax+5.0*alpha[0]*favg[2])+12.24744871391589*(fr[7]+fl[7])*amax+6.324555320336761*alpha[1]*favg[1]); 
...
// w[NDIM]: Cell-center coordinates. dxv[NDIM]: Cell spacing. H/f: Input Hamiltonian/distribution function. out: Incremented output 
  ...
  double alpha[3]; 
  alpha[0] = 0.3535533905932737*geoZ[0]*(3.872983346207417*Phi[7]-3.0*Phi[3]+1.732050807568877*Phi[1])*dfac_x; 
  alpha[1] = -0.3535533905932737*geoZ[0]*(6.708203932499369*Phi[6]-3.872983346207417*Phi[4])*dfac_x; 
  double amax = 0.0; 
  bool upwind = false; 
  if(upwind) 
    amax = std::abs(alpha0); 
...
//
// //
  Ghat[0] = -0.25*(1.414213562373095*((fr[0]-1.0*fl[0])*amax-1.0*(alpha[1]*favg[1]+alpha[0]*favg[0]))+(3.16227766016838*(fr[5]-1.0*fl[5])-2.449489742783178*(fr[2]+fl[2]))*amax); 
  Ghat[1] = -0.01666666666666667*(1.414213562373095*((15.0*fr[1]-15.0*fl[1])*amax-15.0*(alpha[0]*favg[1]+favg[0]*alpha[1]))+(9.48683298050514*(5.0*fr[7]-5.0*fl[7])-36.74234614174767*(fr[3]+fl[3]))*amax-18.97366596101028*alpha[1]*favg[2]); 
  Ghat[2] = 0.05*(1.414213562373095*((5.0*fl[4]-5.0*fr[4])*amax+5.0*alpha[0]*favg[2])+12.24744871391589*(fr[6]+fl[6])*amax+6.324555320336761*alpha[1]*favg[1]); 
