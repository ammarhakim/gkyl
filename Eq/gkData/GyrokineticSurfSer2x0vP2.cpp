...
double GyrokineticSurf2x0vSer_X_P2_Bvars_0(const double q_, const double m_,  const double *w, const double *dxv, const double amax_in, const double *Bmag, const double *BmagInv, const double *Gradpar, const double *BdriftX, const double *BdriftY, const double *Phi, const double *fl, const double *fr, double *outl, double *outr) 
{ 
// w[NDIM]: Cell-center coordinates. dxv[NDIM]: Cell spacing. H/f: Input Hamiltonian/distribution function. out: Incremented output 
  ...
  double alpha[3]; 
  alpha[0] = -0.3535533905932737*BmagInv[0]*(3.872983346207417*Phi[6]-3.0*Phi[3]+1.732050807568877*Phi[2])*dfac_y; 
  alpha[1] = 0.3535533905932737*BmagInv[0]*(6.708203932499369*Phi[7]-3.872983346207417*Phi[5])*dfac_y; 
  double amax = 0.0; 
  bool upwind = false; 
  if(upwind) 
    amax = std::abs(alpha0); 
  ...
  Ghat[0] = -0.25*((3.16227766016838*fr[4]-3.16227766016838*fl[4]-2.449489742783178*(fr[1]+fl[1])+1.414213562373095*fr[0]-1.414213562373095*fl[0])*amax-1.414213562373095*alpha[0]*favg[0]); 
  Ghat[1] = -0.08333333333333333*((9.48683298050514*fr[6]-9.48683298050514*fl[6]-7.348469228349534*(fr[3]+fl[3])+4.242640687119286*fr[2]-4.242640687119286*fl[2])*amax-4.242640687119286*alpha[0]*favg[1]); 
  Ghat[2] = 0.05*((12.24744871391589*(fr[7]+fl[7])-7.071067811865476*fr[5]+7.071067811865476*fl[5])*amax+7.071067811865476*alpha[0]*favg[2]); 
...
double GyrokineticSurf2x0vSer_Y_P2_Bvars_0(const double q_, const double m_,  const double *w, const double *dxv, const double amax_in, const double *Bmag, const double *BmagInv, const double *Gradpar, const double *BdriftX, const double *BdriftY, const double *Phi, const double *fl, const double *fr, double *outl, double *outr) 
{ 
// w[NDIM]: Cell-center coordinates. dxv[NDIM]: Cell spacing. H/f: Input Hamiltonian/distribution function. out: Incremented output 
  double dfac_x = 2.0/dxv[0]; 
  double dfac_y = 2.0/dxv[1]; 
  ...
  double alpha[3]; 
  alpha[0] = 0.3535533905932737*BmagInv[0]*(3.872983346207417*Phi[7]-3.0*Phi[3]+1.732050807568877*Phi[1])*dfac_x; 
  alpha[1] = -0.3535533905932737*BmagInv[0]*(6.708203932499369*Phi[6]-3.872983346207417*Phi[4])*dfac_x; 
  double amax = 0.0; 
  bool upwind = false; 
  if(upwind) 
    amax = std::abs(alpha0); 
