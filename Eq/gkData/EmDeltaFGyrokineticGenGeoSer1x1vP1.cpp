...
double EmDeltaFGyrokineticGenGeoVol1x1vSerP1_Bvars(const double q_, const double m_, const double *w, const double *dxv, const double *bmag, const double *jacobTotInv, const double *cmag, const double *b_x, const double *b_y, const double *b_z, const double *phi, const double *Apar, const double* dApardt, const double *f0, const double *f1, double *out) 
{ 
  ...
  out[1] += 0.8660254037844386*alpha0x[0]*f1[0]+0.8660254037844386*alpha1x[0]*f0[0]; 
  out[2] += 0.8660254037844386*alpha1vpar[0]*f0[0]; 
  out[3] += 0.8660254037844386*alpha0x[0]*f1[2]+0.8660254037844386*alpha1x[0]*f0[2]+0.8660254037844386*alpha1vpar[0]*f0[1]; 
  out[1] += 0.8660254037844386*alpha1x[0]*f1[0]; 
  out[2] += 0.8660254037844386*alpha1vpar[0]*f1[0]; 
  out[3] += 0.8660254037844386*alpha1x[0]*f1[2]+0.8660254037844386*alpha1vpar[0]*f1[1]; 
  return cflFreq; 
} 
double EmDeltaFGyrokineticGenGeoVol1x1vSerP1_Bvarsx(const double q_, const double m_, const double *w, const double *dxv, const double *bmag, const double *jacobTotInv, const double *cmag, const double *b_x, const double *b_y, const double *b_z, const double *phi, const double *Apar, const double* dApardt, const double *f0, const double *f1, double *out) 
{ 
  // Apar: parallel component of magnetic vector potential.
  // dApardt: time derivative of Apar.
  // jacobTotInv: reciprocal of the conf-space jacobian time the guiding center coordinate Jacobian.
  ...
  out[1] += 0.8660254037844386*alpha0x[3]*f1[3]+0.8660254037844386*alpha0x[2]*f1[2]+0.8660254037844386*alpha0x[1]*f1[1]+0.8660254037844386*alpha1x[1]*f0[1]+0.8660254037844386*alpha0x[0]*f1[0]+0.8660254037844386*alpha1x[0]*f0[0]; 
  out[2] += 0.8660254037844386*alpha1vpar[3]*f0[3]+0.8660254037844386*alpha1vpar[2]*f0[2]+0.8660254037844386*alpha1vpar[1]*f0[1]+0.8660254037844386*alpha1vpar[0]*f0[0]; 
  out[3] += 0.8660254037844386*alpha0x[1]*f1[3]+0.8660254037844386*alpha1vpar[2]*f0[3]+0.8660254037844386*alpha1x[1]*f0[3]+0.8660254037844386*f0[2]*alpha1vpar[3]+0.8660254037844386*f1[1]*alpha0x[3]+0.8660254037844386*alpha0x[0]*f1[2]+0.8660254037844386*alpha1x[0]*f0[2]+0.8660254037844386*f1[0]*alpha0x[2]+0.8660254037844386*alpha1vpar[0]*f0[1]+0.8660254037844386*f0[0]*alpha1vpar[1]; 
  out[1] += 0.8660254037844386*alpha1x[1]*f1[1]+0.8660254037844386*alpha1x[0]*f1[0]; 
  out[2] += 0.8660254037844386*alpha1vpar[3]*f1[3]+0.8660254037844386*alpha1vpar[2]*f1[2]+0.8660254037844386*alpha1vpar[1]*f1[1]+0.8660254037844386*alpha1vpar[0]*f1[0]; 
  out[3] += 0.8660254037844386*alpha1vpar[2]*f1[3]+0.8660254037844386*alpha1x[1]*f1[3]+0.8660254037844386*f1[2]*alpha1vpar[3]+0.8660254037844386*alpha1x[0]*f1[2]+0.8660254037844386*alpha1vpar[0]*f1[1]+0.8660254037844386*f1[0]*alpha1vpar[1]; 
  return cflFreq; 
} 
double EmDeltaFGyrokineticGenGeoStep2Vol1x1vSerP1(const double q_, const double m_, const double *w, const double *dxv, const double *dApardt, const double *f0, const double *f1, double *out) 
{ 
  // q_,m_: species charge and mass.
  // w[NDIM]: cell-center.
  // dxv[NDIM]: cell length.
  ...
  out[3] += -(1.224744871391589*(dApardt[0]*f1[1]+f1[0]*dApardt[1])*q_*rdvpar2)/m_; 
  double cflFreq = 0.0; 
  double alphaL = 0.0; 
  double alphaR = 0.0; 
  // Evaluate alpha at left surface quadrature points.
