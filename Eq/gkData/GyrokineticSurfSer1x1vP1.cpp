...
double alphaQuad;
  ...
  incr[2] = 0.5*(alpha[0]*fupwind[1]+fupwind[0]*alpha[1])*dfac_x; 
  incr[3] = -0.8660254037844386*(alpha[0]*fupwind[1]+fupwind[0]*alpha[1])*dfac_x; 
...
// w[NDIM]: Cell-center coordinates. dxv[NDIM]: Cell spacing. H/f: Input Hamiltonian/distribution function. out: Incremented output 
  ...
  double alpha[2]; 
  alpha[0] = (0.25*Phi[1]*dfac_x*(dfac_v*(4.242640687119286*Bmag[1]*(BmagInv[1]*geoY[1]+BmagInv[0]*geoY[0])*dfac_x*m_*wv-4.898979485566357*Gradpar[0]*q_)-4.242640687119286*Bmag[1]*(BmagInv[1]*geoY[1]+BmagInv[0]*geoY[0])*dfac_x*m_))/(dfac_v*m_); 
  alpha[1] = (0.25*Phi[1]*dfac_x*(dfac_v*(4.242640687119286*Bmag[1]*(BmagInv[0]*geoY[1]+geoY[0]*BmagInv[1])*dfac_x*m_*wv-4.898979485566357*Gradpar[1]*q_)-4.242640687119286*Bmag[1]*(BmagInv[0]*geoY[1]+geoY[0]*BmagInv[1])*dfac_x*m_))/(dfac_v*m_); 
#if upwindType == SURFAVG 
  if (alpha0>0) { 
