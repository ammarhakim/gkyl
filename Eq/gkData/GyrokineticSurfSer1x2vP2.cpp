...
// w[NDIM]: Cell-center coordinates. dxv[NDIM]: Cell spacing. H/f: Input Hamiltonian/distribution function. out: Incremented output 
  ...
  alpha[1] = -(5.477225575051662*dfac_x*(Bmag[2]*wm+Phi[2]*q_))/m_; 
...
//
// //
  Ghat[0] = -0.1767766952966368*((4.47213595499958*fr[8]-4.47213595499958*fl[8]-3.464101615137754*(fr[2]+fl[2])+2.0*fr[0]-2.0*fl[0])*amax-1.414213562373095*(alpha[3]*favg[3]+alpha[2]*favg[2]+alpha[1]*favg[1]+alpha[0]*favg[0])); 
  Ghat[1] = -0.01178511301977579*((67.08203932499369*fr[12]-67.08203932499369*fl[12]-51.96152422706631*(fr[4]+fl[4])+30.0*fr[1]-30.0*fl[1])*amax-18.97366596101028*(alpha[3]*favg[6]+alpha[1]*favg[4])-21.21320343559643*(alpha[2]*favg[3]+favg[2]*alpha[3]+alpha[0]*favg[1]+favg[0]*alpha[1])); 
  Ghat[2] = -0.02041241452319315*((38.72983346207417*fr[14]-38.72983346207417*fl[14]-30.0*(fr[6]+fl[6])+17.32050807568877*fr[3]-17.32050807568877*fl[3])*amax-10.95445115010333*alpha[3]*favg[7]-10.95445115010332*alpha[2]*favg[5]-12.24744871391589*(alpha[1]*favg[3]+favg[1]*alpha[3]+alpha[0]*favg[2]+favg[0]*alpha[2])); 
  Ghat[3] = -7.365695637359863e-4*((1073.312629199899*fr[18]-1073.312629199899*fl[18]-831.384387633061*(fr[10]+fl[10])+480.0*fr[5]-480.0*fl[5])*amax+((-303.5786553761646*alpha[2])-82.15838362577493*alpha[0])*favg[7]-303.5786553761646*(alpha[1]*favg[6]+alpha[3]*(favg[5]+favg[4]))-339.411254969543*(alpha[0]*favg[3]+favg[0]*alpha[3]+alpha[1]*favg[2]+favg[1]*alpha[2])); 
  Ghat[4] = 0.01178511301977579*((51.96152422706632*(fr[11]+fl[11])-30.0*fr[7]+30.0*fl[7])*amax+21.21320343559643*alpha[2]*favg[6]+21.21320343559643*alpha[0]*favg[4]+18.97366596101028*(alpha[3]*favg[3]+alpha[1]*favg[1])); 
  ...
  Ghat[6] = 0.009128709291752763*((67.0820393249937*(fr[17]+fl[17])-38.72983346207417*fr[13]+38.72983346207417*fl[13])*amax+21.90890230020666*alpha[3]*favg[7]+27.38612787525831*alpha[0]*favg[6]+27.38612787525831*alpha[2]*favg[4]+24.49489742783179*(alpha[1]*favg[3]+favg[1]*alpha[3])); 
  Ghat[7] = 0.01178511301977579*((51.96152422706632*(fr[19]+fl[19])-30.0*fr[15]+30.0*fl[15])*amax+21.21320343559643*alpha[0]*favg[7]+16.97056274847715*alpha[3]*favg[6]+21.21320343559643*alpha[1]*favg[5]+18.97366596101028*(alpha[2]*favg[3]+favg[2]*alpha[3])); 
  incr[0] = 0.7071067811865475*Ghat[0]*dfac_v; 
  incr[1] = 0.7071067811865475*Ghat[1]*dfac_v; 
  incr[2] = -1.224744871391589*Ghat[0]*dfac_v; 
  incr[3] = 0.7071067811865475*Ghat[2]*dfac_v; 
