...
// w[NDIM]: Cell-center coordinates. dxv[NDIM]: Cell spacing. H/f: Input Hamiltonian/distribution function. out: Incremented output 
  ...
  outX[1] += 0.8660254037844386*alphax[0]*f[0]; 
  outX[3] += 0.8660254037844386*alphax[0]*f[2]; 
  double alphav[4]; 
  ...
  outV[2] += 0.8660254037844386*alphav[0]*f[0]; 
  outV[3] += 0.8660254037844386*alphav[0]*f[1]; 
  return cflRate; 
} 
double EmGyrokineticStep2VolPositivity1x1vSer_P1(const double q_, const double m_, const double *w, const double *dxv, 
...
#endif 
  cflRateByDir[0] += cflRateByDir[1];
  outX[1] += 0.8660254037844386*(alphax[1]*f[1]+alphax[0]*f[0]); 
  outX[3] += 0.8660254037844386*(alphax[1]*f[3]+alphax[0]*f[2]); 
  double alphav[4]; 
  alphav[0] = -(1.732050807568877*Gradpar[0]*Phi[1]*dfac_v*dfac_x*q_)/m_; 
...
#endif 
  cflRateByDir[0] += cflRateByDir[2];
  outV[2] += 0.8660254037844386*(alphav[1]*f[1]+alphav[0]*f[0]); 
  outV[3] += 0.8660254037844386*(alphav[0]*f[1]+f[0]*alphav[1]); 
  return cflRate; 
} 
