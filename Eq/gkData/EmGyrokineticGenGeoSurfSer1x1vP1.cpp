#include <GyrokineticGenGeoModDecl.h> 
double EmGyrokineticGenGeoSurf1x1vSer_X_P1_Bvars_0(const double q_, const double m_, const double cflL, const double cflR, const double *w, const double *dxv, const double amax_in, const double *Bmag, const double *jacobTotInv, const double *Gradpar, const double *geoX, const double *geoY, const double *geoZ, const double *Phi, const double *Apar, const double *AparL, const double *dApardt, const double *dApardtPrev, const double *fl, const double *fr, double *outl, double *outr, double *emModL, double *emModR) 
{ 
// w[NDIM]: Cell-center coordinates. dxv[NDIM]: Cell spacing. H/f: Input Hamiltonian/distribution function. out: Incremented output 
  double dfac_z = 2.0/dxv[0]; 
  double dfac_v = 2.0/dxv[1]; 
  double wz = w[0]; 
  double wv = w[1]; 
  double wv2 = wv*wv; 
  double dfac_v2 = dfac_v*dfac_v; 
  double q2 = q_*q_; 
  double incr[4]; 
  double hamil[4]; 
  hamil[0] = (0.3333333333333333*(3.0*dfac_v2*(m_*wv2+1.414213562373095*Phi[0]*q_)+m_))/dfac_v2; 
  hamil[1] = 1.414213562373095*Phi[1]*q_; 
  hamil[2] = (1.154700538379252*m_*wv)/dfac_v; 
  double BstarZ_by_Bmag[4]; 
  BstarZ_by_Bmag[0] = Gradpar[0]*jacobTotInv[0]; 

  double BstarZ_by_BmagL[4]; 
  BstarZ_by_BmagL[0] = Gradpar[0]*jacobTotInv[0]; 

  // surface-averaged phase velocity in this direction 
  double alpha0 = (0.1082531754730548*(BstarZ_by_BmagL[0]+BstarZ_by_Bmag[0])*hamil[2]*dfac_v)/m_; 

  double alpha[2]; 
  alpha[0] = (0.3061862178478971*(BstarZ_by_BmagL[0]+BstarZ_by_Bmag[0])*hamil[2]*dfac_v)/m_; 
#if upwindType == SURFAVG 
  if (alpha0>0) { 
  incr[0] = 0.3535533905932737*alpha[0]*(1.732050807568877*fl[1]+fl[0])*dfac_z; 
  incr[1] = -0.3535533905932737*alpha[0]*(3.0*fl[1]+1.732050807568877*fl[0])*dfac_z; 
  incr[2] = 0.3535533905932737*alpha[0]*(1.732050807568877*fl[3]+fl[2])*dfac_z; 
  incr[3] = -0.3535533905932737*alpha[0]*(3.0*fl[3]+1.732050807568877*fl[2])*dfac_z; 
  } else { 
  incr[0] = -0.3535533905932737*alpha[0]*(1.732050807568877*fr[1]-1.0*fr[0])*dfac_z; 
  incr[1] = 0.3535533905932737*alpha[0]*(3.0*fr[1]-1.732050807568877*fr[0])*dfac_z; 
  incr[2] = -0.3535533905932737*alpha[0]*(1.732050807568877*fr[3]-1.0*fr[2])*dfac_z; 
  incr[3] = 0.3535533905932737*alpha[0]*(3.0*fr[3]-1.732050807568877*fr[2])*dfac_z; 
  }
#elif upwindType == QUAD 
double fupwind[2];
double fupwindQuad[2];
double alphaQuad;
  alphaQuad = 0.7071067811865475*alpha[0]; 
  fupwindQuad[0] = 0.5*(((-0.8660254037844386*(fr[3]+fl[3]))+0.5*fr[2]-0.5*fl[2]+0.8660254037844386*(fr[1]+fl[1])-0.5*fr[0]+0.5*fl[0])*sgn(alphaQuad)+0.8660254037844386*fr[3]-0.8660254037844386*fl[3]-0.5*(fr[2]+fl[2])-0.8660254037844386*fr[1]+0.8660254037844386*fl[1]+0.5*(fr[0]+fl[0])); 
  alphaQuad = 0.7071067811865475*alpha[0]; 
  fupwindQuad[1] = 0.5*((0.8660254037844386*(fr[3]+fl[3])-0.5*fr[2]+0.5*fl[2]+0.8660254037844386*(fr[1]+fl[1])-0.5*fr[0]+0.5*fl[0])*sgn(alphaQuad)-0.8660254037844386*fr[3]+0.8660254037844386*fl[3]+0.5*(fr[2]+fl[2])-0.8660254037844386*fr[1]+0.8660254037844386*fl[1]+0.5*(fr[0]+fl[0])); 
  fupwind[0] = 0.7071067811865475*(fupwindQuad[1]+fupwindQuad[0]); 
  fupwind[1] = 0.7071067811865475*(fupwindQuad[1]-1.0*fupwindQuad[0]); 
  incr[0] = 0.5*alpha[0]*fupwind[0]*dfac_z; 
  incr[1] = -0.8660254037844386*alpha[0]*fupwind[0]*dfac_z; 
  incr[2] = 0.5*alpha[0]*fupwind[1]*dfac_z; 
  incr[3] = -0.8660254037844386*alpha[0]*fupwind[1]*dfac_z; 

#endif 
  outr[0] += incr[0]; 
  outr[1] += incr[1]; 
  outr[2] += incr[2]; 
  outr[3] += incr[3]; 

  outl[0] += -1.0*incr[0]; 
  outl[1] += incr[1]; 
  outl[2] += -1.0*incr[2]; 
  outl[3] += incr[3]; 
  return std::abs(alpha0); 
} 
double EmGyrokineticGenGeoSurf1x1vSer_Vpar_P1_Bvars_0(const double q_, const double m_, const double cflL, const double cflR, const double *w, const double *dxv, const double amax_in, const double *Bmag, const double *jacobTotInv, const double *Gradpar, const double *geoX, const double *geoY, const double *geoZ, const double *Phi, const double *Apar, const double *AparL, const double *dApardt, const double *dApardtPrev, const double *fl, const double *fr, double *outl, double *outr, double *emModL, double *emModR) 
{ 
// w[NDIM]: Cell-center coordinates. dxv[NDIM]: Cell spacing. H/f: Input Hamiltonian/distribution function. out: Incremented output 
  double dfac_z = 2.0/dxv[0]; 
  double dfac_v = 2.0/dxv[1]; 
  double wz = w[0]; 
  double wv = w[1]; 
  double wv2 = wv*wv; 
  double dfac_v2 = dfac_v*dfac_v; 
  double q2 = q_*q_; 
  double incr[4]; 
  double incrEmMod[4]; 
  double hamil[4]; 
  hamil[0] = (0.3333333333333333*(3.0*dfac_v2*(m_*wv2+1.414213562373095*Phi[0]*q_)+m_))/dfac_v2; 
  hamil[1] = 1.414213562373095*Phi[1]*q_; 
  hamil[2] = (1.154700538379252*m_*wv)/dfac_v; 
  double BstarZ_by_Bmag[4]; 
  BstarZ_by_Bmag[0] = Gradpar[0]*jacobTotInv[0]; 

  // surface-averaged phase velocity in this direction 
  double alpha0 = -(0.125*(2.828427124746191*dApardtPrev[0]*q_+1.732050807568877*BstarZ_by_Bmag[0]*hamil[1]*dfac_z))/m_; 

  double alpha[2]; 
  alpha[0] = -(0.6123724356957944*BstarZ_by_Bmag[0]*hamil[1]*dfac_z)/m_; 
  double alphaUp[2]; 
  alphaUp[0] = -(0.3535533905932737*(2.828427124746191*dApardtPrev[0]*q_+1.732050807568877*BstarZ_by_Bmag[0]*hamil[1]*dfac_z))/m_; 
  alphaUp[1] = -(1.0*dApardtPrev[1]*q_)/m_; 
#if upwindType == SURFAVG 
  if (alpha0>0) { 
  incr[0] = 0.3535533905932737*alpha[0]*(1.732050807568877*fl[2]+fl[0])*dfac_v; 
  incr[1] = 0.3535533905932737*alpha[0]*(1.732050807568877*fl[3]+fl[1])*dfac_v; 
  incr[2] = -0.3535533905932737*alpha[0]*(3.0*fl[2]+1.732050807568877*fl[0])*dfac_v; 
  incr[3] = -0.3535533905932737*alpha[0]*(3.0*fl[3]+1.732050807568877*fl[1])*dfac_v; 
  incrEmMod[0] = 0.5*(1.732050807568877*fl[2]+fl[0])*dfac_v; 
  incrEmMod[1] = 0.5*(1.732050807568877*fl[3]+fl[1])*dfac_v; 
  incrEmMod[2] = -0.5*(3.0*fl[2]+1.732050807568877*fl[0])*dfac_v; 
  incrEmMod[3] = -0.5*(3.0*fl[3]+1.732050807568877*fl[1])*dfac_v; 
  } else { 
  incrEmMod[0] = -0.5*(1.732050807568877*fr[2]-1.0*fr[0])*dfac_v; 
  incrEmMod[1] = -0.5*(1.732050807568877*fr[3]-1.0*fr[1])*dfac_v; 
  incrEmMod[2] = 0.5*(3.0*fr[2]-1.732050807568877*fr[0])*dfac_v; 
  incrEmMod[3] = 0.5*(3.0*fr[3]-1.732050807568877*fr[1])*dfac_v; 
  incr[0] = -0.3535533905932737*alpha[0]*(1.732050807568877*fr[2]-1.0*fr[0])*dfac_v; 
  incr[1] = -0.3535533905932737*alpha[0]*(1.732050807568877*fr[3]-1.0*fr[1])*dfac_v; 
  incr[2] = 0.3535533905932737*alpha[0]*(3.0*fr[2]-1.732050807568877*fr[0])*dfac_v; 
  incr[3] = 0.3535533905932737*alpha[0]*(3.0*fr[3]-1.732050807568877*fr[1])*dfac_v; 
  }
#elif upwindType == QUAD 
double fupwind[2];
double fupwindQuad[2];
double alphaQuad;
  alphaQuad = 0.7071067811865475*alphaUp[0]-0.7071067811865475*alphaUp[1]; 
  fupwindQuad[0] = 0.5*(((-0.8660254037844386*(fr[3]+fl[3]))+0.8660254037844386*(fr[2]+fl[2])+0.5*fr[1]-0.5*(fl[1]+fr[0])+0.5*fl[0])*sgn(alphaQuad)+0.8660254037844386*fr[3]-0.8660254037844386*(fl[3]+fr[2])+0.8660254037844386*fl[2]-0.5*(fr[1]+fl[1])+0.5*(fr[0]+fl[0])); 
  alphaQuad = 0.7071067811865475*(alphaUp[1]+alphaUp[0]); 
  fupwindQuad[1] = 0.5*((0.8660254037844386*(fr[3]+fl[3]+fr[2]+fl[2])-0.5*(fr[1]+fr[0])+0.5*(fl[1]+fl[0]))*sgn(alphaQuad)-0.8660254037844386*(fr[3]+fr[2])+0.8660254037844386*(fl[3]+fl[2])+0.5*(fr[1]+fl[1]+fr[0]+fl[0])); 
  fupwind[0] = 0.7071067811865475*(fupwindQuad[1]+fupwindQuad[0]); 
  fupwind[1] = 0.7071067811865475*(fupwindQuad[1]-1.0*fupwindQuad[0]); 
  incrEmMod[0] = 0.7071067811865475*fupwind[0]*dfac_v; 
  incrEmMod[1] = 0.7071067811865475*fupwind[1]*dfac_v; 
  incrEmMod[2] = -1.224744871391589*fupwind[0]*dfac_v; 
  incrEmMod[3] = -1.224744871391589*fupwind[1]*dfac_v; 
  incr[0] = 0.5*alpha[0]*fupwind[0]*dfac_v; 
  incr[1] = 0.5*alpha[0]*fupwind[1]*dfac_v; 
  incr[2] = -0.8660254037844386*alpha[0]*fupwind[0]*dfac_v; 
  incr[3] = -0.8660254037844386*alpha[0]*fupwind[1]*dfac_v; 

#endif 
  outr[0] += incr[0]; 
  outr[1] += incr[1]; 
  outr[2] += incr[2]; 
  outr[3] += incr[3]; 

  outl[0] += -1.0*incr[0]; 
  outl[1] += -1.0*incr[1]; 
  outl[2] += incr[2]; 
  outl[3] += incr[3]; 
  emModR[0] += incrEmMod[0]; 
  emModR[1] += incrEmMod[1]; 
  emModR[2] += incrEmMod[2]; 
  emModR[3] += incrEmMod[3]; 
  emModL[0] += -1.0*incrEmMod[0]; 
  emModL[1] += -1.0*incrEmMod[1]; 
  emModL[2] += incrEmMod[2]; 
  emModL[3] += incrEmMod[3]; 
  return std::abs(alpha0); 
} 
double EmGyrokineticGenGeoSurf1x1vSerStep2_Vpar_P1_Bvars_0(const double q_, const double m_, const double cflL, const double cflR, const double *w, const double *dxv, const double amax_in, const double *Bmag, const double *jacobTotInv, const double *Gradpar, const double *geoX, const double *geoY, const double *geoZ, const double *Phi, const double *Apar, const double *AparL, const double *dApardt, const double *dApardtPrev, const double *fl, const double *fr, double *outl, double *outr, double *emModL, double *emModR) 
{ 
// w[NDIM]: Cell-center coordinates. dxv[NDIM]: Cell spacing. H/f: Input Hamiltonian/distribution function. out: Incremented output 
  double dfac_z = 2.0/dxv[0]; 
  double dfac_v = 2.0/dxv[1]; 
  double wz = w[0]; 
  double wv = w[1]; 
  double wv2 = wv*wv; 
  double dfac_v2 = dfac_v*dfac_v; 
  double q2 = q_*q_; 
  double incr[4]; 
  double hamil[4]; 
  hamil[0] = (0.3333333333333333*(3.0*dfac_v2*(m_*wv2+1.414213562373095*Phi[0]*q_)+m_))/dfac_v2; 
  hamil[1] = 1.414213562373095*Phi[1]*q_; 
  hamil[2] = (1.154700538379252*m_*wv)/dfac_v; 
  double BstarZ_by_Bmag[4]; 
  BstarZ_by_Bmag[0] = Gradpar[0]*jacobTotInv[0]; 

  // surface-averaged phase velocity in this direction 
  double alpha0 = -(0.125*(2.828427124746191*dApardtPrev[0]*q_+1.732050807568877*BstarZ_by_Bmag[0]*hamil[1]*dfac_z))/m_; 

  double alpha[2]; 
  alpha[0] = -(1.0*dApardt[0]*q_)/m_; 
  alpha[1] = -(1.0*dApardt[1]*q_)/m_; 
  double alphaUp[2]; 
  alphaUp[0] = -(0.3535533905932737*(2.828427124746191*dApardtPrev[0]*q_+1.732050807568877*BstarZ_by_Bmag[0]*hamil[1]*dfac_z))/m_; 
  alphaUp[1] = -(1.0*dApardtPrev[1]*q_)/m_; 
#if upwindType == SURFAVG 
  if (alpha0>0) { 
  incr[0] = 0.3535533905932737*(1.732050807568877*(alpha[1]*fl[3]+alpha[0]*fl[2])+alpha[1]*fl[1]+alpha[0]*fl[0])*dfac_v; 
  incr[1] = 0.3535533905932737*(1.732050807568877*(alpha[0]*fl[3]+alpha[1]*fl[2])+alpha[0]*fl[1]+fl[0]*alpha[1])*dfac_v; 
  incr[2] = -0.3535533905932737*(3.0*(alpha[1]*fl[3]+alpha[0]*fl[2])+1.732050807568877*(alpha[1]*fl[1]+alpha[0]*fl[0]))*dfac_v; 
  incr[3] = -0.3535533905932737*(3.0*(alpha[0]*fl[3]+alpha[1]*fl[2])+1.732050807568877*(alpha[0]*fl[1]+fl[0]*alpha[1]))*dfac_v; 
  } else { 
  incr[0] = -0.3535533905932737*(1.732050807568877*(alpha[1]*fr[3]+alpha[0]*fr[2])-1.0*(alpha[1]*fr[1]+alpha[0]*fr[0]))*dfac_v; 
  incr[1] = -0.3535533905932737*(1.732050807568877*(alpha[0]*fr[3]+alpha[1]*fr[2])-1.0*(alpha[0]*fr[1]+fr[0]*alpha[1]))*dfac_v; 
  incr[2] = 0.3535533905932737*(3.0*(alpha[1]*fr[3]+alpha[0]*fr[2])-1.732050807568877*(alpha[1]*fr[1]+alpha[0]*fr[0]))*dfac_v; 
  incr[3] = 0.3535533905932737*(3.0*(alpha[0]*fr[3]+alpha[1]*fr[2])-1.732050807568877*(alpha[0]*fr[1]+fr[0]*alpha[1]))*dfac_v; 
  }
#elif upwindType == QUAD 
double fupwind[2];
double fupwindQuad[2];
double alphaQuad;
  alphaQuad = 0.7071067811865475*alphaUp[0]-0.7071067811865475*alphaUp[1]; 
  fupwindQuad[0] = 0.5*(((-0.8660254037844386*(fr[3]+fl[3]))+0.8660254037844386*(fr[2]+fl[2])+0.5*fr[1]-0.5*(fl[1]+fr[0])+0.5*fl[0])*sgn(alphaQuad)+0.8660254037844386*fr[3]-0.8660254037844386*(fl[3]+fr[2])+0.8660254037844386*fl[2]-0.5*(fr[1]+fl[1])+0.5*(fr[0]+fl[0])); 
  alphaQuad = 0.7071067811865475*(alphaUp[1]+alphaUp[0]); 
  fupwindQuad[1] = 0.5*((0.8660254037844386*(fr[3]+fl[3]+fr[2]+fl[2])-0.5*(fr[1]+fr[0])+0.5*(fl[1]+fl[0]))*sgn(alphaQuad)-0.8660254037844386*(fr[3]+fr[2])+0.8660254037844386*(fl[3]+fl[2])+0.5*(fr[1]+fl[1]+fr[0]+fl[0])); 
  fupwind[0] = 0.7071067811865475*(fupwindQuad[1]+fupwindQuad[0]); 
  fupwind[1] = 0.7071067811865475*(fupwindQuad[1]-1.0*fupwindQuad[0]); 
  incr[0] = 0.5*(alpha[1]*fupwind[1]+alpha[0]*fupwind[0])*dfac_v; 
  incr[1] = 0.5*(alpha[0]*fupwind[1]+fupwind[0]*alpha[1])*dfac_v; 
  incr[2] = -0.8660254037844386*(alpha[1]*fupwind[1]+alpha[0]*fupwind[0])*dfac_v; 
  incr[3] = -0.8660254037844386*(alpha[0]*fupwind[1]+fupwind[0]*alpha[1])*dfac_v; 

#endif 
  outr[0] += incr[0]; 
  outr[1] += incr[1]; 
  outr[2] += incr[2]; 
  outr[3] += incr[3]; 

  outl[0] += -1.0*incr[0]; 
  outl[1] += -1.0*incr[1]; 
  outl[2] += incr[2]; 
  outl[3] += incr[3]; 
  return std::abs(alpha0); 
} 
double EmGyrokineticGenGeoSurf1x1vSer_X_P1_Bvars_1(const double q_, const double m_, const double cflL, const double cflR, const double *w, const double *dxv, const double amax_in, const double *Bmag, const double *jacobTotInv, const double *Gradpar, const double *geoX, const double *geoY, const double *geoZ, const double *Phi, const double *Apar, const double *AparL, const double *dApardt, const double *dApardtPrev, const double *fl, const double *fr, double *outl, double *outr, double *emModL, double *emModR) 
{ 
// w[NDIM]: Cell-center coordinates. dxv[NDIM]: Cell spacing. H/f: Input Hamiltonian/distribution function. out: Incremented output 
  double dfac_z = 2.0/dxv[0]; 
  double dfac_v = 2.0/dxv[1]; 
  double wz = w[0]; 
  double wv = w[1]; 
  double wv2 = wv*wv; 
  double dfac_v2 = dfac_v*dfac_v; 
  double q2 = q_*q_; 
  double incr[4]; 
  double hamil[4]; 
  hamil[0] = (0.3333333333333333*(3.0*dfac_v2*(m_*wv2+1.414213562373095*Phi[0]*q_)+m_))/dfac_v2; 
  hamil[1] = 1.414213562373095*Phi[1]*q_; 
  hamil[2] = (1.154700538379252*m_*wv)/dfac_v; 
  double BstarZ_by_Bmag[4]; 
  BstarZ_by_Bmag[0] = Gradpar[1]*jacobTotInv[1]+Gradpar[0]*jacobTotInv[0]; 
  BstarZ_by_Bmag[1] = Gradpar[0]*jacobTotInv[1]+jacobTotInv[0]*Gradpar[1]; 

  double BstarZ_by_BmagL[4]; 
  BstarZ_by_BmagL[0] = Gradpar[1]*jacobTotInv[1]+Gradpar[0]*jacobTotInv[0]; 
  BstarZ_by_BmagL[1] = Gradpar[0]*jacobTotInv[1]+jacobTotInv[0]*Gradpar[1]; 

  // surface-averaged phase velocity in this direction 
  double alpha0 = -(0.0625*(3.0*(BstarZ_by_BmagL[1]+BstarZ_by_Bmag[1])-1.732050807568877*(BstarZ_by_BmagL[0]+BstarZ_by_Bmag[0]))*hamil[2]*dfac_v)/m_; 

  double alpha[2]; 
  alpha[0] = -(0.1767766952966368*(3.0*(BstarZ_by_BmagL[1]+BstarZ_by_Bmag[1])-1.732050807568877*(BstarZ_by_BmagL[0]+BstarZ_by_Bmag[0]))*hamil[2]*dfac_v)/m_; 
#if upwindType == SURFAVG 
  if (alpha0>0) { 
  incr[0] = 0.3535533905932737*alpha[0]*(1.732050807568877*fl[1]+fl[0])*dfac_z; 
  incr[1] = -0.3535533905932737*alpha[0]*(3.0*fl[1]+1.732050807568877*fl[0])*dfac_z; 
  incr[2] = 0.3535533905932737*alpha[0]*(1.732050807568877*fl[3]+fl[2])*dfac_z; 
  incr[3] = -0.3535533905932737*alpha[0]*(3.0*fl[3]+1.732050807568877*fl[2])*dfac_z; 
  } else { 
  incr[0] = -0.3535533905932737*alpha[0]*(1.732050807568877*fr[1]-1.0*fr[0])*dfac_z; 
  incr[1] = 0.3535533905932737*alpha[0]*(3.0*fr[1]-1.732050807568877*fr[0])*dfac_z; 
  incr[2] = -0.3535533905932737*alpha[0]*(1.732050807568877*fr[3]-1.0*fr[2])*dfac_z; 
  incr[3] = 0.3535533905932737*alpha[0]*(3.0*fr[3]-1.732050807568877*fr[2])*dfac_z; 
  }
#elif upwindType == QUAD 
double fupwind[2];
double fupwindQuad[2];
double alphaQuad;
  alphaQuad = 0.7071067811865475*alpha[0]; 
  fupwindQuad[0] = 0.5*(((-0.8660254037844386*(fr[3]+fl[3]))+0.5*fr[2]-0.5*fl[2]+0.8660254037844386*(fr[1]+fl[1])-0.5*fr[0]+0.5*fl[0])*sgn(alphaQuad)+0.8660254037844386*fr[3]-0.8660254037844386*fl[3]-0.5*(fr[2]+fl[2])-0.8660254037844386*fr[1]+0.8660254037844386*fl[1]+0.5*(fr[0]+fl[0])); 
  alphaQuad = 0.7071067811865475*alpha[0]; 
  fupwindQuad[1] = 0.5*((0.8660254037844386*(fr[3]+fl[3])-0.5*fr[2]+0.5*fl[2]+0.8660254037844386*(fr[1]+fl[1])-0.5*fr[0]+0.5*fl[0])*sgn(alphaQuad)-0.8660254037844386*fr[3]+0.8660254037844386*fl[3]+0.5*(fr[2]+fl[2])-0.8660254037844386*fr[1]+0.8660254037844386*fl[1]+0.5*(fr[0]+fl[0])); 
  fupwind[0] = 0.7071067811865475*(fupwindQuad[1]+fupwindQuad[0]); 
  fupwind[1] = 0.7071067811865475*(fupwindQuad[1]-1.0*fupwindQuad[0]); 
  incr[0] = 0.5*alpha[0]*fupwind[0]*dfac_z; 
  incr[1] = -0.8660254037844386*alpha[0]*fupwind[0]*dfac_z; 
  incr[2] = 0.5*alpha[0]*fupwind[1]*dfac_z; 
  incr[3] = -0.8660254037844386*alpha[0]*fupwind[1]*dfac_z; 

#endif 
  outr[0] += incr[0]; 
  outr[1] += incr[1]; 
  outr[2] += incr[2]; 
  outr[3] += incr[3]; 

  outl[0] += -1.0*incr[0]; 
  outl[1] += incr[1]; 
  outl[2] += -1.0*incr[2]; 
  outl[3] += incr[3]; 
  return std::abs(alpha0); 
} 
double EmGyrokineticGenGeoSurf1x1vSer_Vpar_P1_Bvars_1(const double q_, const double m_, const double cflL, const double cflR, const double *w, const double *dxv, const double amax_in, const double *Bmag, const double *jacobTotInv, const double *Gradpar, const double *geoX, const double *geoY, const double *geoZ, const double *Phi, const double *Apar, const double *AparL, const double *dApardt, const double *dApardtPrev, const double *fl, const double *fr, double *outl, double *outr, double *emModL, double *emModR) 
{ 
// w[NDIM]: Cell-center coordinates. dxv[NDIM]: Cell spacing. H/f: Input Hamiltonian/distribution function. out: Incremented output 
  double dfac_z = 2.0/dxv[0]; 
  double dfac_v = 2.0/dxv[1]; 
  double wz = w[0]; 
  double wv = w[1]; 
  double wv2 = wv*wv; 
  double dfac_v2 = dfac_v*dfac_v; 
  double q2 = q_*q_; 
  double incr[4]; 
  double incrEmMod[4]; 
  double hamil[4]; 
  hamil[0] = (0.3333333333333333*(3.0*dfac_v2*(m_*wv2+1.414213562373095*Phi[0]*q_)+m_))/dfac_v2; 
  hamil[1] = 1.414213562373095*Phi[1]*q_; 
  hamil[2] = (1.154700538379252*m_*wv)/dfac_v; 
  double BstarZ_by_Bmag[4]; 
  BstarZ_by_Bmag[0] = Gradpar[1]*jacobTotInv[1]+Gradpar[0]*jacobTotInv[0]; 
  BstarZ_by_Bmag[1] = Gradpar[0]*jacobTotInv[1]+jacobTotInv[0]*Gradpar[1]; 

  // surface-averaged phase velocity in this direction 
  double alpha0 = -(0.125*(2.828427124746191*dApardtPrev[0]*q_+1.732050807568877*BstarZ_by_Bmag[0]*hamil[1]*dfac_z))/m_; 

  double alpha[2]; 
  alpha[0] = -(0.6123724356957944*BstarZ_by_Bmag[0]*hamil[1]*dfac_z)/m_; 
  alpha[1] = -(0.6123724356957944*BstarZ_by_Bmag[1]*hamil[1]*dfac_z)/m_; 
  double alphaUp[2]; 
  alphaUp[0] = -(0.3535533905932737*(2.828427124746191*dApardtPrev[0]*q_+1.732050807568877*BstarZ_by_Bmag[0]*hamil[1]*dfac_z))/m_; 
  alphaUp[1] = -(0.3535533905932737*(2.828427124746191*dApardtPrev[1]*q_+1.732050807568877*BstarZ_by_Bmag[1]*hamil[1]*dfac_z))/m_; 
#if upwindType == SURFAVG 
  if (alpha0>0) { 
  incr[0] = 0.3535533905932737*(1.732050807568877*(alpha[1]*fl[3]+alpha[0]*fl[2])+alpha[1]*fl[1]+alpha[0]*fl[0])*dfac_v; 
  incr[1] = 0.3535533905932737*(1.732050807568877*(alpha[0]*fl[3]+alpha[1]*fl[2])+alpha[0]*fl[1]+fl[0]*alpha[1])*dfac_v; 
  incr[2] = -0.3535533905932737*(3.0*(alpha[1]*fl[3]+alpha[0]*fl[2])+1.732050807568877*(alpha[1]*fl[1]+alpha[0]*fl[0]))*dfac_v; 
  incr[3] = -0.3535533905932737*(3.0*(alpha[0]*fl[3]+alpha[1]*fl[2])+1.732050807568877*(alpha[0]*fl[1]+fl[0]*alpha[1]))*dfac_v; 
  incrEmMod[0] = 0.5*(1.732050807568877*fl[2]+fl[0])*dfac_v; 
  incrEmMod[1] = 0.5*(1.732050807568877*fl[3]+fl[1])*dfac_v; 
  incrEmMod[2] = -0.5*(3.0*fl[2]+1.732050807568877*fl[0])*dfac_v; 
  incrEmMod[3] = -0.5*(3.0*fl[3]+1.732050807568877*fl[1])*dfac_v; 
  } else { 
  incrEmMod[0] = -0.5*(1.732050807568877*fr[2]-1.0*fr[0])*dfac_v; 
  incrEmMod[1] = -0.5*(1.732050807568877*fr[3]-1.0*fr[1])*dfac_v; 
  incrEmMod[2] = 0.5*(3.0*fr[2]-1.732050807568877*fr[0])*dfac_v; 
  incrEmMod[3] = 0.5*(3.0*fr[3]-1.732050807568877*fr[1])*dfac_v; 
  incr[0] = -0.3535533905932737*(1.732050807568877*(alpha[1]*fr[3]+alpha[0]*fr[2])-1.0*(alpha[1]*fr[1]+alpha[0]*fr[0]))*dfac_v; 
  incr[1] = -0.3535533905932737*(1.732050807568877*(alpha[0]*fr[3]+alpha[1]*fr[2])-1.0*(alpha[0]*fr[1]+fr[0]*alpha[1]))*dfac_v; 
  incr[2] = 0.3535533905932737*(3.0*(alpha[1]*fr[3]+alpha[0]*fr[2])-1.732050807568877*(alpha[1]*fr[1]+alpha[0]*fr[0]))*dfac_v; 
  incr[3] = 0.3535533905932737*(3.0*(alpha[0]*fr[3]+alpha[1]*fr[2])-1.732050807568877*(alpha[0]*fr[1]+fr[0]*alpha[1]))*dfac_v; 
  }
#elif upwindType == QUAD 
double fupwind[2];
double fupwindQuad[2];
double alphaQuad;
  alphaQuad = 0.7071067811865475*alphaUp[0]-0.7071067811865475*alphaUp[1]; 
  fupwindQuad[0] = 0.5*(((-0.8660254037844386*(fr[3]+fl[3]))+0.8660254037844386*(fr[2]+fl[2])+0.5*fr[1]-0.5*(fl[1]+fr[0])+0.5*fl[0])*sgn(alphaQuad)+0.8660254037844386*fr[3]-0.8660254037844386*(fl[3]+fr[2])+0.8660254037844386*fl[2]-0.5*(fr[1]+fl[1])+0.5*(fr[0]+fl[0])); 
  alphaQuad = 0.7071067811865475*(alphaUp[1]+alphaUp[0]); 
  fupwindQuad[1] = 0.5*((0.8660254037844386*(fr[3]+fl[3]+fr[2]+fl[2])-0.5*(fr[1]+fr[0])+0.5*(fl[1]+fl[0]))*sgn(alphaQuad)-0.8660254037844386*(fr[3]+fr[2])+0.8660254037844386*(fl[3]+fl[2])+0.5*(fr[1]+fl[1]+fr[0]+fl[0])); 
  fupwind[0] = 0.7071067811865475*(fupwindQuad[1]+fupwindQuad[0]); 
  fupwind[1] = 0.7071067811865475*(fupwindQuad[1]-1.0*fupwindQuad[0]); 
  incrEmMod[0] = 0.7071067811865475*fupwind[0]*dfac_v; 
  incrEmMod[1] = 0.7071067811865475*fupwind[1]*dfac_v; 
  incrEmMod[2] = -1.224744871391589*fupwind[0]*dfac_v; 
  incrEmMod[3] = -1.224744871391589*fupwind[1]*dfac_v; 
  incr[0] = 0.5*(alpha[1]*fupwind[1]+alpha[0]*fupwind[0])*dfac_v; 
  incr[1] = 0.5*(alpha[0]*fupwind[1]+fupwind[0]*alpha[1])*dfac_v; 
  incr[2] = -0.8660254037844386*(alpha[1]*fupwind[1]+alpha[0]*fupwind[0])*dfac_v; 
  incr[3] = -0.8660254037844386*(alpha[0]*fupwind[1]+fupwind[0]*alpha[1])*dfac_v; 

#endif 
  outr[0] += incr[0]; 
  outr[1] += incr[1]; 
  outr[2] += incr[2]; 
  outr[3] += incr[3]; 

  outl[0] += -1.0*incr[0]; 
  outl[1] += -1.0*incr[1]; 
  outl[2] += incr[2]; 
  outl[3] += incr[3]; 
  emModR[0] += incrEmMod[0]; 
  emModR[1] += incrEmMod[1]; 
  emModR[2] += incrEmMod[2]; 
  emModR[3] += incrEmMod[3]; 
  emModL[0] += -1.0*incrEmMod[0]; 
  emModL[1] += -1.0*incrEmMod[1]; 
  emModL[2] += incrEmMod[2]; 
  emModL[3] += incrEmMod[3]; 
  return std::abs(alpha0); 
} 
double EmGyrokineticGenGeoSurf1x1vSerStep2_Vpar_P1_Bvars_1(const double q_, const double m_, const double cflL, const double cflR, const double *w, const double *dxv, const double amax_in, const double *Bmag, const double *jacobTotInv, const double *Gradpar, const double *geoX, const double *geoY, const double *geoZ, const double *Phi, const double *Apar, const double *AparL, const double *dApardt, const double *dApardtPrev, const double *fl, const double *fr, double *outl, double *outr, double *emModL, double *emModR) 
{ 
// w[NDIM]: Cell-center coordinates. dxv[NDIM]: Cell spacing. H/f: Input Hamiltonian/distribution function. out: Incremented output 
  double dfac_z = 2.0/dxv[0]; 
  double dfac_v = 2.0/dxv[1]; 
  double wz = w[0]; 
  double wv = w[1]; 
  double wv2 = wv*wv; 
  double dfac_v2 = dfac_v*dfac_v; 
  double q2 = q_*q_; 
  double incr[4]; 
  double hamil[4]; 
  hamil[0] = (0.3333333333333333*(3.0*dfac_v2*(m_*wv2+1.414213562373095*Phi[0]*q_)+m_))/dfac_v2; 
  hamil[1] = 1.414213562373095*Phi[1]*q_; 
  hamil[2] = (1.154700538379252*m_*wv)/dfac_v; 
  double BstarZ_by_Bmag[4]; 
  BstarZ_by_Bmag[0] = Gradpar[1]*jacobTotInv[1]+Gradpar[0]*jacobTotInv[0]; 
  BstarZ_by_Bmag[1] = Gradpar[0]*jacobTotInv[1]+jacobTotInv[0]*Gradpar[1]; 

  // surface-averaged phase velocity in this direction 
  double alpha0 = -(0.125*(2.828427124746191*dApardtPrev[0]*q_+1.732050807568877*BstarZ_by_Bmag[0]*hamil[1]*dfac_z))/m_; 

  double alpha[2]; 
  alpha[0] = -(1.0*dApardt[0]*q_)/m_; 
  alpha[1] = -(1.0*dApardt[1]*q_)/m_; 
  double alphaUp[2]; 
  alphaUp[0] = -(0.3535533905932737*(2.828427124746191*dApardtPrev[0]*q_+1.732050807568877*BstarZ_by_Bmag[0]*hamil[1]*dfac_z))/m_; 
  alphaUp[1] = -(0.3535533905932737*(2.828427124746191*dApardtPrev[1]*q_+1.732050807568877*BstarZ_by_Bmag[1]*hamil[1]*dfac_z))/m_; 
#if upwindType == SURFAVG 
  if (alpha0>0) { 
  incr[0] = 0.3535533905932737*(1.732050807568877*(alpha[1]*fl[3]+alpha[0]*fl[2])+alpha[1]*fl[1]+alpha[0]*fl[0])*dfac_v; 
  incr[1] = 0.3535533905932737*(1.732050807568877*(alpha[0]*fl[3]+alpha[1]*fl[2])+alpha[0]*fl[1]+fl[0]*alpha[1])*dfac_v; 
  incr[2] = -0.3535533905932737*(3.0*(alpha[1]*fl[3]+alpha[0]*fl[2])+1.732050807568877*(alpha[1]*fl[1]+alpha[0]*fl[0]))*dfac_v; 
  incr[3] = -0.3535533905932737*(3.0*(alpha[0]*fl[3]+alpha[1]*fl[2])+1.732050807568877*(alpha[0]*fl[1]+fl[0]*alpha[1]))*dfac_v; 
  } else { 
  incr[0] = -0.3535533905932737*(1.732050807568877*(alpha[1]*fr[3]+alpha[0]*fr[2])-1.0*(alpha[1]*fr[1]+alpha[0]*fr[0]))*dfac_v; 
  incr[1] = -0.3535533905932737*(1.732050807568877*(alpha[0]*fr[3]+alpha[1]*fr[2])-1.0*(alpha[0]*fr[1]+fr[0]*alpha[1]))*dfac_v; 
  incr[2] = 0.3535533905932737*(3.0*(alpha[1]*fr[3]+alpha[0]*fr[2])-1.732050807568877*(alpha[1]*fr[1]+alpha[0]*fr[0]))*dfac_v; 
  incr[3] = 0.3535533905932737*(3.0*(alpha[0]*fr[3]+alpha[1]*fr[2])-1.732050807568877*(alpha[0]*fr[1]+fr[0]*alpha[1]))*dfac_v; 
  }
#elif upwindType == QUAD 
double fupwind[2];
double fupwindQuad[2];
double alphaQuad;
  alphaQuad = 0.7071067811865475*alphaUp[0]-0.7071067811865475*alphaUp[1]; 
  fupwindQuad[0] = 0.5*(((-0.8660254037844386*(fr[3]+fl[3]))+0.8660254037844386*(fr[2]+fl[2])+0.5*fr[1]-0.5*(fl[1]+fr[0])+0.5*fl[0])*sgn(alphaQuad)+0.8660254037844386*fr[3]-0.8660254037844386*(fl[3]+fr[2])+0.8660254037844386*fl[2]-0.5*(fr[1]+fl[1])+0.5*(fr[0]+fl[0])); 
  alphaQuad = 0.7071067811865475*(alphaUp[1]+alphaUp[0]); 
  fupwindQuad[1] = 0.5*((0.8660254037844386*(fr[3]+fl[3]+fr[2]+fl[2])-0.5*(fr[1]+fr[0])+0.5*(fl[1]+fl[0]))*sgn(alphaQuad)-0.8660254037844386*(fr[3]+fr[2])+0.8660254037844386*(fl[3]+fl[2])+0.5*(fr[1]+fl[1]+fr[0]+fl[0])); 
  fupwind[0] = 0.7071067811865475*(fupwindQuad[1]+fupwindQuad[0]); 
  fupwind[1] = 0.7071067811865475*(fupwindQuad[1]-1.0*fupwindQuad[0]); 
  incr[0] = 0.5*(alpha[1]*fupwind[1]+alpha[0]*fupwind[0])*dfac_v; 
  incr[1] = 0.5*(alpha[0]*fupwind[1]+fupwind[0]*alpha[1])*dfac_v; 
  incr[2] = -0.8660254037844386*(alpha[1]*fupwind[1]+alpha[0]*fupwind[0])*dfac_v; 
  incr[3] = -0.8660254037844386*(alpha[0]*fupwind[1]+fupwind[0]*alpha[1])*dfac_v; 

#endif 
  outr[0] += incr[0]; 
  outr[1] += incr[1]; 
  outr[2] += incr[2]; 
  outr[3] += incr[3]; 

  outl[0] += -1.0*incr[0]; 
  outl[1] += -1.0*incr[1]; 
  outl[2] += incr[2]; 
  outl[3] += incr[3]; 
  return std::abs(alpha0); 
} 
