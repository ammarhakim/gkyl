...
// w[NDIM]: Cell-center coordinates. dxv[NDIM]: Cell spacing. H/f: Input Hamiltonian/distribution function. out: Incremented output 
  ...
  if(alphaQuad > 0) {
  fhatALQuad[0] = -0.7071067811865475*(fL_AL[1]-1.0*fL_AL[0]); 
  } else {
  fhatALQuad[0] = -0.7071067811865475*(fR_AL[1]-1.0*fR_AL[0]); 
  } 
  alphaQuad = 0.7071067811865475*alpha[0]; 
  if(alphaQuad > 0) {
  fhatALQuad[1] = 0.7071067811865475*(fL_AL[1]+fL_AL[0]); 
  } else {
  fhatALQuad[1] = 0.7071067811865475*(fR_AL[1]+fR_AL[0]); 
  } 
  ...
  outlPos[0] = 0.1666666666666667*(incr[3]+1.732050807568877*(incr[2]-1.0*incr[1])-3.0*incr[0]); 
  outlPos[1] = 0.1666666666666667*((-1.0*incr[3])+1.732050807568877*(incr[2]+incr[1])-3.0*incr[0]); 
  outlPos[2] = -0.1666666666666667*(incr[3]+1.732050807568877*(incr[2]+incr[1])+3.0*incr[0]); 
  outlPos[3] = -0.1666666666666667*((-1.0*incr[3])+1.732050807568877*(incr[2]-1.0*incr[1])+3.0*incr[0]); 
  outrPos[0] = -0.1666666666666667*((-1.0*incr[3])+1.732050807568877*(incr[2]+incr[1])-3.0*incr[0]); 
  outrPos[1] = -0.1666666666666667*(incr[3]+1.732050807568877*(incr[2]-1.0*incr[1])-3.0*incr[0]); 
  outrPos[2] = 0.1666666666666667*((-1.0*incr[3])+1.732050807568877*(incr[2]-1.0*incr[1])+3.0*incr[0]); 
  outrPos[3] = 0.1666666666666667*(incr[3]+1.732050807568877*(incr[2]+incr[1])+3.0*incr[0]); 
  if(outlPos[1] < EPSILON) limFac = std::min(1.0, -fluxFracL*(-0.1666666666666667*(fl[3]+1.732050807568877*(fl[2]-1.0*fl[1])-3.0*fl[0]))/dtApprox/outlPos[1]); 
  else limFac = 1.0; 
  if(outrPos[0] < EPSILON) limFac = std::min(limFac, -fluxFracR*(-0.1666666666666667*((-1.0*fr[3])+1.732050807568877*(fr[2]+fr[1])-3.0*fr[0]))/dtApprox/outrPos[0]); 
  ...
  if(outrPos[2] < EPSILON) limFac = std::min(limFac, -fluxFracR*(0.1666666666666667*((-1.0*fr[3])+1.732050807568877*(fr[2]-1.0*fr[1])+3.0*fr[0]))/dtApprox/outrPos[2]); 
  if(limFac < 0.) limFac = 0.; 
  outlPos[3] *= limFac; 
  outlPos[2] *= limFac; 
  outrPos[3] *= limFac; 
  ...
  outr[1] += 0.8660254037844386*(outrPos[3]-1.0*outrPos[2]+outrPos[1]-1.0*outrPos[0]); 
  outr[2] += 0.8660254037844386*(outrPos[3]+outrPos[2]-1.0*(outrPos[1]+outrPos[0])); 
  outr[3] += 0.5*(3.0*outrPos[3]-3.0*(outrPos[2]+outrPos[1])+3.0*outrPos[0]); 

  outl[0] += 0.5*(outlPos[3]+outlPos[2]+outlPos[1]+outlPos[0]); 
  outl[1] += 0.8660254037844386*(outlPos[3]-1.0*outlPos[2]+outlPos[1]-1.0*outlPos[0]); 
  outl[2] += 0.8660254037844386*(outlPos[3]+outlPos[2]-1.0*(outlPos[1]+outlPos[0])); 
...
// w[NDIM]: Cell-center coordinates. dxv[NDIM]: Cell spacing. H/f: Input Hamiltonian/distribution function. out: Incremented output 
  ...
  outlPos[0] = -0.1666666666666667*((-1.0*incr[3])+1.732050807568877*(incr[2]-1.0*incr[1])+3.0*incr[0]); 
  outlPos[1] = -0.1666666666666667*(incr[3]+1.732050807568877*(incr[2]+incr[1])+3.0*incr[0]); 
  outlPos[2] = 0.1666666666666667*((-1.0*incr[3])+1.732050807568877*(incr[2]+incr[1])-3.0*incr[0]); 
  outlPos[3] = 0.1666666666666667*(incr[3]+1.732050807568877*(incr[2]-1.0*incr[1])-3.0*incr[0]); 
  outrPos[0] = -0.1666666666666667*((-1.0*incr[3])+1.732050807568877*(incr[2]+incr[1])-3.0*incr[0]); 
  outrPos[1] = -0.1666666666666667*(incr[3]+1.732050807568877*(incr[2]-1.0*incr[1])-3.0*incr[0]); 
  outrPos[2] = 0.1666666666666667*((-1.0*incr[3])+1.732050807568877*(incr[2]-1.0*incr[1])+3.0*incr[0]); 
  outrPos[3] = 0.1666666666666667*(incr[3]+1.732050807568877*(incr[2]+incr[1])+3.0*incr[0]); 
  if(outlPos[2] < EPSILON) limFac = std::min(1.0, -fluxFracL*(0.1666666666666667*((-1.0*fl[3])+1.732050807568877*(fl[2]-1.0*fl[1])+3.0*fl[0]))/dtApprox/outlPos[2]); 
  else limFac = 1.0; 
  if(outrPos[0] < EPSILON) limFac = std::min(limFac, -fluxFracR*(-0.1666666666666667*((-1.0*fr[3])+1.732050807568877*(fr[2]+fr[1])-3.0*fr[0]))/dtApprox/outrPos[0]); 
  if(limFac < 0.) limFac = 0.; 
  outlPos[2] *= limFac; 
  ...
  if(outrPos[1] < EPSILON) limFac = std::min(limFac, -fluxFracR*(-0.1666666666666667*(fr[3]+1.732050807568877*(fr[2]-1.0*fr[1])-3.0*fr[0]))/dtApprox/outrPos[1]); 
  if(limFac < 0.) limFac = 0.; 
  outlPos[3] *= limFac; 
  outlPos[1] *= limFac; 
  outrPos[3] *= limFac; 
