...
double GyrokineticGenGeoVol3x2vSerP1_Bvars(const double q_, const double m_, const double *w, const double *dxv, const double *bmag, const double *jacobTotInv, const double *cmag, const double *b_x, const double *b_y, const double *b_z, const double *phi, const double *f, double *out) 
{ 
  ...
  BstarZdBmag[0] = 0.7071067811865475*cmag[0]*jacobTotInv[0]; 

  double cflFreq = 0.0; 
  double alphaL = 0.0; 
  double alphaR = 0.0; 
  double alphax[32]; 
  alphax[0] = (1.732050807568877*jacobTotInv[0]*rdx2*(0.125*b_y[0]*hamil[3]*rdz2-0.125*b_z[0]*hamil[2]*rdy2))/q_; 
  alphax[1] = (1.732050807568877*jacobTotInv[0]*rdx2*(0.125*b_y[0]*hamil[7]*rdz2-0.125*b_z[0]*hamil[6]*rdy2))/q_; 
  alphax[2] = (0.2165063509461096*b_y[0]*jacobTotInv[0]*hamil[8]*rdx2*rdz2)/q_; 
  alphax[3] = -(0.2165063509461096*b_z[0]*jacobTotInv[0]*hamil[8]*rdx2*rdy2)/q_; 
  alphax[6] = (0.2165063509461096*b_y[0]*jacobTotInv[0]*hamil[16]*rdx2*rdz2)/q_; 
  alphax[7] = -(0.2165063509461096*b_z[0]*jacobTotInv[0]*hamil[16]*rdx2*rdy2)/q_; 
#if cflType == SURFAVG 
  ...
  double alphay[32]; 
  alphay[0] = (1.732050807568877*jacobTotInv[0]*rdy2*(0.125*b_z[0]*hamil[1]*rdx2-0.125*b_x[0]*hamil[3]*rdz2))/q_; 
  alphay[1] = -(0.2165063509461096*b_x[0]*jacobTotInv[0]*hamil[7]*rdy2*rdz2)/q_; 
  alphay[2] = (1.732050807568877*jacobTotInv[0]*rdy2*(0.125*b_z[0]*hamil[6]*rdx2-0.125*b_x[0]*hamil[8]*rdz2))/q_; 
  alphay[3] = (0.2165063509461096*b_z[0]*jacobTotInv[0]*hamil[7]*rdx2*rdy2)/q_; 
  alphay[6] = -(0.2165063509461096*b_x[0]*jacobTotInv[0]*hamil[16]*rdy2*rdz2)/q_; 
  alphay[8] = (0.2165063509461096*b_z[0]*jacobTotInv[0]*hamil[16]*rdx2*rdy2)/q_; 
#if cflType == SURFAVG 
  // Evaluate surface-averaged alpha on left.
  ...
  alphaz[1] = (0.2165063509461096*b_x[0]*jacobTotInv[0]*hamil[6]*rdy2*rdz2)/q_; 
  alphaz[2] = -(0.2165063509461096*b_y[0]*jacobTotInv[0]*hamil[6]*rdx2*rdz2)/q_; 
  alphaz[3] = (1.732050807568877*jacobTotInv[0]*(0.125*b_x[0]*hamil[8]*rdy2-0.125*b_y[0]*hamil[7]*rdx2)*rdz2)/q_; 
  alphaz[7] = (0.2165063509461096*b_x[0]*jacobTotInv[0]*hamil[16]*rdy2*rdz2)/q_; 
  alphaz[8] = -(0.2165063509461096*b_y[0]*jacobTotInv[0]*hamil[16]*rdx2*rdz2)/q_; 
#if cflType == SURFAVG 
  // Evaluate surface-averaged alpha on left.
  ...
  double alphavpar[32]; 
  alphavpar[0] = -(0.3061862178478971*BstarZdBmag[0]*hamil[3]*rdvpar2*rdz2)/m_; 
  alphavpar[1] = -(0.3061862178478971*BstarZdBmag[0]*hamil[7]*rdvpar2*rdz2)/m_; 
  alphavpar[2] = -(0.3061862178478971*BstarZdBmag[0]*hamil[8]*rdvpar2*rdz2)/m_; 
...
#elif cflType == QUAD 
  // Evaluate alpha at left surface quadrature points.
  alphaL = 0.03125*(0.1767766952966368*alphavpar[6]-0.1767766952966368*(alphavpar[2]+alphavpar[1])+0.1767766952966368*alphavpar[0]); 
  cflFreq += -0.5*(alphaL-std::abs(alphaL)); 
  alphaL = 0.03125*(0.1767766952966368*(alphavpar[1]+alphavpar[0])-0.1767766952966368*(alphavpar[6]+alphavpar[2])); 
  cflFreq += -0.5*(alphaL-std::abs(alphaL)); 
  alphaL = 0.03125*((-0.1767766952966368*alphavpar[6])+0.1767766952966368*alphavpar[2]-0.1767766952966368*alphavpar[1]+0.1767766952966368*alphavpar[0]); 
  cflFreq += -0.5*(alphaL-std::abs(alphaL)); 
  alphaL = 0.0055242717280199*(alphavpar[6]+alphavpar[2]+alphavpar[1]+alphavpar[0]); 
  cflFreq += -0.5*(alphaL-std::abs(alphaL)); 
  alphaL = 0.03125*(0.1767766952966368*alphavpar[6]-0.1767766952966368*(alphavpar[2]+alphavpar[1])+0.1767766952966368*alphavpar[0]); 
  cflFreq += -0.5*(alphaL-std::abs(alphaL)); 
  alphaL = 0.03125*(0.1767766952966368*(alphavpar[1]+alphavpar[0])-0.1767766952966368*(alphavpar[6]+alphavpar[2])); 
  ...
  cflFreq += -0.5*(alphaL-std::abs(alphaL)); 
  // Evaluate alpha at right surface quadrature points.
  alphaR = 0.03125*(0.1767766952966368*alphavpar[6]-0.1767766952966368*(alphavpar[2]+alphavpar[1])+0.1767766952966368*alphavpar[0]); 
  cflFreq += 0.5*(alphaR+std::abs(alphaR)); 
  alphaR = 0.03125*(0.1767766952966368*(alphavpar[1]+alphavpar[0])-0.1767766952966368*(alphavpar[6]+alphavpar[2])); 
  cflFreq += 0.5*(alphaR+std::abs(alphaR)); 
  alphaR = 0.03125*((-0.1767766952966368*alphavpar[6])+0.1767766952966368*alphavpar[2]-0.1767766952966368*alphavpar[1]+0.1767766952966368*alphavpar[0]); 
  cflFreq += 0.5*(alphaR+std::abs(alphaR)); 
  alphaR = 0.0055242717280199*(alphavpar[6]+alphavpar[2]+alphavpar[1]+alphavpar[0]); 
  cflFreq += 0.5*(alphaR+std::abs(alphaR)); 
  alphaR = 0.03125*(0.1767766952966368*alphavpar[6]-0.1767766952966368*(alphavpar[2]+alphavpar[1])+0.1767766952966368*alphavpar[0]); 
  cflFreq += 0.5*(alphaR+std::abs(alphaR)); 
  alphaR = 0.03125*(0.1767766952966368*(alphavpar[1]+alphavpar[0])-0.1767766952966368*(alphavpar[6]+alphavpar[2])); 
  ...
  out[4] += 0.3061862178478971*(alphavpar[6]*f[6]+alphavpar[2]*f[2]+alphavpar[1]*f[1]+alphavpar[0]*f[0]); 
  ...
  out[9] += 0.3061862178478971*(alphax[7]*f[18]+alphax[6]*f[17]+alphax[3]*f[11]+alphax[2]*f[10]+alphax[1]*f[9]+alphavpar[2]*f[6]+f[2]*alphavpar[6]+alphax[0]*f[4]+alphavpar[0]*f[1]+f[0]*alphavpar[1]); 
  out[10] += 0.3061862178478971*(alphay[8]*f[19]+alphay[6]*f[17]+alphay[3]*f[11]+alphay[2]*f[10]+alphay[1]*f[9]+alphavpar[1]*f[6]+f[1]*alphavpar[6]+alphay[0]*f[4]+alphavpar[0]*f[2]+f[0]*alphavpar[2]); 
  out[11] += 0.3061862178478971*(alphaz[8]*f[19]+alphaz[7]*f[18]+alphavpar[6]*f[16]+alphaz[3]*f[11]+alphaz[2]*f[10]+alphaz[1]*f[9]+alphavpar[2]*f[8]+alphavpar[1]*f[7]+alphaz[0]*f[4]+alphavpar[0]*f[3]); 
  ...
  out[15] += 0.3061862178478971*(alphavpar[6]*f[20]+alphavpar[2]*f[13]+alphavpar[1]*f[12]+alphavpar[0]*f[5]); 
  ...
  out[17] += 0.3061862178478971*((alphay[8]+alphax[7])*f[26]+alphax[3]*f[19]+alphay[3]*f[18]+(alphay[2]+alphax[1])*f[17]+(alphay[6]+alphax[0])*f[10]+(alphax[6]+alphay[0])*f[9]+alphavpar[0]*f[6]+f[0]*alphavpar[6]+(alphax[2]+alphay[1])*f[4]+alphavpar[1]*f[2]+f[1]*alphavpar[2]); 
  out[18] += 0.3061862178478971*((alphaz[8]+alphax[6])*f[26]+alphax[2]*f[19]+(alphaz[3]+alphax[1])*f[18]+alphaz[2]*f[17]+alphavpar[2]*f[16]+(alphaz[7]+alphax[0])*f[11]+(alphax[7]+alphaz[0])*f[9]+alphavpar[6]*f[8]+alphavpar[0]*f[7]+(alphax[3]+alphaz[1])*f[4]+alphavpar[1]*f[3]); 
  out[19] += 0.3061862178478971*((alphaz[7]+alphay[6])*f[26]+(alphaz[3]+alphay[2])*f[19]+alphay[1]*f[18]+alphaz[1]*f[17]+alphavpar[1]*f[16]+(alphaz[8]+alphay[0])*f[11]+(alphay[8]+alphaz[0])*f[10]+alphavpar[0]*f[8]+alphavpar[6]*f[7]+(alphay[3]+alphaz[2])*f[4]+alphavpar[2]*f[3]); 
  ...
  out[23] += 0.3061862178478971*(alphax[7]*f[29]+alphax[6]*f[28]+alphax[3]*f[25]+alphax[2]*f[24]+alphax[1]*f[23]+alphavpar[2]*f[20]+alphax[0]*f[15]+alphavpar[6]*f[13]+alphavpar[0]*f[12]+alphavpar[1]*f[5]); 
  out[24] += 0.3061862178478971*(alphay[8]*f[30]+alphay[6]*f[28]+alphay[3]*f[25]+alphay[2]*f[24]+alphay[1]*f[23]+alphavpar[1]*f[20]+alphay[0]*f[15]+alphavpar[0]*f[13]+alphavpar[6]*f[12]+alphavpar[2]*f[5]); 
  out[25] += 0.3061862178478971*(alphaz[8]*f[30]+alphaz[7]*f[29]+alphavpar[6]*f[27]+alphaz[3]*f[25]+alphaz[2]*f[24]+alphaz[1]*f[23]+alphavpar[2]*f[22]+alphavpar[1]*f[21]+alphaz[0]*f[15]+alphavpar[0]*f[14]); 
  out[26] += 0.3061862178478971*((alphaz[3]+alphay[2]+alphax[1])*f[26]+(alphaz[7]+alphay[6]+alphax[0])*f[19]+(alphaz[8]+alphax[6]+alphay[0])*f[18]+(alphay[8]+alphax[7]+alphaz[0])*f[17]+alphavpar[0]*f[16]+(alphax[2]+alphay[1])*f[11]+(alphax[3]+alphaz[1])*f[10]+(alphay[3]+alphaz[2])*f[9]+alphavpar[1]*f[8]+alphavpar[2]*f[7]+f[3]*alphavpar[6]); 
  ...
  out[28] += 0.3061862178478971*((alphay[8]+alphax[7])*f[31]+alphax[3]*f[30]+alphay[3]*f[29]+(alphay[2]+alphax[1])*f[28]+(alphay[6]+alphax[0])*f[24]+(alphax[6]+alphay[0])*f[23]+alphavpar[0]*f[20]+(alphax[2]+alphay[1])*f[15]+alphavpar[1]*f[13]+alphavpar[2]*f[12]+f[5]*alphavpar[6]); 
  out[29] += 0.3061862178478971*((alphaz[8]+alphax[6])*f[31]+alphax[2]*f[30]+(alphaz[3]+alphax[1])*f[29]+alphaz[2]*f[28]+alphavpar[2]*f[27]+(alphaz[7]+alphax[0])*f[25]+(alphax[7]+alphaz[0])*f[23]+alphavpar[6]*f[22]+alphavpar[0]*f[21]+(alphax[3]+alphaz[1])*f[15]+alphavpar[1]*f[14]); 
  out[30] += 0.3061862178478971*((alphaz[7]+alphay[6])*f[31]+(alphaz[3]+alphay[2])*f[30]+alphay[1]*f[29]+alphaz[1]*f[28]+alphavpar[1]*f[27]+(alphaz[8]+alphay[0])*f[25]+(alphay[8]+alphaz[0])*f[24]+alphavpar[0]*f[22]+alphavpar[6]*f[21]+(alphay[3]+alphaz[2])*f[15]+alphavpar[2]*f[14]); 
  out[31] += 0.3061862178478971*((alphaz[3]+alphay[2]+alphax[1])*f[31]+(alphaz[7]+alphay[6]+alphax[0])*f[30]+(alphaz[8]+alphax[6]+alphay[0])*f[29]+(alphay[8]+alphax[7]+alphaz[0])*f[28]+alphavpar[0]*f[27]+(alphax[2]+alphay[1])*f[25]+(alphax[3]+alphaz[1])*f[24]+(alphay[3]+alphaz[2])*f[23]+alphavpar[1]*f[22]+alphavpar[2]*f[21]+alphavpar[6]*f[14]); 
  return cflFreq; 
} 
double GyrokineticGenGeoVol3x2vSerP1_Bvarsx(const double q_, const double m_, const double *w, const double *dxv, const double *bmag, const double *jacobTotInv, const double *cmag, const double *b_x, const double *b_y, const double *b_z, const double *phi, const double *f, double *out) 
{ 
  // jacobTotInv: reciprocal of the conf-space jacobian time the guiding center coordinate Jacobian.
  // b_x,b_y,b_z: covariant components of the field aligned unit vector.
  // q_,m_: species charge and mass.
  ...
  double BstarYdBmag[32]; 
  BstarYdBmag[0] = -(1.224744871391589*jacobTotInv[0]*b_z[1]*m_*rdx2*wvpar)/q_; 
  BstarYdBmag[1] = -(1.224744871391589*b_z[1]*jacobTotInv[1]*m_*rdx2*wvpar)/q_; 
  ...
  double BstarZdBmag[32]; 
  BstarZdBmag[0] = (0.7071067811865475*(1.732050807568877*jacobTotInv[0]*b_y[1]*m_*rdx2*wvpar+(cmag[1]*jacobTotInv[1]+cmag[0]*jacobTotInv[0])*q_))/q_; 
  BstarZdBmag[1] = (0.7071067811865475*(1.732050807568877*b_y[1]*jacobTotInv[1]*m_*rdx2*wvpar+(cmag[0]*jacobTotInv[1]+jacobTotInv[0]*cmag[1])*q_))/q_; 
  ...
  double alphaR = 0.0; 
  double alphax[32]; 
  alphax[0] = (1.732050807568877*rdx2*(0.125*((b_y[0]*jacobTotInv[1]+jacobTotInv[0]*b_y[1])*hamil[7]+(b_y[1]*jacobTotInv[1]+b_y[0]*jacobTotInv[0])*hamil[3])*rdz2-0.125*((b_z[0]*jacobTotInv[1]+jacobTotInv[0]*b_z[1])*hamil[6]+(b_z[1]*jacobTotInv[1]+b_z[0]*jacobTotInv[0])*hamil[2])*rdy2))/q_; 
  alphax[1] = (1.732050807568877*rdx2*((0.125*(b_y[0]*jacobTotInv[0]*hamil[7]+(b_y[0]*jacobTotInv[1]+jacobTotInv[0]*b_y[1])*hamil[3])+0.225*b_y[1]*jacobTotInv[1]*hamil[7])*rdz2+((-0.125*(b_z[0]*jacobTotInv[0]*hamil[6]+(b_z[0]*jacobTotInv[1]+jacobTotInv[0]*b_z[1])*hamil[2]))-0.225*b_z[1]*jacobTotInv[1]*hamil[6])*rdy2))/q_; 
  alphax[2] = (0.2165063509461096*((b_y[0]*jacobTotInv[1]+jacobTotInv[0]*b_y[1])*hamil[16]+(b_y[1]*jacobTotInv[1]+b_y[0]*jacobTotInv[0])*hamil[8])*rdx2*rdz2)/q_; 
  alphax[3] = -(0.2165063509461096*((b_z[0]*jacobTotInv[1]+jacobTotInv[0]*b_z[1])*hamil[16]+(b_z[1]*jacobTotInv[1]+b_z[0]*jacobTotInv[0])*hamil[8])*rdx2*rdy2)/q_; 
  alphax[6] = (1.732050807568877*(0.125*(b_y[0]*jacobTotInv[0]*hamil[16]+(b_y[0]*jacobTotInv[1]+jacobTotInv[0]*b_y[1])*hamil[8])+0.225*b_y[1]*jacobTotInv[1]*hamil[16])*rdx2*rdz2)/q_; 
  alphax[7] = (1.732050807568877*((-0.125*(b_z[0]*jacobTotInv[0]*hamil[16]+(b_z[0]*jacobTotInv[1]+jacobTotInv[0]*b_z[1])*hamil[8]))-0.225*b_z[1]*jacobTotInv[1]*hamil[16])*rdx2*rdy2)/q_; 
#if cflType == SURFAVG 
  // Evaluate surface-averaged alpha on left.
  alphaL = -0.0625*(2.449489742783178*alphax[1]-1.414213562373095*alphax[0]); 
  cflFreq += -0.5*(alphaL-std::abs(alphaL)); 
  ...
  alphay[2] = (1.732050807568877*rdy2*(0.125*(b_z[1]*jacobTotInv[1]+b_z[0]*jacobTotInv[0])*hamil[6]*rdx2-0.125*((b_x[0]*jacobTotInv[1]+jacobTotInv[0]*b_x[1])*hamil[16]+(b_x[1]*jacobTotInv[1]+b_x[0]*jacobTotInv[0])*hamil[8])*rdz2))/q_; 
  alphay[3] = (0.2165063509461096*(b_z[1]*jacobTotInv[1]+b_z[0]*jacobTotInv[0])*hamil[7]*rdx2*rdy2)/q_; 
  ...
  alphay[6] = (1.732050807568877*rdy2*(((-0.125*(b_x[0]*jacobTotInv[0]*hamil[16]+(b_x[0]*jacobTotInv[1]+jacobTotInv[0]*b_x[1])*hamil[8]))-0.225*b_x[1]*jacobTotInv[1]*hamil[16])*rdz2+0.125*(b_z[0]*jacobTotInv[1]+jacobTotInv[0]*b_z[1])*hamil[6]*rdx2))/q_; 
  alphay[7] = (0.2165063509461096*(b_z[0]*jacobTotInv[1]+jacobTotInv[0]*b_z[1])*hamil[7]*rdx2*rdy2)/q_; 
  alphay[8] = (0.2165063509461096*(b_z[1]*jacobTotInv[1]+b_z[0]*jacobTotInv[0])*hamil[16]*rdx2*rdy2)/q_; 
  ...
  alphay[16] = (0.2165063509461096*(b_z[0]*jacobTotInv[1]+jacobTotInv[0]*b_z[1])*hamil[16]*rdx2*rdy2)/q_; 
#if cflType == SURFAVG 
  // Evaluate surface-averaged alpha on left.
  alphaL = -0.0625*(2.449489742783178*alphay[2]-1.414213562373095*alphay[0]); 
  cflFreq += -0.5*(alphaL-std::abs(alphaL)); 
  ...
  alphaz[2] = -(0.2165063509461096*(b_y[1]*jacobTotInv[1]+b_y[0]*jacobTotInv[0])*hamil[6]*rdx2*rdz2)/q_; 
  alphaz[3] = (1.732050807568877*(0.125*((b_x[0]*jacobTotInv[1]+jacobTotInv[0]*b_x[1])*hamil[16]+(b_x[1]*jacobTotInv[1]+b_x[0]*jacobTotInv[0])*hamil[8])*rdy2-0.125*(b_y[1]*jacobTotInv[1]+b_y[0]*jacobTotInv[0])*hamil[7]*rdx2)*rdz2)/q_; 
  ...
  alphaz[6] = -(0.2165063509461096*(b_y[0]*jacobTotInv[1]+jacobTotInv[0]*b_y[1])*hamil[6]*rdx2*rdz2)/q_; 
  alphaz[7] = (1.732050807568877*((0.125*(b_x[0]*jacobTotInv[0]*hamil[16]+(b_x[0]*jacobTotInv[1]+jacobTotInv[0]*b_x[1])*hamil[8])+0.225*b_x[1]*jacobTotInv[1]*hamil[16])*rdy2-0.125*(b_y[0]*jacobTotInv[1]+jacobTotInv[0]*b_y[1])*hamil[7]*rdx2)*rdz2)/q_; 
  alphaz[8] = -(0.2165063509461096*(b_y[1]*jacobTotInv[1]+b_y[0]*jacobTotInv[0])*hamil[16]*rdx2*rdz2)/q_; 
  ...
  alphaz[16] = -(0.2165063509461096*(b_y[0]*jacobTotInv[1]+jacobTotInv[0]*b_y[1])*hamil[16]*rdx2*rdz2)/q_; 
#if cflType == SURFAVG 
  // Evaluate surface-averaged alpha on left.
  alphaL = -0.0625*(2.449489742783178*alphaz[3]-1.414213562373095*alphaz[0]); 
  cflFreq += -0.5*(alphaL-std::abs(alphaL)); 
  ...
  double alphavpar[32]; 
  alphavpar[0] = -(0.3061862178478971*rdvpar2*((BstarZdBmag[1]*hamil[7]+BstarZdBmag[0]*hamil[3])*rdz2+(BstarYdBmag[1]*hamil[6]+BstarYdBmag[0]*hamil[2])*rdy2))/m_; 
  alphavpar[1] = -(0.3061862178478971*rdvpar2*((BstarZdBmag[0]*hamil[7]+BstarZdBmag[1]*hamil[3])*rdz2+(BstarYdBmag[0]*hamil[6]+BstarYdBmag[1]*hamil[2])*rdy2))/m_; 
  alphavpar[2] = -(0.3061862178478971*(BstarZdBmag[1]*hamil[16]+BstarZdBmag[0]*hamil[8])*rdvpar2*rdz2)/m_; 
  alphavpar[3] = -(0.3061862178478971*(BstarYdBmag[1]*hamil[16]+BstarYdBmag[0]*hamil[8])*rdvpar2*rdy2)/m_; 
  ...
  alphavpar[6] = -(0.3061862178478971*(BstarZdBmag[0]*hamil[16]+BstarZdBmag[1]*hamil[8])*rdvpar2*rdz2)/m_; 
  alphavpar[7] = -(0.3061862178478971*(BstarYdBmag[0]*hamil[16]+BstarYdBmag[1]*hamil[8])*rdvpar2*rdy2)/m_; 
...
#elif cflType == QUAD 
  // Evaluate alpha at left surface quadrature points.
  alphaL = 0.03125*((-0.3061862178478971*(alphavpar[18]+alphavpar[17]))+0.3061862178478971*(alphavpar[11]+alphavpar[10]+alphavpar[9])+0.1767766952966368*(alphavpar[7]+alphavpar[6])-0.3061862178478971*alphavpar[4]-0.1767766952966368*(alphavpar[3]+alphavpar[2]+alphavpar[1])+0.1767766952966368*alphavpar[0]); 
  cflFreq += -0.5*(alphaL-std::abs(alphaL)); 
  alphaL = 0.03125*(0.3061862178478971*(alphavpar[18]+alphavpar[17]+alphavpar[11]+alphavpar[10])-0.3061862178478971*alphavpar[9]-0.1767766952966368*(alphavpar[7]+alphavpar[6])-0.3061862178478971*alphavpar[4]-0.1767766952966368*(alphavpar[3]+alphavpar[2])+0.1767766952966368*(alphavpar[1]+alphavpar[0])); 
  cflFreq += -0.5*(alphaL-std::abs(alphaL)); 
  alphaL = 0.03125*((-0.3061862178478971*alphavpar[18])+0.3061862178478971*(alphavpar[17]+alphavpar[11])-0.3061862178478971*alphavpar[10]+0.3061862178478971*alphavpar[9]+0.1767766952966368*alphavpar[7]-0.1767766952966368*alphavpar[6]-0.3061862178478971*alphavpar[4]-0.1767766952966368*alphavpar[3]+0.1767766952966368*alphavpar[2]-0.1767766952966368*alphavpar[1]+0.1767766952966368*alphavpar[0]); 
  cflFreq += -0.5*(alphaL-std::abs(alphaL)); 
  alphaL = 0.03125*(0.3061862178478971*alphavpar[18]-0.3061862178478971*alphavpar[17]+0.3061862178478971*alphavpar[11]-0.3061862178478971*(alphavpar[10]+alphavpar[9])-0.1767766952966368*alphavpar[7]+0.1767766952966368*alphavpar[6]-0.3061862178478971*alphavpar[4]-0.1767766952966368*alphavpar[3]+0.1767766952966368*(alphavpar[2]+alphavpar[1]+alphavpar[0])); 
  cflFreq += -0.5*(alphaL-std::abs(alphaL)); 
  alphaL = 0.03125*(0.3061862178478971*alphavpar[18]-0.3061862178478971*(alphavpar[17]+alphavpar[11])+0.3061862178478971*(alphavpar[10]+alphavpar[9])-0.1767766952966368*alphavpar[7]+0.1767766952966368*alphavpar[6]-0.3061862178478971*alphavpar[4]+0.1767766952966368*alphavpar[3]-0.1767766952966368*(alphavpar[2]+alphavpar[1])+0.1767766952966368*alphavpar[0]); 
  cflFreq += -0.5*(alphaL-std::abs(alphaL)); 
  alphaL = 0.03125*((-0.3061862178478971*alphavpar[18])+0.3061862178478971*alphavpar[17]-0.3061862178478971*alphavpar[11]+0.3061862178478971*alphavpar[10]-0.3061862178478971*alphavpar[9]+0.1767766952966368*alphavpar[7]-0.1767766952966368*alphavpar[6]-0.3061862178478971*alphavpar[4]+0.1767766952966368*alphavpar[3]-0.1767766952966368*alphavpar[2]+0.1767766952966368*(alphavpar[1]+alphavpar[0])); 
  cflFreq += -0.5*(alphaL-std::abs(alphaL)); 
  alphaL = 0.03125*(0.3061862178478971*(alphavpar[18]+alphavpar[17])-0.3061862178478971*(alphavpar[11]+alphavpar[10])+0.3061862178478971*alphavpar[9]-0.1767766952966368*(alphavpar[7]+alphavpar[6])-0.3061862178478971*alphavpar[4]+0.1767766952966368*(alphavpar[3]+alphavpar[2])-0.1767766952966368*alphavpar[1]+0.1767766952966368*alphavpar[0]); 
  cflFreq += -0.5*(alphaL-std::abs(alphaL)); 
  alphaL = 0.03125*((-0.3061862178478971*(alphavpar[18]+alphavpar[17]+alphavpar[11]+alphavpar[10]+alphavpar[9]))+0.1767766952966368*(alphavpar[7]+alphavpar[6])-0.3061862178478971*alphavpar[4]+0.1767766952966368*(alphavpar[3]+alphavpar[2]+alphavpar[1]+alphavpar[0])); 
  cflFreq += -0.5*(alphaL-std::abs(alphaL)); 
  alphaL = 0.03125*((-0.3061862178478971*(alphavpar[18]+alphavpar[17]))+0.3061862178478971*(alphavpar[11]+alphavpar[10]+alphavpar[9])+0.1767766952966368*(alphavpar[7]+alphavpar[6])-0.3061862178478971*alphavpar[4]-0.1767766952966368*(alphavpar[3]+alphavpar[2]+alphavpar[1])+0.1767766952966368*alphavpar[0]); 
  cflFreq += -0.5*(alphaL-std::abs(alphaL)); 
  alphaL = 0.03125*(0.3061862178478971*(alphavpar[18]+alphavpar[17]+alphavpar[11]+alphavpar[10])-0.3061862178478971*alphavpar[9]-0.1767766952966368*(alphavpar[7]+alphavpar[6])-0.3061862178478971*alphavpar[4]-0.1767766952966368*(alphavpar[3]+alphavpar[2])+0.1767766952966368*(alphavpar[1]+alphavpar[0])); 
  ...
  cflFreq += -0.5*(alphaL-std::abs(alphaL)); 
  // Evaluate alpha at right surface quadrature points.
  alphaR = 0.03125*(0.3061862178478971*(alphavpar[18]+alphavpar[17])-0.3061862178478971*(alphavpar[11]+alphavpar[10]+alphavpar[9])+0.1767766952966368*(alphavpar[7]+alphavpar[6])+0.3061862178478971*alphavpar[4]-0.1767766952966368*(alphavpar[3]+alphavpar[2]+alphavpar[1])+0.1767766952966368*alphavpar[0]); 
  cflFreq += 0.5*(alphaR+std::abs(alphaR)); 
  alphaR = 0.03125*((-0.3061862178478971*(alphavpar[18]+alphavpar[17]+alphavpar[11]+alphavpar[10]))+0.3061862178478971*alphavpar[9]-0.1767766952966368*(alphavpar[7]+alphavpar[6])+0.3061862178478971*alphavpar[4]-0.1767766952966368*(alphavpar[3]+alphavpar[2])+0.1767766952966368*(alphavpar[1]+alphavpar[0])); 
  cflFreq += 0.5*(alphaR+std::abs(alphaR)); 
  alphaR = 0.03125*(0.3061862178478971*alphavpar[18]-0.3061862178478971*(alphavpar[17]+alphavpar[11])+0.3061862178478971*alphavpar[10]-0.3061862178478971*alphavpar[9]+0.1767766952966368*alphavpar[7]-0.1767766952966368*alphavpar[6]+0.3061862178478971*alphavpar[4]-0.1767766952966368*alphavpar[3]+0.1767766952966368*alphavpar[2]-0.1767766952966368*alphavpar[1]+0.1767766952966368*alphavpar[0]); 
  cflFreq += 0.5*(alphaR+std::abs(alphaR)); 
  alphaR = 0.03125*((-0.3061862178478971*alphavpar[18])+0.3061862178478971*alphavpar[17]-0.3061862178478971*alphavpar[11]+0.3061862178478971*(alphavpar[10]+alphavpar[9])-0.1767766952966368*alphavpar[7]+0.1767766952966368*alphavpar[6]+0.3061862178478971*alphavpar[4]-0.1767766952966368*alphavpar[3]+0.1767766952966368*(alphavpar[2]+alphavpar[1]+alphavpar[0])); 
  cflFreq += 0.5*(alphaR+std::abs(alphaR)); 
  alphaR = 0.03125*((-0.3061862178478971*alphavpar[18])+0.3061862178478971*(alphavpar[17]+alphavpar[11])-0.3061862178478971*(alphavpar[10]+alphavpar[9])-0.1767766952966368*alphavpar[7]+0.1767766952966368*alphavpar[6]+0.3061862178478971*alphavpar[4]+0.1767766952966368*alphavpar[3]-0.1767766952966368*(alphavpar[2]+alphavpar[1])+0.1767766952966368*alphavpar[0]); 
  cflFreq += 0.5*(alphaR+std::abs(alphaR)); 
  alphaR = 0.03125*(0.3061862178478971*alphavpar[18]-0.3061862178478971*alphavpar[17]+0.3061862178478971*alphavpar[11]-0.3061862178478971*alphavpar[10]+0.3061862178478971*alphavpar[9]+0.1767766952966368*alphavpar[7]-0.1767766952966368*alphavpar[6]+0.3061862178478971*alphavpar[4]+0.1767766952966368*alphavpar[3]-0.1767766952966368*alphavpar[2]+0.1767766952966368*(alphavpar[1]+alphavpar[0])); 
  cflFreq += 0.5*(alphaR+std::abs(alphaR)); 
  alphaR = 0.03125*((-0.3061862178478971*(alphavpar[18]+alphavpar[17]))+0.3061862178478971*(alphavpar[11]+alphavpar[10])-0.3061862178478971*alphavpar[9]-0.1767766952966368*(alphavpar[7]+alphavpar[6])+0.3061862178478971*alphavpar[4]+0.1767766952966368*(alphavpar[3]+alphavpar[2])-0.1767766952966368*alphavpar[1]+0.1767766952966368*alphavpar[0]); 
  cflFreq += 0.5*(alphaR+std::abs(alphaR)); 
  alphaR = 0.03125*(0.3061862178478971*(alphavpar[18]+alphavpar[17]+alphavpar[11]+alphavpar[10]+alphavpar[9])+0.1767766952966368*(alphavpar[7]+alphavpar[6])+0.3061862178478971*alphavpar[4]+0.1767766952966368*(alphavpar[3]+alphavpar[2]+alphavpar[1]+alphavpar[0])); 
  cflFreq += 0.5*(alphaR+std::abs(alphaR)); 
  alphaR = 0.03125*(0.3061862178478971*(alphavpar[18]+alphavpar[17])-0.3061862178478971*(alphavpar[11]+alphavpar[10]+alphavpar[9])+0.1767766952966368*(alphavpar[7]+alphavpar[6])+0.3061862178478971*alphavpar[4]-0.1767766952966368*(alphavpar[3]+alphavpar[2]+alphavpar[1])+0.1767766952966368*alphavpar[0]); 
  cflFreq += 0.5*(alphaR+std::abs(alphaR)); 
  alphaR = 0.03125*((-0.3061862178478971*(alphavpar[18]+alphavpar[17]+alphavpar[11]+alphavpar[10]))+0.3061862178478971*alphavpar[9]-0.1767766952966368*(alphavpar[7]+alphavpar[6])+0.3061862178478971*alphavpar[4]-0.1767766952966368*(alphavpar[3]+alphavpar[2])+0.1767766952966368*(alphavpar[1]+alphavpar[0])); 
  ...
  out[4] += 0.3061862178478971*(alphavpar[18]*f[18]+alphavpar[17]*f[17]+alphavpar[11]*f[11]+alphavpar[10]*f[10]+alphavpar[9]*f[9]+alphavpar[7]*f[7]+alphavpar[6]*f[6]+alphavpar[4]*f[4]+alphavpar[3]*f[3]+alphavpar[2]*f[2]+alphavpar[1]*f[1]+alphavpar[0]*f[0]); 
  ...
  out[9] += 0.3061862178478971*((alphavpar[11]+alphax[7])*f[18]+f[11]*alphavpar[18]+(alphavpar[10]+alphax[6])*f[17]+f[10]*alphavpar[17]+alphax[3]*f[11]+alphax[2]*f[10]+(alphavpar[4]+alphax[1])*f[9]+f[4]*alphavpar[9]+alphavpar[3]*f[7]+f[3]*alphavpar[7]+alphavpar[2]*f[6]+f[2]*alphavpar[6]+alphax[0]*f[4]+alphavpar[0]*f[1]+f[0]*alphavpar[1]); 
  out[10] += 0.3061862178478971*((alphavpar[18]+alphay[16])*f[26]+alphay[12]*f[23]+(alphavpar[11]+alphay[8])*f[19]+alphay[7]*f[18]+(alphavpar[9]+alphay[6])*f[17]+f[9]*alphavpar[17]+alphavpar[7]*f[16]+alphay[5]*f[15]+alphay[3]*f[11]+(alphavpar[4]+alphay[2])*f[10]+f[4]*alphavpar[10]+alphay[1]*f[9]+f[1]*alphay[9]+alphavpar[3]*f[8]+alphavpar[1]*f[6]+f[1]*alphavpar[6]+alphay[0]*f[4]+f[0]*alphay[4]+alphavpar[0]*f[2]+f[0]*alphavpar[2]); 
  out[11] += 0.3061862178478971*((alphavpar[17]+alphaz[16])*f[26]+alphaz[12]*f[23]+(alphavpar[10]+alphaz[8])*f[19]+(alphavpar[9]+alphaz[7])*f[18]+f[9]*alphavpar[18]+alphaz[6]*f[17]+alphavpar[6]*f[16]+alphaz[5]*f[15]+(alphavpar[4]+alphaz[3])*f[11]+f[4]*alphavpar[11]+alphaz[2]*f[10]+alphaz[1]*f[9]+f[1]*alphaz[9]+alphavpar[2]*f[8]+alphavpar[1]*f[7]+f[1]*alphavpar[7]+alphaz[0]*f[4]+f[0]*alphaz[4]+alphavpar[0]*f[3]+f[0]*alphavpar[3]); 
  out[12] += 0.3061862178478971*(alphax[7]*f[21]+alphax[6]*f[20]+alphax[3]*f[14]+alphax[2]*f[13]+alphax[1]*f[12]+alphax[0]*f[5]); 
  ...
  out[15] += 0.3061862178478971*(alphavpar[18]*f[29]+alphavpar[17]*f[28]+alphavpar[11]*f[25]+alphavpar[10]*f[24]+alphavpar[9]*f[23]+alphavpar[7]*f[21]+alphavpar[6]*f[20]+alphavpar[4]*f[15]+alphavpar[3]*f[14]+alphavpar[2]*f[13]+alphavpar[1]*f[12]+alphavpar[0]*f[5]); 
  ...
  out[17] += 0.3061862178478971*((alphavpar[11]+alphay[8]+alphax[7])*f[26]+alphay[5]*f[23]+(alphavpar[18]+alphay[16]+alphax[3])*f[19]+alphay[3]*f[18]+(alphavpar[4]+alphay[2]+alphax[1])*f[17]+f[4]*alphavpar[17]+alphavpar[3]*f[16]+alphay[12]*f[15]+alphay[7]*f[11]+(alphavpar[9]+alphay[6]+alphax[0])*f[10]+f[9]*(alphavpar[10]+alphax[6]+alphay[0])+f[0]*alphay[9]+alphavpar[7]*f[8]+alphavpar[0]*f[6]+f[0]*alphavpar[6]+(alphax[2]+alphay[1])*f[4]+f[1]*alphay[4]+alphavpar[1]*f[2]+f[1]*alphavpar[2]); 
  out[18] += 0.3061862178478971*((alphavpar[10]+alphaz[8]+alphax[6])*f[26]+alphaz[5]*f[23]+(alphavpar[17]+alphaz[16]+alphax[2])*f[19]+(alphavpar[4]+alphaz[3]+alphax[1])*f[18]+f[4]*alphavpar[18]+alphaz[2]*f[17]+alphavpar[2]*f[16]+alphaz[12]*f[15]+(alphavpar[9]+alphaz[7]+alphax[0])*f[11]+f[9]*alphavpar[11]+alphaz[6]*f[10]+(alphax[7]+alphaz[0])*f[9]+f[0]*alphaz[9]+alphavpar[6]*f[8]+alphavpar[0]*f[7]+f[0]*alphavpar[7]+(alphax[3]+alphaz[1])*f[4]+f[1]*alphaz[4]+alphavpar[1]*f[3]+f[1]*alphavpar[3]); 
  out[19] += 0.3061862178478971*(alphay[12]*f[29]+alphaz[12]*f[28]+(alphavpar[9]+alphaz[7]+alphay[6])*f[26]+alphay[5]*f[25]+alphaz[5]*f[24]+(alphavpar[4]+alphaz[3]+alphay[2])*f[19]+(alphavpar[17]+alphaz[16]+alphay[1])*f[18]+f[17]*(alphavpar[18]+alphay[16]+alphaz[1])+alphavpar[1]*f[16]+(alphavpar[10]+alphaz[8]+alphay[0])*f[11]+f[10]*(alphavpar[11]+alphay[8]+alphaz[0])+(alphay[7]+alphaz[6])*f[9]+f[6]*alphaz[9]+f[7]*alphay[9]+alphavpar[0]*f[8]+alphavpar[6]*f[7]+f[6]*alphavpar[7]+(alphay[3]+alphaz[2])*f[4]+f[2]*alphaz[4]+f[3]*(alphay[4]+alphavpar[2])+f[2]*alphavpar[3]); 
  ...
  out[23] += 0.3061862178478971*((alphavpar[11]+alphax[7])*f[29]+(alphavpar[10]+alphax[6])*f[28]+(alphavpar[18]+alphax[3])*f[25]+(alphavpar[17]+alphax[2])*f[24]+(alphavpar[4]+alphax[1])*f[23]+alphavpar[3]*f[21]+alphavpar[2]*f[20]+(alphavpar[9]+alphax[0])*f[15]+alphavpar[7]*f[14]+alphavpar[6]*f[13]+alphavpar[0]*f[12]+alphavpar[1]*f[5]); 
  out[24] += 0.3061862178478971*((alphavpar[18]+alphay[16])*f[31]+(alphavpar[11]+alphay[8])*f[30]+alphay[7]*f[29]+(alphavpar[9]+alphay[6])*f[28]+alphavpar[7]*f[27]+alphay[3]*f[25]+(alphavpar[4]+alphay[2])*f[24]+(alphavpar[17]+alphay[1])*f[23]+alphavpar[3]*f[22]+alphavpar[1]*f[20]+(alphavpar[10]+alphay[0])*f[15]+alphavpar[0]*f[13]+(alphay[9]+alphavpar[6])*f[12]+f[9]*alphay[12]+(alphay[4]+alphavpar[2])*f[5]+f[4]*alphay[5]); 
  out[25] += 0.3061862178478971*((alphavpar[17]+alphaz[16])*f[31]+(alphavpar[10]+alphaz[8])*f[30]+(alphavpar[9]+alphaz[7])*f[29]+alphaz[6]*f[28]+alphavpar[6]*f[27]+(alphavpar[4]+alphaz[3])*f[25]+alphaz[2]*f[24]+(alphavpar[18]+alphaz[1])*f[23]+alphavpar[2]*f[22]+alphavpar[1]*f[21]+(alphavpar[11]+alphaz[0])*f[15]+alphavpar[0]*f[14]+(alphaz[9]+alphavpar[7])*f[12]+f[9]*alphaz[12]+(alphaz[4]+alphavpar[3])*f[5]+f[4]*alphaz[5]); 
  out[26] += 0.3061862178478971*(alphay[5]*f[29]+alphaz[5]*f[28]+(alphavpar[4]+alphaz[3]+alphay[2]+alphax[1])*f[26]+alphay[12]*f[25]+alphaz[12]*f[24]+(alphavpar[9]+alphaz[7]+alphay[6]+alphax[0])*f[19]+(alphavpar[10]+alphaz[8]+alphax[6]+alphay[0])*f[18]+f[10]*alphavpar[18]+(alphavpar[11]+alphay[8]+alphax[7]+alphaz[0])*f[17]+f[11]*alphavpar[17]+alphavpar[0]*f[16]+f[11]*alphaz[16]+f[10]*alphay[16]+(alphax[2]+alphay[1])*f[11]+(alphax[3]+alphaz[1])*f[10]+(alphay[3]+alphaz[2])*f[9]+f[2]*alphaz[9]+f[3]*alphay[9]+alphavpar[1]*f[8]+(alphay[4]+alphavpar[2])*f[7]+f[4]*alphay[7]+f[2]*alphavpar[7]+(alphaz[4]+alphavpar[3])*f[6]+f[4]*alphaz[6]+f[3]*alphavpar[6]); 
...
{ 
  ...
  double BstarXdBmag[32]; 
  BstarXdBmag[0] = -(1.224744871391589*jacobTotInv[0]*b_y[3]*m_*rdz2*wvpar)/q_; 
  BstarXdBmag[3] = -(1.224744871391589*b_y[3]*jacobTotInv[3]*m_*rdz2*wvpar)/q_; 
  ...
  double BstarYdBmag[32]; 
  BstarYdBmag[0] = (1.224744871391589*jacobTotInv[0]*b_x[3]*m_*rdz2*wvpar)/q_; 
  BstarYdBmag[3] = (1.224744871391589*b_x[3]*jacobTotInv[3]*m_*rdz2*wvpar)/q_; 
  ...
  double BstarZdBmag[32]; 
  BstarZdBmag[0] = 0.7071067811865475*(cmag[3]*jacobTotInv[3]+cmag[0]*jacobTotInv[0]); 
  BstarZdBmag[3] = 0.7071067811865475*(cmag[0]*jacobTotInv[3]+jacobTotInv[0]*cmag[3]); 
  ...
  alphax[1] = (1.732050807568877*rdx2*(0.125*(b_y[3]*jacobTotInv[3]+b_y[0]*jacobTotInv[0])*hamil[7]*rdz2-0.125*((b_z[0]*jacobTotInv[3]+jacobTotInv[0]*b_z[3])*hamil[16]+(b_z[3]*jacobTotInv[3]+b_z[0]*jacobTotInv[0])*hamil[6])*rdy2))/q_; 
  alphax[2] = (0.2165063509461096*(b_y[3]*jacobTotInv[3]+b_y[0]*jacobTotInv[0])*hamil[8]*rdx2*rdz2)/q_; 
  ...
  alphax[6] = (0.2165063509461096*(b_y[3]*jacobTotInv[3]+b_y[0]*jacobTotInv[0])*hamil[16]*rdx2*rdz2)/q_; 
  alphax[7] = (1.732050807568877*rdx2*(0.125*(b_y[0]*jacobTotInv[3]+jacobTotInv[0]*b_y[3])*hamil[7]*rdz2+((-0.125*(b_z[0]*jacobTotInv[0]*hamil[16]+(b_z[0]*jacobTotInv[3]+jacobTotInv[0]*b_z[3])*hamil[6]))-0.225*b_z[3]*jacobTotInv[3]*hamil[16])*rdy2))/q_; 
  alphax[8] = (0.2165063509461096*(b_y[0]*jacobTotInv[3]+jacobTotInv[0]*b_y[3])*hamil[8]*rdx2*rdz2)/q_; 
  ...
  alphax[16] = (0.2165063509461096*(b_y[0]*jacobTotInv[3]+jacobTotInv[0]*b_y[3])*hamil[16]*rdx2*rdz2)/q_; 
#if cflType == SURFAVG 
  // Evaluate surface-averaged alpha on left.
  alphaL = -0.0625*(2.449489742783178*alphax[1]-1.414213562373095*alphax[0]); 
  cflFreq += -0.5*(alphaL-std::abs(alphaL)); 
  ...
  alphay[1] = -(0.2165063509461096*(b_x[3]*jacobTotInv[3]+b_x[0]*jacobTotInv[0])*hamil[7]*rdy2*rdz2)/q_; 
  alphay[2] = (1.732050807568877*rdy2*(0.125*((b_z[0]*jacobTotInv[3]+jacobTotInv[0]*b_z[3])*hamil[16]+(b_z[3]*jacobTotInv[3]+b_z[0]*jacobTotInv[0])*hamil[6])*rdx2-0.125*(b_x[3]*jacobTotInv[3]+b_x[0]*jacobTotInv[0])*hamil[8]*rdz2))/q_; 
  ...
  alphay[6] = -(0.2165063509461096*(b_x[3]*jacobTotInv[3]+b_x[0]*jacobTotInv[0])*hamil[16]*rdy2*rdz2)/q_; 
  alphay[7] = -(0.2165063509461096*(b_x[0]*jacobTotInv[3]+jacobTotInv[0]*b_x[3])*hamil[7]*rdy2*rdz2)/q_; 
  alphay[8] = (1.732050807568877*rdy2*((0.125*(b_z[0]*jacobTotInv[0]*hamil[16]+(b_z[0]*jacobTotInv[3]+jacobTotInv[0]*b_z[3])*hamil[6])+0.225*b_z[3]*jacobTotInv[3]*hamil[16])*rdx2-0.125*(b_x[0]*jacobTotInv[3]+jacobTotInv[0]*b_x[3])*hamil[8]*rdz2))/q_; 
  ...
  alphay[16] = -(0.2165063509461096*(b_x[0]*jacobTotInv[3]+jacobTotInv[0]*b_x[3])*hamil[16]*rdy2*rdz2)/q_; 
#if cflType == SURFAVG 
  // Evaluate surface-averaged alpha on left.
  alphaL = -0.0625*(2.449489742783178*alphay[2]-1.414213562373095*alphay[0]); 
  cflFreq += -0.5*(alphaL-std::abs(alphaL)); 
  ...
  alphaz[1] = (0.2165063509461096*((b_x[0]*jacobTotInv[3]+jacobTotInv[0]*b_x[3])*hamil[16]+(b_x[3]*jacobTotInv[3]+b_x[0]*jacobTotInv[0])*hamil[6])*rdy2*rdz2)/q_; 
  alphaz[2] = -(0.2165063509461096*((b_y[0]*jacobTotInv[3]+jacobTotInv[0]*b_y[3])*hamil[16]+(b_y[3]*jacobTotInv[3]+b_y[0]*jacobTotInv[0])*hamil[6])*rdx2*rdz2)/q_; 
  ...
  alphaz[7] = (1.732050807568877*(0.125*(b_x[0]*jacobTotInv[0]*hamil[16]+(b_x[0]*jacobTotInv[3]+jacobTotInv[0]*b_x[3])*hamil[6])+0.225*b_x[3]*jacobTotInv[3]*hamil[16])*rdy2*rdz2)/q_; 
  alphaz[8] = (1.732050807568877*((-0.125*(b_y[0]*jacobTotInv[0]*hamil[16]+(b_y[0]*jacobTotInv[3]+jacobTotInv[0]*b_y[3])*hamil[6]))-0.225*b_y[3]*jacobTotInv[3]*hamil[16])*rdx2*rdz2)/q_; 
#if cflType == SURFAVG 
  // Evaluate surface-averaged alpha on left.
  alphaL = -0.0625*(2.449489742783178*alphaz[3]-1.414213562373095*alphaz[0]); 
  cflFreq += -0.5*(alphaL-std::abs(alphaL)); 
  ...
  double alphavpar[32]; 
  alphavpar[0] = -(0.3061862178478971*rdvpar2*(BstarZdBmag[0]*hamil[3]*rdz2+(BstarYdBmag[3]*hamil[8]+BstarYdBmag[0]*hamil[2])*rdy2+(BstarXdBmag[3]*hamil[7]+BstarXdBmag[0]*hamil[1])*rdx2))/m_; 
  alphavpar[1] = -(0.3061862178478971*rdvpar2*(BstarZdBmag[0]*hamil[7]*rdz2+(BstarYdBmag[3]*hamil[16]+BstarYdBmag[0]*hamil[6])*rdy2))/m_; 
  alphavpar[2] = -(0.3061862178478971*rdvpar2*(BstarZdBmag[0]*hamil[8]*rdz2+(BstarXdBmag[3]*hamil[16]+BstarXdBmag[0]*hamil[6])*rdx2))/m_; 
  alphavpar[3] = -(0.3061862178478971*rdvpar2*(BstarZdBmag[3]*hamil[3]*rdz2+(BstarYdBmag[0]*hamil[8]+hamil[2]*BstarYdBmag[3])*rdy2+(BstarXdBmag[0]*hamil[7]+hamil[1]*BstarXdBmag[3])*rdx2))/m_; 
  ...
  alphavpar[6] = -(0.3061862178478971*BstarZdBmag[0]*hamil[16]*rdvpar2*rdz2)/m_; 
  alphavpar[7] = -(0.3061862178478971*rdvpar2*(BstarZdBmag[3]*hamil[7]*rdz2+(BstarYdBmag[0]*hamil[16]+BstarYdBmag[3]*hamil[6])*rdy2))/m_; 
  alphavpar[8] = -(0.3061862178478971*rdvpar2*(BstarZdBmag[3]*hamil[8]*rdz2+(BstarXdBmag[0]*hamil[16]+BstarXdBmag[3]*hamil[6])*rdx2))/m_; 
  ...
  alphavpar[16] = -(0.3061862178478971*BstarZdBmag[3]*hamil[16]*rdvpar2*rdz2)/m_; 
...
#elif cflType == QUAD 
  // Evaluate alpha at left surface quadrature points.
  alphaL = 0.03125*((-0.3061862178478971*(alphavpar[19]+alphavpar[18]))-0.1767766952966368*alphavpar[16]+0.1767766952966368*alphavpar[14]+0.3061862178478971*(alphavpar[11]+alphavpar[10]+alphavpar[9])+0.1767766952966368*(alphavpar[8]+alphavpar[7]+alphavpar[6])-0.1767766952966368*alphavpar[5]-0.3061862178478971*alphavpar[4]-0.1767766952966368*(alphavpar[3]+alphavpar[2]+alphavpar[1])+0.1767766952966368*alphavpar[0]); 
  cflFreq += -0.5*(alphaL-std::abs(alphaL)); 
  alphaL = 0.03125*((-0.3061862178478971*alphavpar[19])+0.3061862178478971*alphavpar[18]+0.1767766952966368*(alphavpar[16]+alphavpar[14])+0.3061862178478971*(alphavpar[11]+alphavpar[10])-0.3061862178478971*alphavpar[9]+0.1767766952966368*alphavpar[8]-0.1767766952966368*(alphavpar[7]+alphavpar[6]+alphavpar[5])-0.3061862178478971*alphavpar[4]-0.1767766952966368*(alphavpar[3]+alphavpar[2])+0.1767766952966368*(alphavpar[1]+alphavpar[0])); 
  cflFreq += -0.5*(alphaL-std::abs(alphaL)); 
  alphaL = 0.03125*(0.3061862178478971*alphavpar[19]-0.3061862178478971*alphavpar[18]+0.1767766952966368*(alphavpar[16]+alphavpar[14])+0.3061862178478971*alphavpar[11]-0.3061862178478971*alphavpar[10]+0.3061862178478971*alphavpar[9]-0.1767766952966368*alphavpar[8]+0.1767766952966368*alphavpar[7]-0.1767766952966368*(alphavpar[6]+alphavpar[5])-0.3061862178478971*alphavpar[4]-0.1767766952966368*alphavpar[3]+0.1767766952966368*alphavpar[2]-0.1767766952966368*alphavpar[1]+0.1767766952966368*alphavpar[0]); 
  cflFreq += -0.5*(alphaL-std::abs(alphaL)); 
  alphaL = 0.03125*(0.3061862178478971*(alphavpar[19]+alphavpar[18])-0.1767766952966368*alphavpar[16]+0.1767766952966368*alphavpar[14]+0.3061862178478971*alphavpar[11]-0.3061862178478971*(alphavpar[10]+alphavpar[9])-0.1767766952966368*(alphavpar[8]+alphavpar[7])+0.1767766952966368*alphavpar[6]-0.1767766952966368*alphavpar[5]-0.3061862178478971*alphavpar[4]-0.1767766952966368*alphavpar[3]+0.1767766952966368*(alphavpar[2]+alphavpar[1]+alphavpar[0])); 
  cflFreq += -0.5*(alphaL-std::abs(alphaL)); 
  ...
  cflFreq += -0.5*(alphaL-std::abs(alphaL)); 
  alphaL = 0.03125*((-0.3061862178478971*(alphavpar[19]+alphavpar[18]))+0.1767766952966368*alphavpar[16]-0.1767766952966368*alphavpar[14]-0.3061862178478971*(alphavpar[11]+alphavpar[10]+alphavpar[9])+0.1767766952966368*(alphavpar[8]+alphavpar[7]+alphavpar[6])-0.1767766952966368*alphavpar[5]-0.3061862178478971*alphavpar[4]+0.1767766952966368*(alphavpar[3]+alphavpar[2]+alphavpar[1]+alphavpar[0])); 
  cflFreq += -0.5*(alphaL-std::abs(alphaL)); 
  alphaL = 0.03125*((-0.3061862178478971*(alphavpar[19]+alphavpar[18]))-0.1767766952966368*(alphavpar[16]+alphavpar[14])+0.3061862178478971*(alphavpar[11]+alphavpar[10]+alphavpar[9])+0.1767766952966368*(alphavpar[8]+alphavpar[7]+alphavpar[6]+alphavpar[5])-0.3061862178478971*alphavpar[4]-0.1767766952966368*(alphavpar[3]+alphavpar[2]+alphavpar[1])+0.1767766952966368*alphavpar[0]); 
  cflFreq += -0.5*(alphaL-std::abs(alphaL)); 
  alphaL = 0.03125*((-0.3061862178478971*alphavpar[19])+0.3061862178478971*alphavpar[18]+0.1767766952966368*alphavpar[16]-0.1767766952966368*alphavpar[14]+0.3061862178478971*(alphavpar[11]+alphavpar[10])-0.3061862178478971*alphavpar[9]+0.1767766952966368*alphavpar[8]-0.1767766952966368*(alphavpar[7]+alphavpar[6])+0.1767766952966368*alphavpar[5]-0.3061862178478971*alphavpar[4]-0.1767766952966368*(alphavpar[3]+alphavpar[2])+0.1767766952966368*(alphavpar[1]+alphavpar[0])); 
  cflFreq += -0.5*(alphaL-std::abs(alphaL)); 
  alphaL = 0.03125*(0.3061862178478971*alphavpar[19]-0.3061862178478971*alphavpar[18]+0.1767766952966368*alphavpar[16]-0.1767766952966368*alphavpar[14]+0.3061862178478971*alphavpar[11]-0.3061862178478971*alphavpar[10]+0.3061862178478971*alphavpar[9]-0.1767766952966368*alphavpar[8]+0.1767766952966368*alphavpar[7]-0.1767766952966368*alphavpar[6]+0.1767766952966368*alphavpar[5]-0.3061862178478971*alphavpar[4]-0.1767766952966368*alphavpar[3]+0.1767766952966368*alphavpar[2]-0.1767766952966368*alphavpar[1]+0.1767766952966368*alphavpar[0]); 
  cflFreq += -0.5*(alphaL-std::abs(alphaL)); 
  alphaL = 0.03125*(0.3061862178478971*(alphavpar[19]+alphavpar[18])-0.1767766952966368*(alphavpar[16]+alphavpar[14])+0.3061862178478971*alphavpar[11]-0.3061862178478971*(alphavpar[10]+alphavpar[9])-0.1767766952966368*(alphavpar[8]+alphavpar[7])+0.1767766952966368*(alphavpar[6]+alphavpar[5])-0.3061862178478971*alphavpar[4]-0.1767766952966368*alphavpar[3]+0.1767766952966368*(alphavpar[2]+alphavpar[1]+alphavpar[0])); 
  cflFreq += -0.5*(alphaL-std::abs(alphaL)); 
  ...
  cflFreq += -0.5*(alphaL-std::abs(alphaL)); 
  alphaL = 0.03125*((-0.3061862178478971*alphavpar[19])+0.3061862178478971*alphavpar[18]-0.1767766952966368*alphavpar[16]+0.1767766952966368*alphavpar[14]-0.3061862178478971*(alphavpar[11]+alphavpar[10])+0.3061862178478971*alphavpar[9]+0.1767766952966368*alphavpar[8]-0.1767766952966368*(alphavpar[7]+alphavpar[6])+0.1767766952966368*alphavpar[5]-0.3061862178478971*alphavpar[4]+0.1767766952966368*(alphavpar[3]+alphavpar[2])-0.1767766952966368*alphavpar[1]+0.1767766952966368*alphavpar[0]); 
  cflFreq += -0.5*(alphaL-std::abs(alphaL)); 
  alphaL = 0.03125*((-0.3061862178478971*(alphavpar[19]+alphavpar[18]))+0.1767766952966368*(alphavpar[16]+alphavpar[14])-0.3061862178478971*(alphavpar[11]+alphavpar[10]+alphavpar[9])+0.1767766952966368*(alphavpar[8]+alphavpar[7]+alphavpar[6]+alphavpar[5])-0.3061862178478971*alphavpar[4]+0.1767766952966368*(alphavpar[3]+alphavpar[2]+alphavpar[1]+alphavpar[0])); 
  cflFreq += -0.5*(alphaL-std::abs(alphaL)); 
  // Evaluate alpha at right surface quadrature points.
  alphaR = 0.03125*(0.3061862178478971*(alphavpar[19]+alphavpar[18])-0.1767766952966368*alphavpar[16]+0.1767766952966368*alphavpar[14]-0.3061862178478971*(alphavpar[11]+alphavpar[10]+alphavpar[9])+0.1767766952966368*(alphavpar[8]+alphavpar[7]+alphavpar[6])-0.1767766952966368*alphavpar[5]+0.3061862178478971*alphavpar[4]-0.1767766952966368*(alphavpar[3]+alphavpar[2]+alphavpar[1])+0.1767766952966368*alphavpar[0]); 
  cflFreq += 0.5*(alphaR+std::abs(alphaR)); 
  alphaR = 0.03125*(0.3061862178478971*alphavpar[19]-0.3061862178478971*alphavpar[18]+0.1767766952966368*(alphavpar[16]+alphavpar[14])-0.3061862178478971*(alphavpar[11]+alphavpar[10])+0.3061862178478971*alphavpar[9]+0.1767766952966368*alphavpar[8]-0.1767766952966368*(alphavpar[7]+alphavpar[6]+alphavpar[5])+0.3061862178478971*alphavpar[4]-0.1767766952966368*(alphavpar[3]+alphavpar[2])+0.1767766952966368*(alphavpar[1]+alphavpar[0])); 
  cflFreq += 0.5*(alphaR+std::abs(alphaR)); 
  alphaR = 0.03125*((-0.3061862178478971*alphavpar[19])+0.3061862178478971*alphavpar[18]+0.1767766952966368*(alphavpar[16]+alphavpar[14])-0.3061862178478971*alphavpar[11]+0.3061862178478971*alphavpar[10]-0.3061862178478971*alphavpar[9]-0.1767766952966368*alphavpar[8]+0.1767766952966368*alphavpar[7]-0.1767766952966368*(alphavpar[6]+alphavpar[5])+0.3061862178478971*alphavpar[4]-0.1767766952966368*alphavpar[3]+0.1767766952966368*alphavpar[2]-0.1767766952966368*alphavpar[1]+0.1767766952966368*alphavpar[0]); 
  cflFreq += 0.5*(alphaR+std::abs(alphaR)); 
  alphaR = 0.03125*((-0.3061862178478971*(alphavpar[19]+alphavpar[18]))-0.1767766952966368*alphavpar[16]+0.1767766952966368*alphavpar[14]-0.3061862178478971*alphavpar[11]+0.3061862178478971*(alphavpar[10]+alphavpar[9])-0.1767766952966368*(alphavpar[8]+alphavpar[7])+0.1767766952966368*alphavpar[6]-0.1767766952966368*alphavpar[5]+0.3061862178478971*alphavpar[4]-0.1767766952966368*alphavpar[3]+0.1767766952966368*(alphavpar[2]+alphavpar[1]+alphavpar[0])); 
  cflFreq += 0.5*(alphaR+std::abs(alphaR)); 
  alphaR = 0.03125*((-0.3061862178478971*(alphavpar[19]+alphavpar[18]))+0.1767766952966368*alphavpar[16]-0.1767766952966368*alphavpar[14]+0.3061862178478971*alphavpar[11]-0.3061862178478971*(alphavpar[10]+alphavpar[9])-0.1767766952966368*(alphavpar[8]+alphavpar[7])+0.1767766952966368*alphavpar[6]-0.1767766952966368*alphavpar[5]+0.3061862178478971*alphavpar[4]+0.1767766952966368*alphavpar[3]-0.1767766952966368*(alphavpar[2]+alphavpar[1])+0.1767766952966368*alphavpar[0]); 
  cflFreq += 0.5*(alphaR+std::abs(alphaR)); 
  alphaR = 0.03125*((-0.3061862178478971*alphavpar[19])+0.3061862178478971*alphavpar[18]-0.1767766952966368*(alphavpar[16]+alphavpar[14])+0.3061862178478971*alphavpar[11]-0.3061862178478971*alphavpar[10]+0.3061862178478971*alphavpar[9]-0.1767766952966368*alphavpar[8]+0.1767766952966368*alphavpar[7]-0.1767766952966368*(alphavpar[6]+alphavpar[5])+0.3061862178478971*alphavpar[4]+0.1767766952966368*alphavpar[3]-0.1767766952966368*alphavpar[2]+0.1767766952966368*(alphavpar[1]+alphavpar[0])); 
  ...
  alphaR = 0.03125*(0.3061862178478971*alphavpar[19]-0.3061862178478971*alphavpar[18]-0.1767766952966368*(alphavpar[16]+alphavpar[14])+0.3061862178478971*(alphavpar[11]+alphavpar[10])-0.3061862178478971*alphavpar[9]+0.1767766952966368*alphavpar[8]-0.1767766952966368*(alphavpar[7]+alphavpar[6]+alphavpar[5])+0.3061862178478971*alphavpar[4]+0.1767766952966368*(alphavpar[3]+alphavpar[2])-0.1767766952966368*alphavpar[1]+0.1767766952966368*alphavpar[0]); 
  cflFreq += 0.5*(alphaR+std::abs(alphaR)); 
  alphaR = 0.03125*(0.3061862178478971*(alphavpar[19]+alphavpar[18])+0.1767766952966368*alphavpar[16]-0.1767766952966368*alphavpar[14]+0.3061862178478971*(alphavpar[11]+alphavpar[10]+alphavpar[9])+0.1767766952966368*(alphavpar[8]+alphavpar[7]+alphavpar[6])-0.1767766952966368*alphavpar[5]+0.3061862178478971*alphavpar[4]+0.1767766952966368*(alphavpar[3]+alphavpar[2]+alphavpar[1]+alphavpar[0])); 
  cflFreq += 0.5*(alphaR+std::abs(alphaR)); 
  alphaR = 0.03125*(0.3061862178478971*(alphavpar[19]+alphavpar[18])-0.1767766952966368*(alphavpar[16]+alphavpar[14])-0.3061862178478971*(alphavpar[11]+alphavpar[10]+alphavpar[9])+0.1767766952966368*(alphavpar[8]+alphavpar[7]+alphavpar[6]+alphavpar[5])+0.3061862178478971*alphavpar[4]-0.1767766952966368*(alphavpar[3]+alphavpar[2]+alphavpar[1])+0.1767766952966368*alphavpar[0]); 
  cflFreq += 0.5*(alphaR+std::abs(alphaR)); 
  alphaR = 0.03125*(0.3061862178478971*alphavpar[19]-0.3061862178478971*alphavpar[18]+0.1767766952966368*alphavpar[16]-0.1767766952966368*alphavpar[14]-0.3061862178478971*(alphavpar[11]+alphavpar[10])+0.3061862178478971*alphavpar[9]+0.1767766952966368*alphavpar[8]-0.1767766952966368*(alphavpar[7]+alphavpar[6])+0.1767766952966368*alphavpar[5]+0.3061862178478971*alphavpar[4]-0.1767766952966368*(alphavpar[3]+alphavpar[2])+0.1767766952966368*(alphavpar[1]+alphavpar[0])); 
  cflFreq += 0.5*(alphaR+std::abs(alphaR)); 
  alphaR = 0.03125*((-0.3061862178478971*alphavpar[19])+0.3061862178478971*alphavpar[18]+0.1767766952966368*alphavpar[16]-0.1767766952966368*alphavpar[14]-0.3061862178478971*alphavpar[11]+0.3061862178478971*alphavpar[10]-0.3061862178478971*alphavpar[9]-0.1767766952966368*alphavpar[8]+0.1767766952966368*alphavpar[7]-0.1767766952966368*alphavpar[6]+0.1767766952966368*alphavpar[5]+0.3061862178478971*alphavpar[4]-0.1767766952966368*alphavpar[3]+0.1767766952966368*alphavpar[2]-0.1767766952966368*alphavpar[1]+0.1767766952966368*alphavpar[0]); 
  cflFreq += 0.5*(alphaR+std::abs(alphaR)); 
  alphaR = 0.03125*((-0.3061862178478971*(alphavpar[19]+alphavpar[18]))-0.1767766952966368*(alphavpar[16]+alphavpar[14])-0.3061862178478971*alphavpar[11]+0.3061862178478971*(alphavpar[10]+alphavpar[9])-0.1767766952966368*(alphavpar[8]+alphavpar[7])+0.1767766952966368*(alphavpar[6]+alphavpar[5])+0.3061862178478971*alphavpar[4]-0.1767766952966368*alphavpar[3]+0.1767766952966368*(alphavpar[2]+alphavpar[1]+alphavpar[0])); 
  cflFreq += 0.5*(alphaR+std::abs(alphaR)); 
  alphaR = 0.03125*((-0.3061862178478971*(alphavpar[19]+alphavpar[18]))+0.1767766952966368*(alphavpar[16]+alphavpar[14])+0.3061862178478971*alphavpar[11]-0.3061862178478971*(alphavpar[10]+alphavpar[9])-0.1767766952966368*(alphavpar[8]+alphavpar[7])+0.1767766952966368*(alphavpar[6]+alphavpar[5])+0.3061862178478971*alphavpar[4]+0.1767766952966368*alphavpar[3]-0.1767766952966368*(alphavpar[2]+alphavpar[1])+0.1767766952966368*alphavpar[0]); 
  cflFreq += 0.5*(alphaR+std::abs(alphaR)); 
  alphaR = 0.03125*((-0.3061862178478971*alphavpar[19])+0.3061862178478971*alphavpar[18]-0.1767766952966368*alphavpar[16]+0.1767766952966368*alphavpar[14]+0.3061862178478971*alphavpar[11]-0.3061862178478971*alphavpar[10]+0.3061862178478971*alphavpar[9]-0.1767766952966368*alphavpar[8]+0.1767766952966368*alphavpar[7]-0.1767766952966368*alphavpar[6]+0.1767766952966368*alphavpar[5]+0.3061862178478971*alphavpar[4]+0.1767766952966368*alphavpar[3]-0.1767766952966368*alphavpar[2]+0.1767766952966368*(alphavpar[1]+alphavpar[0])); 
  cflFreq += 0.5*(alphaR+std::abs(alphaR)); 
  alphaR = 0.03125*(0.3061862178478971*alphavpar[19]-0.3061862178478971*alphavpar[18]-0.1767766952966368*alphavpar[16]+0.1767766952966368*alphavpar[14]+0.3061862178478971*(alphavpar[11]+alphavpar[10])-0.3061862178478971*alphavpar[9]+0.1767766952966368*alphavpar[8]-0.1767766952966368*(alphavpar[7]+alphavpar[6])+0.1767766952966368*alphavpar[5]+0.3061862178478971*alphavpar[4]+0.1767766952966368*(alphavpar[3]+alphavpar[2])-0.1767766952966368*alphavpar[1]+0.1767766952966368*alphavpar[0]); 
  cflFreq += 0.5*(alphaR+std::abs(alphaR)); 
  alphaR = 0.03125*(0.3061862178478971*(alphavpar[19]+alphavpar[18])+0.1767766952966368*(alphavpar[16]+alphavpar[14])+0.3061862178478971*(alphavpar[11]+alphavpar[10]+alphavpar[9])+0.1767766952966368*(alphavpar[8]+alphavpar[7]+alphavpar[6]+alphavpar[5])+0.3061862178478971*alphavpar[4]+0.1767766952966368*(alphavpar[3]+alphavpar[2]+alphavpar[1]+alphavpar[0])); 
  cflFreq += 0.5*(alphaR+std::abs(alphaR)); 
#endif 
  ...
  out[3] += 0.3061862178478971*(alphaz[8]*f[8]+alphaz[7]*f[7]+alphaz[3]*f[3]+alphaz[2]*f[2]+alphaz[1]*f[1]+alphaz[0]*f[0]); 
  out[4] += 0.3061862178478971*(alphavpar[19]*f[19]+alphavpar[18]*f[18]+alphavpar[16]*f[16]+alphavpar[14]*f[14]+alphavpar[11]*f[11]+alphavpar[10]*f[10]+alphavpar[9]*f[9]+alphavpar[8]*f[8]+alphavpar[7]*f[7]+alphavpar[6]*f[6]+alphavpar[5]*f[5]+alphavpar[4]*f[4]+alphavpar[3]*f[3]+alphavpar[2]*f[2]+alphavpar[1]*f[1]+alphavpar[0]*f[0]); 
  ...
  out[9] += 0.3061862178478971*((alphavpar[19]+alphax[16])*f[26]+alphax[14]*f[25]+alphavpar[14]*f[21]+alphax[8]*f[19]+(alphavpar[11]+alphax[7])*f[18]+f[11]*alphavpar[18]+(alphavpar[10]+alphax[6])*f[17]+alphavpar[8]*f[16]+f[8]*alphavpar[16]+alphax[5]*f[15]+alphavpar[5]*f[12]+alphax[3]*f[11]+f[3]*alphax[11]+alphax[2]*f[10]+(alphavpar[4]+alphax[1])*f[9]+f[4]*alphavpar[9]+alphavpar[3]*f[7]+f[3]*alphavpar[7]+alphavpar[2]*f[6]+f[2]*alphavpar[6]+alphax[0]*f[4]+f[0]*alphax[4]+alphavpar[0]*f[1]+f[0]*alphavpar[1]); 
  out[10] += 0.3061862178478971*((alphavpar[18]+alphay[16])*f[26]+alphay[14]*f[25]+alphavpar[14]*f[22]+(alphavpar[11]+alphay[8])*f[19]+f[11]*alphavpar[19]+alphay[7]*f[18]+(alphavpar[9]+alphay[6])*f[17]+alphavpar[7]*f[16]+f[7]*alphavpar[16]+alphay[5]*f[15]+alphavpar[5]*f[13]+alphay[3]*f[11]+f[3]*alphay[11]+(alphavpar[4]+alphay[2])*f[10]+f[4]*alphavpar[10]+alphay[1]*f[9]+alphavpar[3]*f[8]+f[3]*alphavpar[8]+alphavpar[1]*f[6]+f[1]*alphavpar[6]+alphay[0]*f[4]+f[0]*alphay[4]+alphavpar[0]*f[2]+f[0]*alphavpar[2]); 
  out[11] += 0.3061862178478971*((alphavpar[10]+alphaz[8])*f[19]+f[10]*alphavpar[19]+(alphavpar[9]+alphaz[7])*f[18]+f[9]*alphavpar[18]+alphavpar[6]*f[16]+f[6]*alphavpar[16]+alphavpar[5]*f[14]+f[5]*alphavpar[14]+(alphavpar[4]+alphaz[3])*f[11]+f[4]*alphavpar[11]+alphaz[2]*f[10]+alphaz[1]*f[9]+alphavpar[2]*f[8]+f[2]*alphavpar[8]+alphavpar[1]*f[7]+f[1]*alphavpar[7]+alphaz[0]*f[4]+alphavpar[0]*f[3]+f[0]*alphavpar[3]); 
  ...
  out[14] += 0.3061862178478971*(alphaz[8]*f[22]+alphaz[7]*f[21]+alphaz[3]*f[14]+alphaz[2]*f[13]+alphaz[1]*f[12]+alphaz[0]*f[5]); 
  out[15] += 0.3061862178478971*(alphavpar[19]*f[30]+alphavpar[18]*f[29]+alphavpar[16]*f[27]+alphavpar[11]*f[25]+alphavpar[10]*f[24]+alphavpar[9]*f[23]+alphavpar[8]*f[22]+alphavpar[7]*f[21]+alphavpar[6]*f[20]+alphavpar[4]*f[15]+alphavpar[3]*f[14]+f[3]*alphavpar[14]+alphavpar[2]*f[13]+alphavpar[1]*f[12]+alphavpar[0]*f[5]+f[0]*alphavpar[5]); 
  ...
  out[17] += 0.3061862178478971*(alphax[14]*f[30]+alphay[14]*f[29]+alphavpar[14]*f[27]+(alphavpar[11]+alphay[8]+alphax[7])*f[26]+alphax[5]*f[24]+alphay[5]*f[23]+alphavpar[5]*f[20]+(alphavpar[18]+alphay[16]+alphax[3])*f[19]+f[18]*(alphavpar[19]+alphax[16]+alphay[3])+(alphavpar[4]+alphay[2]+alphax[1])*f[17]+alphavpar[3]*f[16]+f[3]*alphavpar[16]+(alphax[8]+alphay[7])*f[11]+f[7]*alphay[11]+f[8]*alphax[11]+(alphavpar[9]+alphay[6]+alphax[0])*f[10]+f[9]*(alphavpar[10]+alphax[6]+alphay[0])+alphavpar[7]*f[8]+f[7]*alphavpar[8]+alphavpar[0]*f[6]+f[0]*alphavpar[6]+(alphax[2]+alphay[1])*f[4]+f[1]*alphay[4]+f[2]*(alphax[4]+alphavpar[1])+f[1]*alphavpar[2]); 
  out[18] += 0.3061862178478971*((alphavpar[10]+alphaz[8]+alphax[6])*f[26]+alphax[5]*f[25]+alphavpar[5]*f[21]+alphax[2]*f[19]+f[17]*alphavpar[19]+(alphavpar[4]+alphaz[3]+alphax[1])*f[18]+f[4]*alphavpar[18]+(alphax[16]+alphaz[2])*f[17]+alphavpar[2]*f[16]+f[2]*alphavpar[16]+alphax[14]*f[15]+f[12]*alphavpar[14]+(alphavpar[9]+alphaz[7]+alphax[0])*f[11]+f[0]*alphax[11]+f[9]*alphavpar[11]+alphax[8]*f[10]+(alphax[7]+alphaz[0])*f[9]+alphavpar[6]*f[8]+f[6]*alphavpar[8]+alphavpar[0]*f[7]+f[0]*alphavpar[7]+(alphax[3]+alphaz[1])*f[4]+f[3]*(alphax[4]+alphavpar[1])+f[1]*alphavpar[3]); 
  out[19] += 0.3061862178478971*((alphavpar[9]+alphaz[7]+alphay[6])*f[26]+alphay[5]*f[25]+alphavpar[5]*f[22]+(alphavpar[4]+alphaz[3]+alphay[2])*f[19]+f[4]*alphavpar[19]+alphay[1]*f[18]+f[17]*(alphavpar[18]+alphay[16]+alphaz[1])+alphavpar[1]*f[16]+f[1]*alphavpar[16]+alphay[14]*f[15]+f[13]*alphavpar[14]+(alphavpar[10]+alphaz[8]+alphay[0])*f[11]+f[0]*alphay[11]+f[10]*(alphavpar[11]+alphay[8]+alphaz[0])+alphay[7]*f[9]+alphavpar[0]*f[8]+f[0]*alphavpar[8]+alphavpar[6]*f[7]+f[6]*alphavpar[7]+(alphay[3]+alphaz[2])*f[4]+f[3]*(alphay[4]+alphavpar[2])+f[2]*alphavpar[3]); 
  ...
  out[28] += 0.3061862178478971*((alphavpar[11]+alphay[8]+alphax[7])*f[31]+(alphavpar[18]+alphay[16]+alphax[3])*f[30]+(alphavpar[19]+alphax[16]+alphay[3])*f[29]+(alphavpar[4]+alphay[2]+alphax[1])*f[28]+alphavpar[3]*f[27]+(alphax[8]+alphay[7])*f[25]+(alphavpar[9]+alphay[6]+alphax[0])*f[24]+(alphavpar[10]+alphax[6]+alphay[0])*f[23]+(alphax[11]+alphavpar[7])*f[22]+(alphay[11]+alphavpar[8])*f[21]+alphavpar[0]*f[20]+alphax[14]*f[19]+alphay[14]*f[18]+alphavpar[14]*f[16]+f[14]*alphavpar[16]+(alphax[2]+alphay[1])*f[15]+(alphax[4]+alphavpar[1])*f[13]+(alphay[4]+alphavpar[2])*f[12]+alphax[5]*f[10]+alphay[5]*f[9]+alphavpar[5]*f[6]+f[5]*alphavpar[6]); 
  out[29] += 0.3061862178478971*((alphavpar[10]+alphaz[8]+alphax[6])*f[31]+alphax[2]*f[30]+(alphavpar[4]+alphaz[3]+alphax[1])*f[29]+(alphavpar[19]+alphax[16]+alphaz[2])*f[28]+alphavpar[2]*f[27]+(alphavpar[9]+alphaz[7]+alphax[0])*f[25]+alphax[8]*f[24]+(alphavpar[11]+alphax[7]+alphaz[0])*f[23]+alphavpar[6]*f[22]+alphavpar[0]*f[21]+alphavpar[8]*f[20]+f[15]*alphavpar[18]+f[13]*alphavpar[16]+(alphax[3]+alphaz[1])*f[15]+(alphax[4]+alphavpar[1])*f[14]+f[4]*alphax[14]+f[1]*alphavpar[14]+alphavpar[3]*f[12]+alphax[5]*f[11]+f[5]*alphax[11]+alphavpar[5]*f[7]+f[5]*alphavpar[7]); 
  out[30] += 0.3061862178478971*((alphavpar[9]+alphaz[7]+alphay[6])*f[31]+(alphavpar[4]+alphaz[3]+alphay[2])*f[30]+alphay[1]*f[29]+(alphavpar[18]+alphay[16]+alphaz[1])*f[28]+alphavpar[1]*f[27]+(alphavpar[10]+alphaz[8]+alphay[0])*f[25]+(alphavpar[11]+alphay[8]+alphaz[0])*f[24]+alphay[7]*f[23]+alphavpar[0]*f[22]+alphavpar[6]*f[21]+alphavpar[7]*f[20]+f[15]*alphavpar[19]+f[12]*alphavpar[16]+(alphay[3]+alphaz[2])*f[15]+(alphay[4]+alphavpar[2])*f[14]+f[4]*alphay[14]+f[2]*alphavpar[14]+alphavpar[3]*f[13]+alphay[5]*f[11]+f[5]*alphay[11]+alphavpar[5]*f[8]+f[5]*alphavpar[8]); 
  out[31] += 0.3061862178478971*((alphavpar[4]+alphaz[3]+alphay[2]+alphax[1])*f[31]+(alphavpar[9]+alphaz[7]+alphay[6]+alphax[0])*f[30]+(alphavpar[10]+alphaz[8]+alphax[6]+alphay[0])*f[29]+(alphavpar[11]+alphay[8]+alphax[7]+alphaz[0])*f[28]+alphavpar[0]*f[27]+(alphax[2]+alphay[1])*f[25]+(alphavpar[18]+alphay[16]+alphax[3]+alphaz[1])*f[24]+(alphavpar[19]+alphax[16]+alphay[3]+alphaz[2])*f[23]+(alphax[4]+alphavpar[1])*f[22]+(alphay[4]+alphavpar[2])*f[21]+alphavpar[3]*f[20]+alphax[5]*f[19]+alphay[5]*f[18]+alphavpar[5]*f[16]+f[5]*alphavpar[16]+(alphax[8]+alphay[7])*f[15]+alphavpar[6]*f[14]+f[9]*alphay[14]+f[10]*alphax[14]+f[6]*alphavpar[14]+(alphax[11]+alphavpar[7])*f[13]+(alphay[11]+alphavpar[8])*f[12]); 
  return cflFreq; 
} 
double GyrokineticGenGeoVol3x2vSerP1_Bvarsxz(const double q_, const double m_, const double *w, const double *dxv, const double *bmag, const double *jacobTotInv, const double *cmag, const double *b_x, const double *b_y, const double *b_z, const double *phi, const double *f, double *out) 
{ 
  // jacobTotInv: reciprocal of the conf-space jacobian time the guiding center coordinate Jacobian.
  // b_x,b_y,b_z: covariant components of the field aligned unit vector.
  // q_,m_: species charge and mass.
  ...
  double BstarXdBmag[32]; 
  BstarXdBmag[0] = -(1.224744871391589*(jacobTotInv[1]*b_y[5]+jacobTotInv[0]*b_y[3])*m_*rdz2*wvpar)/q_; 
  BstarXdBmag[1] = -(1.224744871391589*(jacobTotInv[0]*b_y[5]+jacobTotInv[1]*b_y[3])*m_*rdz2*wvpar)/q_; 
  BstarXdBmag[3] = -(1.224744871391589*(b_y[5]*jacobTotInv[5]+b_y[3]*jacobTotInv[3])*m_*rdz2*wvpar)/q_; 
  BstarXdBmag[4] = -(0.7071067811865475*(jacobTotInv[1]*b_y[5]+jacobTotInv[0]*b_y[3])*m_*rdz2)/(q_*rdvpar2); 
  BstarXdBmag[7] = -(1.224744871391589*(b_y[3]*jacobTotInv[5]+jacobTotInv[3]*b_y[5])*m_*rdz2*wvpar)/q_; 
  ...
  double BstarYdBmag[32]; 
  BstarYdBmag[0] = (1.224744871391589*m_*((jacobTotInv[1]*b_x[5]+jacobTotInv[0]*b_x[3])*rdz2-1.0*(jacobTotInv[3]*b_z[5]+jacobTotInv[0]*b_z[1])*rdx2)*wvpar)/q_; 
  BstarYdBmag[1] = (1.224744871391589*m_*((jacobTotInv[0]*b_x[5]+jacobTotInv[1]*b_x[3])*rdz2-1.0*(b_z[5]*jacobTotInv[5]+b_z[1]*jacobTotInv[1])*rdx2)*wvpar)/q_; 
  BstarYdBmag[3] = (1.224744871391589*m_*((b_x[5]*jacobTotInv[5]+b_x[3]*jacobTotInv[3])*rdz2-1.0*(jacobTotInv[0]*b_z[5]+b_z[1]*jacobTotInv[3])*rdx2)*wvpar)/q_; 
  ...
  BstarYdBmag[7] = (1.224744871391589*m_*((b_x[3]*jacobTotInv[5]+jacobTotInv[3]*b_x[5])*rdz2-1.0*(b_z[1]*jacobTotInv[5]+jacobTotInv[1]*b_z[5])*rdx2)*wvpar)/q_; 
  ...
  double BstarZdBmag[32]; 
  BstarZdBmag[0] = (0.7071067811865475*(1.732050807568877*(jacobTotInv[3]*b_y[5]+jacobTotInv[0]*b_y[1])*m_*rdx2*wvpar+(cmag[5]*jacobTotInv[5]+cmag[3]*jacobTotInv[3]+cmag[1]*jacobTotInv[1]+cmag[0]*jacobTotInv[0])*q_))/q_; 
  BstarZdBmag[1] = (0.7071067811865475*(1.732050807568877*(b_y[5]*jacobTotInv[5]+b_y[1]*jacobTotInv[1])*m_*rdx2*wvpar+(cmag[3]*jacobTotInv[5]+jacobTotInv[3]*cmag[5]+cmag[0]*jacobTotInv[1]+jacobTotInv[0]*cmag[1])*q_))/q_; 
  BstarZdBmag[3] = (0.7071067811865475*(1.732050807568877*(jacobTotInv[0]*b_y[5]+b_y[1]*jacobTotInv[3])*m_*rdx2*wvpar+(cmag[1]*jacobTotInv[5]+jacobTotInv[1]*cmag[5]+cmag[0]*jacobTotInv[3]+jacobTotInv[0]*cmag[3])*q_))/q_; 
  BstarZdBmag[4] = (0.7071067811865475*(jacobTotInv[3]*b_y[5]+jacobTotInv[0]*b_y[1])*m_*rdx2)/(q_*rdvpar2); 
  BstarZdBmag[7] = (0.7071067811865475*(1.732050807568877*(b_y[1]*jacobTotInv[5]+jacobTotInv[1]*b_y[5])*m_*rdx2*wvpar+(cmag[0]*jacobTotInv[5]+jacobTotInv[0]*cmag[5]+cmag[1]*jacobTotInv[3]+jacobTotInv[1]*cmag[3])*q_))/q_; 
  ...
  alphax[2] = (0.2165063509461096*((b_y[3]*jacobTotInv[5]+jacobTotInv[3]*b_y[5]+b_y[0]*jacobTotInv[1]+jacobTotInv[0]*b_y[1])*hamil[16]+(b_y[5]*jacobTotInv[5]+b_y[3]*jacobTotInv[3]+b_y[1]*jacobTotInv[1]+b_y[0]*jacobTotInv[0])*hamil[8])*rdx2*rdz2)/q_; 
  ...
  alphax[6] = (1.732050807568877*(0.125*(b_y[0]*jacobTotInv[0]*hamil[16]+(b_y[3]*jacobTotInv[5]+jacobTotInv[3]*b_y[5]+b_y[0]*jacobTotInv[1]+jacobTotInv[0]*b_y[1])*hamil[8])+(0.225*b_y[5]*jacobTotInv[5]+0.125*b_y[3]*jacobTotInv[3]+0.225*b_y[1]*jacobTotInv[1])*hamil[16])*rdx2*rdz2)/q_; 
  ...
  alphax[8] = (0.2165063509461096*((b_y[0]*jacobTotInv[5]+jacobTotInv[0]*b_y[5]+b_y[1]*jacobTotInv[3]+jacobTotInv[1]*b_y[3])*hamil[16]+(b_y[1]*jacobTotInv[5]+jacobTotInv[1]*b_y[5]+b_y[0]*jacobTotInv[3]+jacobTotInv[0]*b_y[3])*hamil[8])*rdx2*rdz2)/q_; 
  ...
  alphax[16] = (1.732050807568877*(0.125*((b_y[0]*jacobTotInv[3]+jacobTotInv[0]*b_y[3])*hamil[16]+(b_y[0]*jacobTotInv[5]+jacobTotInv[0]*b_y[5]+b_y[1]*jacobTotInv[3]+jacobTotInv[1]*b_y[3])*hamil[8])+0.225*(b_y[1]*jacobTotInv[5]+jacobTotInv[1]*b_y[5])*hamil[16])*rdx2*rdz2)/q_; 
  ...
  alphay[8] = (1.732050807568877*rdy2*((0.125*((b_z[1]*jacobTotInv[1]+b_z[0]*jacobTotInv[0])*hamil[16]+(b_z[1]*jacobTotInv[5]+jacobTotInv[1]*b_z[5]+b_z[0]*jacobTotInv[3]+jacobTotInv[0]*b_z[3])*hamil[6])+0.225*(b_z[5]*jacobTotInv[5]+b_z[3]*jacobTotInv[3])*hamil[16])*rdx2-0.125*((b_x[0]*jacobTotInv[5]+jacobTotInv[0]*b_x[5]+b_x[1]*jacobTotInv[3]+jacobTotInv[1]*b_x[3])*hamil[16]+(b_x[1]*jacobTotInv[5]+jacobTotInv[1]*b_x[5]+b_x[0]*jacobTotInv[3]+jacobTotInv[0]*b_x[3])*hamil[8])*rdz2))/q_; 
  alphay[9] = (0.3061862178478971*hamil[4]*BstarYdBmag[9]*rdvpar2*rdy2)/m_; 
  alphay[11] = (0.3061862178478971*hamil[4]*BstarYdBmag[11]*rdvpar2*rdy2)/m_; 
  ...
  alphay[16] = (1.732050807568877*rdy2*(((-0.125*((b_x[0]*jacobTotInv[3]+jacobTotInv[0]*b_x[3])*hamil[16]+(b_x[0]*jacobTotInv[5]+jacobTotInv[0]*b_x[5]+b_x[1]*jacobTotInv[3]+jacobTotInv[1]*b_x[3])*hamil[8]))-0.225*(b_x[1]*jacobTotInv[5]+jacobTotInv[1]*b_x[5])*hamil[16])*rdz2+(0.125*((b_z[0]*jacobTotInv[1]+jacobTotInv[0]*b_z[1])*hamil[16]+(b_z[0]*jacobTotInv[5]+jacobTotInv[0]*b_z[5]+b_z[1]*jacobTotInv[3]+jacobTotInv[1]*b_z[3])*hamil[6])+0.225*(b_z[3]*jacobTotInv[5]+jacobTotInv[3]*b_z[5])*hamil[16])*rdx2))/q_; 
  ...
  alphaz[2] = -(0.2165063509461096*((b_y[1]*jacobTotInv[5]+jacobTotInv[1]*b_y[5]+b_y[0]*jacobTotInv[3]+jacobTotInv[0]*b_y[3])*hamil[16]+(b_y[5]*jacobTotInv[5]+b_y[3]*jacobTotInv[3]+b_y[1]*jacobTotInv[1]+b_y[0]*jacobTotInv[0])*hamil[6])*rdx2*rdz2)/q_; 
  ...
  alphaz[6] = -(0.2165063509461096*((b_y[0]*jacobTotInv[5]+jacobTotInv[0]*b_y[5]+b_y[1]*jacobTotInv[3]+jacobTotInv[1]*b_y[3])*hamil[16]+(b_y[3]*jacobTotInv[5]+jacobTotInv[3]*b_y[5]+b_y[0]*jacobTotInv[1]+jacobTotInv[0]*b_y[1])*hamil[6])*rdx2*rdz2)/q_; 
  ...
  alphaz[8] = (1.732050807568877*((-0.125*((b_y[1]*jacobTotInv[1]+b_y[0]*jacobTotInv[0])*hamil[16]+(b_y[1]*jacobTotInv[5]+jacobTotInv[1]*b_y[5]+b_y[0]*jacobTotInv[3]+jacobTotInv[0]*b_y[3])*hamil[6]))-0.225*(b_y[5]*jacobTotInv[5]+b_y[3]*jacobTotInv[3])*hamil[16])*rdx2*rdz2)/q_; 
  alphaz[9] = (0.3061862178478971*hamil[4]*BstarZdBmag[9]*rdvpar2*rdz2)/m_; 
  ...
  alphaz[16] = (1.732050807568877*((-0.125*((b_y[0]*jacobTotInv[1]+jacobTotInv[0]*b_y[1])*hamil[16]+(b_y[0]*jacobTotInv[5]+jacobTotInv[0]*b_y[5]+b_y[1]*jacobTotInv[3]+jacobTotInv[1]*b_y[3])*hamil[6]))-0.225*(b_y[3]*jacobTotInv[5]+jacobTotInv[3]*b_y[5])*hamil[16])*rdx2*rdz2)/q_; 
  ...
  double alphavpar[32]; 
  alphavpar[0] = -(0.3061862178478971*rdvpar2*((BstarZdBmag[1]*hamil[7]+BstarZdBmag[0]*hamil[3])*rdz2+(BstarYdBmag[7]*hamil[16]+BstarYdBmag[3]*hamil[8]+BstarYdBmag[1]*hamil[6]+BstarYdBmag[0]*hamil[2])*rdy2+(BstarXdBmag[3]*hamil[7]+BstarXdBmag[0]*hamil[1])*rdx2))/m_; 
  alphavpar[1] = -(0.3061862178478971*rdvpar2*((BstarZdBmag[0]*hamil[7]+BstarZdBmag[1]*hamil[3])*rdz2+(BstarYdBmag[3]*hamil[16]+BstarYdBmag[7]*hamil[8]+BstarYdBmag[0]*hamil[6]+BstarYdBmag[1]*hamil[2])*rdy2+(BstarXdBmag[7]*hamil[7]+BstarXdBmag[1]*hamil[1])*rdx2))/m_; 
  alphavpar[2] = -(0.3061862178478971*rdvpar2*((BstarZdBmag[1]*hamil[16]+BstarZdBmag[0]*hamil[8])*rdz2+(BstarXdBmag[3]*hamil[16]+BstarXdBmag[0]*hamil[6])*rdx2))/m_; 
  alphavpar[3] = -(0.3061862178478971*rdvpar2*((BstarZdBmag[7]*hamil[7]+BstarZdBmag[3]*hamil[3])*rdz2+(BstarYdBmag[1]*hamil[16]+BstarYdBmag[0]*hamil[8]+hamil[6]*BstarYdBmag[7]+hamil[2]*BstarYdBmag[3])*rdy2+(BstarXdBmag[0]*hamil[7]+hamil[1]*BstarXdBmag[3])*rdx2))/m_; 
  ...
  alphavpar[6] = -(0.3061862178478971*rdvpar2*((BstarZdBmag[0]*hamil[16]+BstarZdBmag[1]*hamil[8])*rdz2+(BstarXdBmag[7]*hamil[16]+BstarXdBmag[1]*hamil[6])*rdx2))/m_; 
  alphavpar[7] = -(0.3061862178478971*rdvpar2*((BstarZdBmag[3]*hamil[7]+hamil[3]*BstarZdBmag[7])*rdz2+(BstarYdBmag[0]*hamil[16]+BstarYdBmag[1]*hamil[8]+hamil[2]*BstarYdBmag[7]+BstarYdBmag[3]*hamil[6])*rdy2+(BstarXdBmag[1]*hamil[7]+hamil[1]*BstarXdBmag[7])*rdx2))/m_; 
  alphavpar[8] = -(0.3061862178478971*rdvpar2*((BstarZdBmag[7]*hamil[16]+BstarZdBmag[3]*hamil[8])*rdz2+(BstarXdBmag[0]*hamil[16]+BstarXdBmag[3]*hamil[6])*rdx2))/m_; 
  ...
  alphavpar[16] = -(0.3061862178478971*rdvpar2*((BstarZdBmag[3]*hamil[16]+BstarZdBmag[7]*hamil[8])*rdz2+(BstarXdBmag[1]*hamil[16]+hamil[6]*BstarXdBmag[7])*rdx2))/m_; 
...
#elif cflType == QUAD 
  // Evaluate alpha at left surface quadrature points.
  alphaL = 0.03125*(0.3061862178478971*(alphavpar[29]+alphavpar[26])-0.3061862178478971*(alphavpar[25]+alphavpar[23])-0.1767766952966368*alphavpar[21]-0.3061862178478971*(alphavpar[19]+alphavpar[18]+alphavpar[17])-0.1767766952966368*alphavpar[16]+0.3061862178478971*alphavpar[15]+0.1767766952966368*(alphavpar[14]+alphavpar[12])+0.3061862178478971*(alphavpar[11]+alphavpar[10]+alphavpar[9])+0.1767766952966368*(alphavpar[8]+alphavpar[7]+alphavpar[6])-0.1767766952966368*alphavpar[5]-0.3061862178478971*alphavpar[4]-0.1767766952966368*(alphavpar[3]+alphavpar[2]+alphavpar[1])+0.1767766952966368*alphavpar[0]); 
  cflFreq += -0.5*(alphaL-std::abs(alphaL)); 
  alphaL = 0.03125*((-0.3061862178478971*(alphavpar[29]+alphavpar[26]+alphavpar[25]))+0.3061862178478971*alphavpar[23]+0.1767766952966368*alphavpar[21]-0.3061862178478971*alphavpar[19]+0.3061862178478971*(alphavpar[18]+alphavpar[17])+0.1767766952966368*alphavpar[16]+0.3061862178478971*alphavpar[15]+0.1767766952966368*alphavpar[14]-0.1767766952966368*alphavpar[12]+0.3061862178478971*(alphavpar[11]+alphavpar[10])-0.3061862178478971*alphavpar[9]+0.1767766952966368*alphavpar[8]-0.1767766952966368*(alphavpar[7]+alphavpar[6]+alphavpar[5])-0.3061862178478971*alphavpar[4]-0.1767766952966368*(alphavpar[3]+alphavpar[2])+0.1767766952966368*(alphavpar[1]+alphavpar[0])); 
  cflFreq += -0.5*(alphaL-std::abs(alphaL)); 
  alphaL = 0.03125*(0.3061862178478971*alphavpar[29]-0.3061862178478971*(alphavpar[26]+alphavpar[25]+alphavpar[23])-0.1767766952966368*alphavpar[21]+0.3061862178478971*alphavpar[19]-0.3061862178478971*alphavpar[18]+0.3061862178478971*alphavpar[17]+0.1767766952966368*alphavpar[16]+0.3061862178478971*alphavpar[15]+0.1767766952966368*(alphavpar[14]+alphavpar[12])+0.3061862178478971*alphavpar[11]-0.3061862178478971*alphavpar[10]+0.3061862178478971*alphavpar[9]-0.1767766952966368*alphavpar[8]+0.1767766952966368*alphavpar[7]-0.1767766952966368*(alphavpar[6]+alphavpar[5])-0.3061862178478971*alphavpar[4]-0.1767766952966368*alphavpar[3]+0.1767766952966368*alphavpar[2]-0.1767766952966368*alphavpar[1]+0.1767766952966368*alphavpar[0]); 
  cflFreq += -0.5*(alphaL-std::abs(alphaL)); 
  alphaL = 0.03125*((-0.3061862178478971*alphavpar[29])+0.3061862178478971*alphavpar[26]-0.3061862178478971*alphavpar[25]+0.3061862178478971*alphavpar[23]+0.1767766952966368*alphavpar[21]+0.3061862178478971*(alphavpar[19]+alphavpar[18])-0.3061862178478971*alphavpar[17]-0.1767766952966368*alphavpar[16]+0.3061862178478971*alphavpar[15]+0.1767766952966368*alphavpar[14]-0.1767766952966368*alphavpar[12]+0.3061862178478971*alphavpar[11]-0.3061862178478971*(alphavpar[10]+alphavpar[9])-0.1767766952966368*(alphavpar[8]+alphavpar[7])+0.1767766952966368*alphavpar[6]-0.1767766952966368*alphavpar[5]-0.3061862178478971*alphavpar[4]-0.1767766952966368*alphavpar[3]+0.1767766952966368*(alphavpar[2]+alphavpar[1]+alphavpar[0])); 
  cflFreq += -0.5*(alphaL-std::abs(alphaL)); 
  alphaL = 0.03125*((-0.3061862178478971*(alphavpar[29]+alphavpar[26]))+0.3061862178478971*alphavpar[25]-0.3061862178478971*alphavpar[23]+0.1767766952966368*alphavpar[21]+0.3061862178478971*(alphavpar[19]+alphavpar[18])-0.3061862178478971*alphavpar[17]+0.1767766952966368*alphavpar[16]+0.3061862178478971*alphavpar[15]-0.1767766952966368*alphavpar[14]+0.1767766952966368*alphavpar[12]-0.3061862178478971*alphavpar[11]+0.3061862178478971*(alphavpar[10]+alphavpar[9])-0.1767766952966368*(alphavpar[8]+alphavpar[7])+0.1767766952966368*alphavpar[6]-0.1767766952966368*alphavpar[5]-0.3061862178478971*alphavpar[4]+0.1767766952966368*alphavpar[3]-0.1767766952966368*(alphavpar[2]+alphavpar[1])+0.1767766952966368*alphavpar[0]); 
  cflFreq += -0.5*(alphaL-std::abs(alphaL)); 
  alphaL = 0.03125*(0.3061862178478971*(alphavpar[29]+alphavpar[26]+alphavpar[25]+alphavpar[23])-0.1767766952966368*alphavpar[21]+0.3061862178478971*alphavpar[19]-0.3061862178478971*alphavpar[18]+0.3061862178478971*alphavpar[17]-0.1767766952966368*alphavpar[16]+0.3061862178478971*alphavpar[15]-0.1767766952966368*(alphavpar[14]+alphavpar[12])-0.3061862178478971*alphavpar[11]+0.3061862178478971*alphavpar[10]-0.3061862178478971*alphavpar[9]-0.1767766952966368*alphavpar[8]+0.1767766952966368*alphavpar[7]-0.1767766952966368*(alphavpar[6]+alphavpar[5])-0.3061862178478971*alphavpar[4]+0.1767766952966368*alphavpar[3]-0.1767766952966368*alphavpar[2]+0.1767766952966368*(alphavpar[1]+alphavpar[0])); 
  cflFreq += -0.5*(alphaL-std::abs(alphaL)); 
  alphaL = 0.03125*((-0.3061862178478971*alphavpar[29])+0.3061862178478971*(alphavpar[26]+alphavpar[25])-0.3061862178478971*alphavpar[23]+0.1767766952966368*alphavpar[21]-0.3061862178478971*alphavpar[19]+0.3061862178478971*(alphavpar[18]+alphavpar[17])-0.1767766952966368*alphavpar[16]+0.3061862178478971*alphavpar[15]-0.1767766952966368*alphavpar[14]+0.1767766952966368*alphavpar[12]-0.3061862178478971*(alphavpar[11]+alphavpar[10])+0.3061862178478971*alphavpar[9]+0.1767766952966368*alphavpar[8]-0.1767766952966368*(alphavpar[7]+alphavpar[6]+alphavpar[5])-0.3061862178478971*alphavpar[4]+0.1767766952966368*(alphavpar[3]+alphavpar[2])-0.1767766952966368*alphavpar[1]+0.1767766952966368*alphavpar[0]); 
  cflFreq += -0.5*(alphaL-std::abs(alphaL)); 
  alphaL = 0.03125*(0.3061862178478971*alphavpar[29]-0.3061862178478971*alphavpar[26]+0.3061862178478971*(alphavpar[25]+alphavpar[23])-0.1767766952966368*alphavpar[21]-0.3061862178478971*(alphavpar[19]+alphavpar[18]+alphavpar[17])+0.1767766952966368*alphavpar[16]+0.3061862178478971*alphavpar[15]-0.1767766952966368*(alphavpar[14]+alphavpar[12])-0.3061862178478971*(alphavpar[11]+alphavpar[10]+alphavpar[9])+0.1767766952966368*(alphavpar[8]+alphavpar[7]+alphavpar[6])-0.1767766952966368*alphavpar[5]-0.3061862178478971*alphavpar[4]+0.1767766952966368*(alphavpar[3]+alphavpar[2]+alphavpar[1]+alphavpar[0])); 
  cflFreq += -0.5*(alphaL-std::abs(alphaL)); 
  alphaL = 0.03125*((-0.3061862178478971*alphavpar[29])+0.3061862178478971*(alphavpar[26]+alphavpar[25]+alphavpar[23])+0.1767766952966368*alphavpar[21]-0.3061862178478971*(alphavpar[19]+alphavpar[18]+alphavpar[17])-0.1767766952966368*alphavpar[16]-0.3061862178478971*alphavpar[15]-0.1767766952966368*(alphavpar[14]+alphavpar[12])+0.3061862178478971*(alphavpar[11]+alphavpar[10]+alphavpar[9])+0.1767766952966368*(alphavpar[8]+alphavpar[7]+alphavpar[6]+alphavpar[5])-0.3061862178478971*alphavpar[4]-0.1767766952966368*(alphavpar[3]+alphavpar[2]+alphavpar[1])+0.1767766952966368*alphavpar[0]); 
  cflFreq += -0.5*(alphaL-std::abs(alphaL)); 
  alphaL = 0.03125*(0.3061862178478971*alphavpar[29]-0.3061862178478971*alphavpar[26]+0.3061862178478971*alphavpar[25]-0.3061862178478971*alphavpar[23]-0.1767766952966368*alphavpar[21]-0.3061862178478971*alphavpar[19]+0.3061862178478971*(alphavpar[18]+alphavpar[17])+0.1767766952966368*alphavpar[16]-0.3061862178478971*alphavpar[15]-0.1767766952966368*alphavpar[14]+0.1767766952966368*alphavpar[12]+0.3061862178478971*(alphavpar[11]+alphavpar[10])-0.3061862178478971*alphavpar[9]+0.1767766952966368*alphavpar[8]-0.1767766952966368*(alphavpar[7]+alphavpar[6])+0.1767766952966368*alphavpar[5]-0.3061862178478971*alphavpar[4]-0.1767766952966368*(alphavpar[3]+alphavpar[2])+0.1767766952966368*(alphavpar[1]+alphavpar[0])); 
  cflFreq += -0.5*(alphaL-std::abs(alphaL)); 
  alphaL = 0.03125*((-0.3061862178478971*(alphavpar[29]+alphavpar[26]))+0.3061862178478971*(alphavpar[25]+alphavpar[23])+0.1767766952966368*alphavpar[21]+0.3061862178478971*alphavpar[19]-0.3061862178478971*alphavpar[18]+0.3061862178478971*alphavpar[17]+0.1767766952966368*alphavpar[16]-0.3061862178478971*alphavpar[15]-0.1767766952966368*(alphavpar[14]+alphavpar[12])+0.3061862178478971*alphavpar[11]-0.3061862178478971*alphavpar[10]+0.3061862178478971*alphavpar[9]-0.1767766952966368*alphavpar[8]+0.1767766952966368*alphavpar[7]-0.1767766952966368*alphavpar[6]+0.1767766952966368*alphavpar[5]-0.3061862178478971*alphavpar[4]-0.1767766952966368*alphavpar[3]+0.1767766952966368*alphavpar[2]-0.1767766952966368*alphavpar[1]+0.1767766952966368*alphavpar[0]); 
  cflFreq += -0.5*(alphaL-std::abs(alphaL)); 
  alphaL = 0.03125*(0.3061862178478971*(alphavpar[29]+alphavpar[26]+alphavpar[25])-0.3061862178478971*alphavpar[23]-0.1767766952966368*alphavpar[21]+0.3061862178478971*(alphavpar[19]+alphavpar[18])-0.3061862178478971*alphavpar[17]-0.1767766952966368*alphavpar[16]-0.3061862178478971*alphavpar[15]-0.1767766952966368*alphavpar[14]+0.1767766952966368*alphavpar[12]+0.3061862178478971*alphavpar[11]-0.3061862178478971*(alphavpar[10]+alphavpar[9])-0.1767766952966368*(alphavpar[8]+alphavpar[7])+0.1767766952966368*(alphavpar[6]+alphavpar[5])-0.3061862178478971*alphavpar[4]-0.1767766952966368*alphavpar[3]+0.1767766952966368*(alphavpar[2]+alphavpar[1]+alphavpar[0])); 
  cflFreq += -0.5*(alphaL-std::abs(alphaL)); 
  alphaL = 0.03125*(0.3061862178478971*alphavpar[29]-0.3061862178478971*(alphavpar[26]+alphavpar[25])+0.3061862178478971*alphavpar[23]-0.1767766952966368*alphavpar[21]+0.3061862178478971*(alphavpar[19]+alphavpar[18])-0.3061862178478971*alphavpar[17]+0.1767766952966368*alphavpar[16]-0.3061862178478971*alphavpar[15]+0.1767766952966368*alphavpar[14]-0.1767766952966368*alphavpar[12]-0.3061862178478971*alphavpar[11]+0.3061862178478971*(alphavpar[10]+alphavpar[9])-0.1767766952966368*(alphavpar[8]+alphavpar[7])+0.1767766952966368*(alphavpar[6]+alphavpar[5])-0.3061862178478971*alphavpar[4]+0.1767766952966368*alphavpar[3]-0.1767766952966368*(alphavpar[2]+alphavpar[1])+0.1767766952966368*alphavpar[0]); 
  cflFreq += -0.5*(alphaL-std::abs(alphaL)); 
  alphaL = 0.03125*((-0.3061862178478971*alphavpar[29])+0.3061862178478971*alphavpar[26]-0.3061862178478971*(alphavpar[25]+alphavpar[23])+0.1767766952966368*alphavpar[21]+0.3061862178478971*alphavpar[19]-0.3061862178478971*alphavpar[18]+0.3061862178478971*alphavpar[17]-0.1767766952966368*alphavpar[16]-0.3061862178478971*alphavpar[15]+0.1767766952966368*(alphavpar[14]+alphavpar[12])-0.3061862178478971*alphavpar[11]+0.3061862178478971*alphavpar[10]-0.3061862178478971*alphavpar[9]-0.1767766952966368*alphavpar[8]+0.1767766952966368*alphavpar[7]-0.1767766952966368*alphavpar[6]+0.1767766952966368*alphavpar[5]-0.3061862178478971*alphavpar[4]+0.1767766952966368*alphavpar[3]-0.1767766952966368*alphavpar[2]+0.1767766952966368*(alphavpar[1]+alphavpar[0])); 
  cflFreq += -0.5*(alphaL-std::abs(alphaL)); 
  alphaL = 0.03125*(0.3061862178478971*(alphavpar[29]+alphavpar[26])-0.3061862178478971*alphavpar[25]+0.3061862178478971*alphavpar[23]-0.1767766952966368*alphavpar[21]-0.3061862178478971*alphavpar[19]+0.3061862178478971*(alphavpar[18]+alphavpar[17])-0.1767766952966368*alphavpar[16]-0.3061862178478971*alphavpar[15]+0.1767766952966368*alphavpar[14]-0.1767766952966368*alphavpar[12]-0.3061862178478971*(alphavpar[11]+alphavpar[10])+0.3061862178478971*alphavpar[9]+0.1767766952966368*alphavpar[8]-0.1767766952966368*(alphavpar[7]+alphavpar[6])+0.1767766952966368*alphavpar[5]-0.3061862178478971*alphavpar[4]+0.1767766952966368*(alphavpar[3]+alphavpar[2])-0.1767766952966368*alphavpar[1]+0.1767766952966368*alphavpar[0]); 
  cflFreq += -0.5*(alphaL-std::abs(alphaL)); 
  alphaL = 0.03125*((-0.3061862178478971*(alphavpar[29]+alphavpar[26]+alphavpar[25]+alphavpar[23]))+0.1767766952966368*alphavpar[21]-0.3061862178478971*(alphavpar[19]+alphavpar[18]+alphavpar[17])+0.1767766952966368*alphavpar[16]-0.3061862178478971*alphavpar[15]+0.1767766952966368*(alphavpar[14]+alphavpar[12])-0.3061862178478971*(alphavpar[11]+alphavpar[10]+alphavpar[9])+0.1767766952966368*(alphavpar[8]+alphavpar[7]+alphavpar[6]+alphavpar[5])-0.3061862178478971*alphavpar[4]+0.1767766952966368*(alphavpar[3]+alphavpar[2]+alphavpar[1]+alphavpar[0])); 
  cflFreq += -0.5*(alphaL-std::abs(alphaL)); 
  // Evaluate alpha at right surface quadrature points.
  alphaR = 0.03125*((-0.3061862178478971*(alphavpar[29]+alphavpar[26]))+0.3061862178478971*(alphavpar[25]+alphavpar[23])-0.1767766952966368*alphavpar[21]+0.3061862178478971*(alphavpar[19]+alphavpar[18]+alphavpar[17])-0.1767766952966368*alphavpar[16]-0.3061862178478971*alphavpar[15]+0.1767766952966368*(alphavpar[14]+alphavpar[12])-0.3061862178478971*(alphavpar[11]+alphavpar[10]+alphavpar[9])+0.1767766952966368*(alphavpar[8]+alphavpar[7]+alphavpar[6])-0.1767766952966368*alphavpar[5]+0.3061862178478971*alphavpar[4]-0.1767766952966368*(alphavpar[3]+alphavpar[2]+alphavpar[1])+0.1767766952966368*alphavpar[0]); 
  cflFreq += 0.5*(alphaR+std::abs(alphaR)); 
  alphaR = 0.03125*(0.3061862178478971*(alphavpar[29]+alphavpar[26]+alphavpar[25])-0.3061862178478971*alphavpar[23]+0.1767766952966368*alphavpar[21]+0.3061862178478971*alphavpar[19]-0.3061862178478971*(alphavpar[18]+alphavpar[17])+0.1767766952966368*alphavpar[16]-0.3061862178478971*alphavpar[15]+0.1767766952966368*alphavpar[14]-0.1767766952966368*alphavpar[12]-0.3061862178478971*(alphavpar[11]+alphavpar[10])+0.3061862178478971*alphavpar[9]+0.1767766952966368*alphavpar[8]-0.1767766952966368*(alphavpar[7]+alphavpar[6]+alphavpar[5])+0.3061862178478971*alphavpar[4]-0.1767766952966368*(alphavpar[3]+alphavpar[2])+0.1767766952966368*(alphavpar[1]+alphavpar[0])); 
  cflFreq += 0.5*(alphaR+std::abs(alphaR)); 
  alphaR = 0.03125*((-0.3061862178478971*alphavpar[29])+0.3061862178478971*(alphavpar[26]+alphavpar[25]+alphavpar[23])-0.1767766952966368*alphavpar[21]-0.3061862178478971*alphavpar[19]+0.3061862178478971*alphavpar[18]-0.3061862178478971*alphavpar[17]+0.1767766952966368*alphavpar[16]-0.3061862178478971*alphavpar[15]+0.1767766952966368*(alphavpar[14]+alphavpar[12])-0.3061862178478971*alphavpar[11]+0.3061862178478971*alphavpar[10]-0.3061862178478971*alphavpar[9]-0.1767766952966368*alphavpar[8]+0.1767766952966368*alphavpar[7]-0.1767766952966368*(alphavpar[6]+alphavpar[5])+0.3061862178478971*alphavpar[4]-0.1767766952966368*alphavpar[3]+0.1767766952966368*alphavpar[2]-0.1767766952966368*alphavpar[1]+0.1767766952966368*alphavpar[0]); 
  cflFreq += 0.5*(alphaR+std::abs(alphaR)); 
  alphaR = 0.03125*(0.3061862178478971*alphavpar[29]-0.3061862178478971*alphavpar[26]+0.3061862178478971*alphavpar[25]-0.3061862178478971*alphavpar[23]+0.1767766952966368*alphavpar[21]-0.3061862178478971*(alphavpar[19]+alphavpar[18])+0.3061862178478971*alphavpar[17]-0.1767766952966368*alphavpar[16]-0.3061862178478971*alphavpar[15]+0.1767766952966368*alphavpar[14]-0.1767766952966368*alphavpar[12]-0.3061862178478971*alphavpar[11]+0.3061862178478971*(alphavpar[10]+alphavpar[9])-0.1767766952966368*(alphavpar[8]+alphavpar[7])+0.1767766952966368*alphavpar[6]-0.1767766952966368*alphavpar[5]+0.3061862178478971*alphavpar[4]-0.1767766952966368*alphavpar[3]+0.1767766952966368*(alphavpar[2]+alphavpar[1]+alphavpar[0])); 
  cflFreq += 0.5*(alphaR+std::abs(alphaR)); 
  alphaR = 0.03125*(0.3061862178478971*(alphavpar[29]+alphavpar[26])-0.3061862178478971*alphavpar[25]+0.3061862178478971*alphavpar[23]+0.1767766952966368*alphavpar[21]-0.3061862178478971*(alphavpar[19]+alphavpar[18])+0.3061862178478971*alphavpar[17]+0.1767766952966368*alphavpar[16]-0.3061862178478971*alphavpar[15]-0.1767766952966368*alphavpar[14]+0.1767766952966368*alphavpar[12]+0.3061862178478971*alphavpar[11]-0.3061862178478971*(alphavpar[10]+alphavpar[9])-0.1767766952966368*(alphavpar[8]+alphavpar[7])+0.1767766952966368*alphavpar[6]-0.1767766952966368*alphavpar[5]+0.3061862178478971*alphavpar[4]+0.1767766952966368*alphavpar[3]-0.1767766952966368*(alphavpar[2]+alphavpar[1])+0.1767766952966368*alphavpar[0]); 
  cflFreq += 0.5*(alphaR+std::abs(alphaR)); 
  alphaR = 0.03125*((-0.3061862178478971*(alphavpar[29]+alphavpar[26]+alphavpar[25]+alphavpar[23]))-0.1767766952966368*alphavpar[21]-0.3061862178478971*alphavpar[19]+0.3061862178478971*alphavpar[18]-0.3061862178478971*alphavpar[17]-0.1767766952966368*alphavpar[16]-0.3061862178478971*alphavpar[15]-0.1767766952966368*(alphavpar[14]+alphavpar[12])+0.3061862178478971*alphavpar[11]-0.3061862178478971*alphavpar[10]+0.3061862178478971*alphavpar[9]-0.1767766952966368*alphavpar[8]+0.1767766952966368*alphavpar[7]-0.1767766952966368*(alphavpar[6]+alphavpar[5])+0.3061862178478971*alphavpar[4]+0.1767766952966368*alphavpar[3]-0.1767766952966368*alphavpar[2]+0.1767766952966368*(alphavpar[1]+alphavpar[0])); 
  cflFreq += 0.5*(alphaR+std::abs(alphaR)); 
  alphaR = 0.03125*(0.3061862178478971*alphavpar[29]-0.3061862178478971*(alphavpar[26]+alphavpar[25])+0.3061862178478971*alphavpar[23]+0.1767766952966368*alphavpar[21]+0.3061862178478971*alphavpar[19]-0.3061862178478971*(alphavpar[18]+alphavpar[17])-0.1767766952966368*alphavpar[16]-0.3061862178478971*alphavpar[15]-0.1767766952966368*alphavpar[14]+0.1767766952966368*alphavpar[12]+0.3061862178478971*(alphavpar[11]+alphavpar[10])-0.3061862178478971*alphavpar[9]+0.1767766952966368*alphavpar[8]-0.1767766952966368*(alphavpar[7]+alphavpar[6]+alphavpar[5])+0.3061862178478971*alphavpar[4]+0.1767766952966368*(alphavpar[3]+alphavpar[2])-0.1767766952966368*alphavpar[1]+0.1767766952966368*alphavpar[0]); 
  cflFreq += 0.5*(alphaR+std::abs(alphaR)); 
  alphaR = 0.03125*((-0.3061862178478971*alphavpar[29])+0.3061862178478971*alphavpar[26]-0.3061862178478971*(alphavpar[25]+alphavpar[23])-0.1767766952966368*alphavpar[21]+0.3061862178478971*(alphavpar[19]+alphavpar[18]+alphavpar[17])+0.1767766952966368*alphavpar[16]-0.3061862178478971*alphavpar[15]-0.1767766952966368*(alphavpar[14]+alphavpar[12])+0.3061862178478971*(alphavpar[11]+alphavpar[10]+alphavpar[9])+0.1767766952966368*(alphavpar[8]+alphavpar[7]+alphavpar[6])-0.1767766952966368*alphavpar[5]+0.3061862178478971*alphavpar[4]+0.1767766952966368*(alphavpar[3]+alphavpar[2]+alphavpar[1]+alphavpar[0])); 
  cflFreq += 0.5*(alphaR+std::abs(alphaR)); 
  alphaR = 0.03125*(0.3061862178478971*alphavpar[29]-0.3061862178478971*(alphavpar[26]+alphavpar[25]+alphavpar[23])+0.1767766952966368*alphavpar[21]+0.3061862178478971*(alphavpar[19]+alphavpar[18]+alphavpar[17])-0.1767766952966368*alphavpar[16]+0.3061862178478971*alphavpar[15]-0.1767766952966368*(alphavpar[14]+alphavpar[12])-0.3061862178478971*(alphavpar[11]+alphavpar[10]+alphavpar[9])+0.1767766952966368*(alphavpar[8]+alphavpar[7]+alphavpar[6]+alphavpar[5])+0.3061862178478971*alphavpar[4]-0.1767766952966368*(alphavpar[3]+alphavpar[2]+alphavpar[1])+0.1767766952966368*alphavpar[0]); 
  cflFreq += 0.5*(alphaR+std::abs(alphaR)); 
  alphaR = 0.03125*((-0.3061862178478971*alphavpar[29])+0.3061862178478971*alphavpar[26]-0.3061862178478971*alphavpar[25]+0.3061862178478971*alphavpar[23]-0.1767766952966368*alphavpar[21]+0.3061862178478971*alphavpar[19]-0.3061862178478971*(alphavpar[18]+alphavpar[17])+0.1767766952966368*alphavpar[16]+0.3061862178478971*alphavpar[15]-0.1767766952966368*alphavpar[14]+0.1767766952966368*alphavpar[12]-0.3061862178478971*(alphavpar[11]+alphavpar[10])+0.3061862178478971*alphavpar[9]+0.1767766952966368*alphavpar[8]-0.1767766952966368*(alphavpar[7]+alphavpar[6])+0.1767766952966368*alphavpar[5]+0.3061862178478971*alphavpar[4]-0.1767766952966368*(alphavpar[3]+alphavpar[2])+0.1767766952966368*(alphavpar[1]+alphavpar[0])); 
  cflFreq += 0.5*(alphaR+std::abs(alphaR)); 
  alphaR = 0.03125*(0.3061862178478971*(alphavpar[29]+alphavpar[26])-0.3061862178478971*(alphavpar[25]+alphavpar[23])+0.1767766952966368*alphavpar[21]-0.3061862178478971*alphavpar[19]+0.3061862178478971*alphavpar[18]-0.3061862178478971*alphavpar[17]+0.1767766952966368*alphavpar[16]+0.3061862178478971*alphavpar[15]-0.1767766952966368*(alphavpar[14]+alphavpar[12])-0.3061862178478971*alphavpar[11]+0.3061862178478971*alphavpar[10]-0.3061862178478971*alphavpar[9]-0.1767766952966368*alphavpar[8]+0.1767766952966368*alphavpar[7]-0.1767766952966368*alphavpar[6]+0.1767766952966368*alphavpar[5]+0.3061862178478971*alphavpar[4]-0.1767766952966368*alphavpar[3]+0.1767766952966368*alphavpar[2]-0.1767766952966368*alphavpar[1]+0.1767766952966368*alphavpar[0]); 
  cflFreq += 0.5*(alphaR+std::abs(alphaR)); 
  alphaR = 0.03125*((-0.3061862178478971*(alphavpar[29]+alphavpar[26]+alphavpar[25]))+0.3061862178478971*alphavpar[23]-0.1767766952966368*alphavpar[21]-0.3061862178478971*(alphavpar[19]+alphavpar[18])+0.3061862178478971*alphavpar[17]-0.1767766952966368*alphavpar[16]+0.3061862178478971*alphavpar[15]-0.1767766952966368*alphavpar[14]+0.1767766952966368*alphavpar[12]-0.3061862178478971*alphavpar[11]+0.3061862178478971*(alphavpar[10]+alphavpar[9])-0.1767766952966368*(alphavpar[8]+alphavpar[7])+0.1767766952966368*(alphavpar[6]+alphavpar[5])+0.3061862178478971*alphavpar[4]-0.1767766952966368*alphavpar[3]+0.1767766952966368*(alphavpar[2]+alphavpar[1]+alphavpar[0])); 
  cflFreq += 0.5*(alphaR+std::abs(alphaR)); 
  alphaR = 0.03125*((-0.3061862178478971*alphavpar[29])+0.3061862178478971*(alphavpar[26]+alphavpar[25])-0.3061862178478971*alphavpar[23]-0.1767766952966368*alphavpar[21]-0.3061862178478971*(alphavpar[19]+alphavpar[18])+0.3061862178478971*alphavpar[17]+0.1767766952966368*alphavpar[16]+0.3061862178478971*alphavpar[15]+0.1767766952966368*alphavpar[14]-0.1767766952966368*alphavpar[12]+0.3061862178478971*alphavpar[11]-0.3061862178478971*(alphavpar[10]+alphavpar[9])-0.1767766952966368*(alphavpar[8]+alphavpar[7])+0.1767766952966368*(alphavpar[6]+alphavpar[5])+0.3061862178478971*alphavpar[4]+0.1767766952966368*alphavpar[3]-0.1767766952966368*(alphavpar[2]+alphavpar[1])+0.1767766952966368*alphavpar[0]); 
  cflFreq += 0.5*(alphaR+std::abs(alphaR)); 
  alphaR = 0.03125*(0.3061862178478971*alphavpar[29]-0.3061862178478971*alphavpar[26]+0.3061862178478971*(alphavpar[25]+alphavpar[23])+0.1767766952966368*alphavpar[21]-0.3061862178478971*alphavpar[19]+0.3061862178478971*alphavpar[18]-0.3061862178478971*alphavpar[17]-0.1767766952966368*alphavpar[16]+0.3061862178478971*alphavpar[15]+0.1767766952966368*(alphavpar[14]+alphavpar[12])+0.3061862178478971*alphavpar[11]-0.3061862178478971*alphavpar[10]+0.3061862178478971*alphavpar[9]-0.1767766952966368*alphavpar[8]+0.1767766952966368*alphavpar[7]-0.1767766952966368*alphavpar[6]+0.1767766952966368*alphavpar[5]+0.3061862178478971*alphavpar[4]+0.1767766952966368*alphavpar[3]-0.1767766952966368*alphavpar[2]+0.1767766952966368*(alphavpar[1]+alphavpar[0])); 
  cflFreq += 0.5*(alphaR+std::abs(alphaR)); 
  alphaR = 0.03125*((-0.3061862178478971*(alphavpar[29]+alphavpar[26]))+0.3061862178478971*alphavpar[25]-0.3061862178478971*alphavpar[23]-0.1767766952966368*alphavpar[21]+0.3061862178478971*alphavpar[19]-0.3061862178478971*(alphavpar[18]+alphavpar[17])-0.1767766952966368*alphavpar[16]+0.3061862178478971*alphavpar[15]+0.1767766952966368*alphavpar[14]-0.1767766952966368*alphavpar[12]+0.3061862178478971*(alphavpar[11]+alphavpar[10])-0.3061862178478971*alphavpar[9]+0.1767766952966368*alphavpar[8]-0.1767766952966368*(alphavpar[7]+alphavpar[6])+0.1767766952966368*alphavpar[5]+0.3061862178478971*alphavpar[4]+0.1767766952966368*(alphavpar[3]+alphavpar[2])-0.1767766952966368*alphavpar[1]+0.1767766952966368*alphavpar[0]); 
  cflFreq += 0.5*(alphaR+std::abs(alphaR)); 
  alphaR = 0.03125*(0.3061862178478971*(alphavpar[29]+alphavpar[26]+alphavpar[25]+alphavpar[23])+0.1767766952966368*alphavpar[21]+0.3061862178478971*(alphavpar[19]+alphavpar[18]+alphavpar[17])+0.1767766952966368*alphavpar[16]+0.3061862178478971*alphavpar[15]+0.1767766952966368*(alphavpar[14]+alphavpar[12])+0.3061862178478971*(alphavpar[11]+alphavpar[10]+alphavpar[9])+0.1767766952966368*(alphavpar[8]+alphavpar[7]+alphavpar[6]+alphavpar[5])+0.3061862178478971*alphavpar[4]+0.1767766952966368*(alphavpar[3]+alphavpar[2]+alphavpar[1]+alphavpar[0])); 
  cflFreq += 0.5*(alphaR+std::abs(alphaR)); 
#endif 
  ...
  out[4] += 0.3061862178478971*(alphavpar[29]*f[29]+alphavpar[26]*f[26]+alphavpar[25]*f[25]+alphavpar[23]*f[23]+alphavpar[21]*f[21]+alphavpar[19]*f[19]+alphavpar[18]*f[18]+alphavpar[17]*f[17]+alphavpar[16]*f[16]+alphavpar[15]*f[15]+alphavpar[14]*f[14]+alphavpar[12]*f[12]+alphavpar[11]*f[11]+alphavpar[10]*f[10]+alphavpar[9]*f[9]+alphavpar[8]*f[8]+alphavpar[7]*f[7]+alphavpar[6]*f[6]+alphavpar[5]*f[5]+alphavpar[4]*f[4]+alphavpar[3]*f[3]+alphavpar[2]*f[2]+alphavpar[1]*f[1]+alphavpar[0]*f[0]); 
  ...
  out[9] += 0.3061862178478971*((alphavpar[25]+alphax[21])*f[29]+f[25]*alphavpar[29]+(alphavpar[19]+alphax[16])*f[26]+f[19]*alphavpar[26]+alphax[14]*f[25]+(alphavpar[15]+alphax[12])*f[23]+f[15]*alphavpar[23]+alphavpar[14]*f[21]+f[14]*alphavpar[21]+alphax[8]*f[19]+(alphavpar[11]+alphax[7])*f[18]+f[7]*alphax[18]+f[11]*alphavpar[18]+(alphavpar[10]+alphax[6])*f[17]+f[10]*alphavpar[17]+alphavpar[8]*f[16]+f[8]*alphavpar[16]+alphax[5]*f[15]+alphavpar[5]*f[12]+f[5]*alphavpar[12]+alphax[3]*f[11]+f[3]*alphax[11]+alphax[2]*f[10]+(alphavpar[4]+alphax[1])*f[9]+f[1]*alphax[9]+f[4]*alphavpar[9]+alphavpar[3]*f[7]+f[3]*alphavpar[7]+alphavpar[2]*f[6]+f[2]*alphavpar[6]+alphax[0]*f[4]+f[0]*alphax[4]+alphavpar[0]*f[1]+f[0]*alphavpar[1]); 
  out[10] += 0.3061862178478971*(alphavpar[29]*f[31]+alphavpar[25]*f[30]+alphay[21]*f[29]+alphavpar[23]*f[28]+alphavpar[21]*f[27]+(alphavpar[18]+alphay[16])*f[26]+f[18]*alphavpar[26]+alphay[14]*f[25]+alphavpar[15]*f[24]+alphay[12]*f[23]+alphavpar[14]*f[22]+alphavpar[12]*f[20]+(alphavpar[11]+alphay[8])*f[19]+f[11]*alphavpar[19]+alphay[7]*f[18]+f[7]*alphay[18]+(alphavpar[9]+alphay[6])*f[17]+f[9]*alphavpar[17]+alphavpar[7]*f[16]+f[7]*alphavpar[16]+alphay[5]*f[15]+alphavpar[5]*f[13]+alphay[3]*f[11]+f[3]*alphay[11]+(alphavpar[4]+alphay[2])*f[10]+f[4]*alphavpar[10]+alphay[1]*f[9]+f[1]*alphay[9]+alphavpar[3]*f[8]+f[3]*alphavpar[8]+alphavpar[1]*f[6]+f[1]*alphavpar[6]+alphay[0]*f[4]+f[0]*alphay[4]+alphavpar[0]*f[2]+f[0]*alphavpar[2]); 
  out[11] += 0.3061862178478971*((alphavpar[23]+alphaz[21])*f[29]+f[23]*alphavpar[29]+(alphavpar[17]+alphaz[16])*f[26]+f[17]*alphavpar[26]+(alphavpar[15]+alphaz[14])*f[25]+f[15]*alphavpar[25]+alphaz[12]*f[23]+alphavpar[12]*f[21]+f[12]*alphavpar[21]+(alphavpar[10]+alphaz[8])*f[19]+f[10]*alphavpar[19]+(alphavpar[9]+alphaz[7])*f[18]+f[7]*alphaz[18]+f[9]*alphavpar[18]+alphaz[6]*f[17]+alphavpar[6]*f[16]+f[6]*alphavpar[16]+alphaz[5]*f[15]+alphavpar[5]*f[14]+f[5]*alphavpar[14]+(alphavpar[4]+alphaz[3])*f[11]+f[3]*alphaz[11]+f[4]*alphavpar[11]+alphaz[2]*f[10]+alphaz[1]*f[9]+f[1]*alphaz[9]+alphavpar[2]*f[8]+f[2]*alphavpar[8]+alphavpar[1]*f[7]+f[1]*alphavpar[7]+alphaz[0]*f[4]+f[0]*alphaz[4]+alphavpar[0]*f[3]+f[0]*alphavpar[3]); 
  ...
  out[15] += 0.3061862178478971*(alphavpar[26]*f[31]+alphavpar[19]*f[30]+alphavpar[18]*f[29]+f[18]*alphavpar[29]+alphavpar[17]*f[28]+alphavpar[16]*f[27]+alphavpar[11]*f[25]+f[11]*alphavpar[25]+alphavpar[10]*f[24]+alphavpar[9]*f[23]+f[9]*alphavpar[23]+alphavpar[8]*f[22]+alphavpar[7]*f[21]+f[7]*alphavpar[21]+alphavpar[6]*f[20]+alphavpar[4]*f[15]+f[4]*alphavpar[15]+alphavpar[3]*f[14]+f[3]*alphavpar[14]+alphavpar[2]*f[13]+alphavpar[1]*f[12]+f[1]*alphavpar[12]+alphavpar[0]*f[5]+f[0]*alphavpar[5]); 
  ...
  out[23] += 0.3061862178478971*((alphavpar[19]+alphax[16])*f[31]+(alphavpar[26]+alphax[8])*f[30]+(alphavpar[11]+alphax[7])*f[29]+f[11]*alphavpar[29]+(alphavpar[10]+alphax[6])*f[28]+alphavpar[8]*f[27]+(alphavpar[18]+alphax[3])*f[25]+f[18]*alphavpar[25]+(alphavpar[17]+alphax[2])*f[24]+(alphavpar[4]+alphax[1])*f[23]+f[4]*alphavpar[23]+alphavpar[16]*f[22]+(alphax[18]+alphavpar[3])*f[21]+f[18]*alphax[21]+f[3]*alphavpar[21]+alphavpar[2]*f[20]+(alphavpar[9]+alphax[0])*f[15]+f[9]*alphavpar[15]+(alphax[11]+alphavpar[7])*f[14]+f[11]*alphax[14]+f[7]*alphavpar[14]+alphavpar[6]*f[13]+(alphax[9]+alphavpar[0])*f[12]+f[9]*alphax[12]+f[0]*alphavpar[12]+(alphax[4]+alphavpar[1])*f[5]+f[4]*alphax[5]+f[1]*alphavpar[5]); 
  out[24] += 0.3061862178478971*((alphavpar[18]+alphay[16])*f[31]+(alphavpar[11]+alphay[8])*f[30]+(alphavpar[26]+alphay[7])*f[29]+f[26]*alphavpar[29]+(alphavpar[9]+alphay[6])*f[28]+alphavpar[7]*f[27]+(alphavpar[19]+alphay[3])*f[25]+f[19]*alphavpar[25]+(alphavpar[4]+alphay[2])*f[24]+(alphavpar[17]+alphay[1])*f[23]+f[17]*alphavpar[23]+alphavpar[3]*f[22]+(alphay[18]+alphavpar[16])*f[21]+f[18]*alphay[21]+f[16]*alphavpar[21]+alphavpar[1]*f[20]+(alphavpar[10]+alphay[0])*f[15]+f[10]*alphavpar[15]+(alphay[11]+alphavpar[8])*f[14]+f[11]*alphay[14]+f[8]*alphavpar[14]+alphavpar[0]*f[13]+(alphay[9]+alphavpar[6])*f[12]+f[9]*alphay[12]+f[6]*alphavpar[12]+(alphay[4]+alphavpar[2])*f[5]+f[4]*alphay[5]+f[2]*alphavpar[5]); 
  out[25] += 0.3061862178478971*((alphavpar[17]+alphaz[16])*f[31]+(alphavpar[10]+alphaz[8])*f[30]+(alphavpar[9]+alphaz[7])*f[29]+f[9]*alphavpar[29]+(alphavpar[26]+alphaz[6])*f[28]+alphavpar[6]*f[27]+(alphavpar[4]+alphaz[3])*f[25]+f[4]*alphavpar[25]+(alphavpar[19]+alphaz[2])*f[24]+(alphavpar[18]+alphaz[1])*f[23]+f[18]*alphavpar[23]+alphavpar[2]*f[22]+(alphaz[18]+alphavpar[1])*f[21]+f[18]*alphaz[21]+f[1]*alphavpar[21]+alphavpar[16]*f[20]+(alphavpar[11]+alphaz[0])*f[15]+f[11]*alphavpar[15]+(alphaz[11]+alphavpar[0])*f[14]+f[11]*alphaz[14]+f[0]*alphavpar[14]+alphavpar[8]*f[13]+(alphaz[9]+alphavpar[7])*f[12]+f[9]*alphaz[12]+f[7]*alphavpar[12]+(alphaz[4]+alphavpar[3])*f[5]+f[4]*alphaz[5]+f[3]*alphavpar[5]); 
  out[26] += 0.3061862178478971*((alphavpar[15]+alphaz[14]+alphax[12])*f[31]+(alphavpar[23]+alphaz[21]+alphax[5])*f[30]+alphay[5]*f[29]+f[24]*alphavpar[29]+(alphavpar[25]+alphax[21]+alphaz[5])*f[28]+alphavpar[5]*f[27]+(alphavpar[4]+alphaz[3]+alphay[2]+alphax[1])*f[26]+f[4]*alphavpar[26]+alphay[12]*f[25]+(alphax[14]+alphaz[12])*f[24]+alphay[14]*f[23]+alphavpar[12]*f[22]+f[15]*alphay[21]+f[13]*alphavpar[21]+alphavpar[14]*f[20]+(alphavpar[9]+alphaz[7]+alphay[6]+alphax[0])*f[19]+f[9]*alphavpar[19]+(alphavpar[10]+alphaz[8]+alphax[6]+alphay[0])*f[18]+f[8]*alphaz[18]+f[0]*alphay[18]+f[6]*alphax[18]+f[10]*alphavpar[18]+(alphavpar[11]+alphay[8]+alphax[7]+alphaz[0])*f[17]+f[11]*alphavpar[17]+(alphaz[11]+alphax[9]+alphavpar[0])*f[16]+f[11]*alphaz[16]+f[10]*alphay[16]+f[9]*alphax[16]+f[0]*alphavpar[16]+(alphax[2]+alphay[1])*f[11]+f[1]*alphay[11]+f[2]*alphax[11]+(alphax[3]+alphaz[1])*f[10]+(alphay[3]+alphaz[2])*f[9]+f[2]*alphaz[9]+f[3]*alphay[9]+(alphax[4]+alphavpar[1])*f[8]+f[4]*alphax[8]+f[1]*alphavpar[8]+(alphay[4]+alphavpar[2])*f[7]+f[4]*alphay[7]+f[2]*alphavpar[7]+(alphaz[4]+alphavpar[3])*f[6]+f[4]*alphaz[6]+f[3]*alphavpar[6]); 
  ...
  out[28] += 0.3061862178478971*((alphavpar[11]+alphay[8]+alphax[7])*f[31]+(alphavpar[18]+alphay[16]+alphax[3])*f[30]+(alphavpar[19]+alphax[16]+alphay[3])*f[29]+f[19]*alphavpar[29]+(alphavpar[4]+alphay[2]+alphax[1])*f[28]+(alphax[18]+alphavpar[3])*f[27]+(alphavpar[25]+alphax[21])*f[26]+f[25]*(alphavpar[26]+alphax[8]+alphay[7])+(alphavpar[9]+alphay[6]+alphax[0])*f[24]+(alphavpar[10]+alphax[6]+alphay[0])*f[23]+f[10]*alphavpar[23]+(alphax[11]+alphavpar[7])*f[22]+(alphay[11]+alphavpar[8])*f[21]+f[11]*alphay[21]+f[8]*alphavpar[21]+(alphax[9]+alphavpar[0])*f[20]+alphax[14]*f[19]+alphay[14]*f[18]+f[14]*alphay[18]+(alphavpar[15]+alphax[12])*f[17]+f[15]*alphavpar[17]+alphavpar[14]*f[16]+f[14]*alphavpar[16]+(alphax[2]+alphay[1])*f[15]+(alphax[4]+alphavpar[1])*f[13]+(alphay[4]+alphavpar[2])*f[12]+f[4]*alphay[12]+f[2]*alphavpar[12]+alphax[5]*f[10]+alphay[5]*f[9]+f[5]*alphay[9]+alphavpar[5]*f[6]+f[5]*alphavpar[6]); 
  out[29] += 0.3061862178478971*((alphavpar[10]+alphaz[8]+alphax[6])*f[31]+(alphavpar[17]+alphaz[16]+alphax[2])*f[30]+(alphavpar[4]+alphaz[3]+alphax[1])*f[29]+f[4]*alphavpar[29]+(alphavpar[19]+alphax[16]+alphaz[2])*f[28]+alphavpar[2]*f[27]+f[24]*alphavpar[26]+(alphavpar[9]+alphaz[7]+alphax[0])*f[25]+f[9]*alphavpar[25]+(alphax[8]+alphaz[6])*f[24]+(alphavpar[11]+alphax[7]+alphaz[0])*f[23]+f[11]*alphavpar[23]+alphavpar[6]*f[22]+(alphaz[11]+alphax[9]+alphavpar[0])*f[21]+f[11]*alphaz[21]+f[9]*alphax[21]+f[0]*alphavpar[21]+alphavpar[8]*f[20]+(alphavpar[15]+alphaz[14]+alphax[12])*f[18]+f[14]*alphaz[18]+f[12]*alphax[18]+f[15]*alphavpar[18]+f[13]*alphavpar[16]+(alphax[3]+alphaz[1])*f[15]+(alphax[4]+alphavpar[1])*f[14]+f[4]*alphax[14]+f[1]*alphavpar[14]+(alphaz[4]+alphavpar[3])*f[12]+f[4]*alphaz[12]+f[3]*alphavpar[12]+alphax[5]*f[11]+f[5]*alphax[11]+alphaz[5]*f[9]+f[5]*alphaz[9]+alphavpar[5]*f[7]+f[5]*alphavpar[7]); 
  out[30] += 0.3061862178478971*((alphavpar[9]+alphaz[7]+alphay[6])*f[31]+(alphavpar[4]+alphaz[3]+alphay[2])*f[30]+(alphavpar[17]+alphaz[16]+alphay[1])*f[29]+f[17]*alphavpar[29]+(alphavpar[18]+alphay[16]+alphaz[1])*f[28]+(alphaz[18]+alphavpar[1])*f[27]+(alphavpar[23]+alphaz[21])*f[26]+f[23]*alphavpar[26]+(alphavpar[10]+alphaz[8]+alphay[0])*f[25]+f[10]*alphavpar[25]+(alphavpar[11]+alphay[8]+alphaz[0])*f[24]+(alphay[7]+alphaz[6])*f[23]+(alphaz[11]+alphavpar[0])*f[22]+(alphay[9]+alphavpar[6])*f[21]+f[9]*alphay[21]+f[6]*alphavpar[21]+(alphaz[9]+alphavpar[7])*f[20]+(alphavpar[15]+alphaz[14])*f[19]+f[15]*alphavpar[19]+alphay[12]*f[18]+f[12]*alphay[18]+alphaz[12]*f[17]+alphavpar[12]*f[16]+f[12]*alphavpar[16]+(alphay[3]+alphaz[2])*f[15]+(alphay[4]+alphavpar[2])*f[14]+f[4]*alphay[14]+f[2]*alphavpar[14]+(alphaz[4]+alphavpar[3])*f[13]+alphay[5]*f[11]+f[5]*alphay[11]+alphaz[5]*f[10]+alphavpar[5]*f[8]+f[5]*alphavpar[8]); 
  out[31] += 0.3061862178478971*((alphavpar[4]+alphaz[3]+alphay[2]+alphax[1])*f[31]+(alphavpar[9]+alphaz[7]+alphay[6]+alphax[0])*f[30]+(alphavpar[10]+alphaz[8]+alphax[6]+alphay[0])*f[29]+f[10]*alphavpar[29]+(alphavpar[11]+alphay[8]+alphax[7]+alphaz[0])*f[28]+(alphaz[11]+alphax[9]+alphavpar[0])*f[27]+(alphavpar[15]+alphaz[14]+alphax[12])*f[26]+f[15]*alphavpar[26]+(alphavpar[17]+alphaz[16]+alphax[2]+alphay[1])*f[25]+f[17]*alphavpar[25]+(alphavpar[18]+alphay[16]+alphax[3]+alphaz[1])*f[24]+(alphavpar[19]+alphax[16]+alphay[3]+alphaz[2])*f[23]+f[19]*alphavpar[23]+(alphaz[18]+alphax[4]+alphavpar[1])*f[22]+(alphay[4]+alphavpar[2])*f[21]+f[19]*alphaz[21]+f[4]*alphay[21]+f[17]*alphax[21]+f[2]*alphavpar[21]+(alphax[18]+alphaz[4]+alphavpar[3])*f[20]+alphax[5]*f[19]+alphay[5]*f[18]+f[5]*alphay[18]+alphaz[5]*f[17]+alphavpar[5]*f[16]+f[5]*alphavpar[16]+(alphax[8]+alphay[7]+alphaz[6])*f[15]+(alphay[9]+alphavpar[6])*f[14]+f[9]*alphay[14]+f[10]*alphax[14]+f[6]*alphavpar[14]+(alphax[11]+alphaz[9]+alphavpar[7])*f[13]+(alphay[11]+alphavpar[8])*f[12]+f[10]*alphaz[12]+f[11]*alphay[12]+f[8]*alphavpar[12]); 
  return cflFreq; 
} 
