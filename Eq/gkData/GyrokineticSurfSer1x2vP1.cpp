...
// w[NDIM]: Cell-center coordinates. dxv[NDIM]: Cell spacing. H/f: Input Hamiltonian/distribution function. out: Incremented output 
...
#if upwindType == SURFAVG 
  if (alpha0>0) { 
  incr[0] = 0.25*alpha[0]*(2.23606797749979*fl[8]+1.732050807568877*fl[2]+fl[0])*dfac_v; 
  incr[1] = 0.25*alpha[0]*(1.732050807568877*fl[4]+fl[1])*dfac_v; 
  incr[2] = -0.25*alpha[0]*(3.872983346207417*fl[8]+3.0*fl[2]+1.732050807568877*fl[0])*dfac_v; 
  ...
  incr[8] = 0.25*alpha[0]*(5.0*fl[8]+3.872983346207417*fl[2]+2.23606797749979*fl[0])*dfac_v; 
  } else { 
  incr[0] = 0.25*alpha[0]*(2.23606797749979*fr[8]-1.732050807568877*fr[2]+fr[0])*dfac_v; 
  incr[1] = -0.25*alpha[0]*(1.732050807568877*fr[4]-1.0*fr[1])*dfac_v; 
  incr[2] = -0.25*alpha[0]*(3.872983346207417*fr[8]-3.0*fr[2]+1.732050807568877*fr[0])*dfac_v; 
  ...
  incr[8] = 0.25*alpha[0]*(5.0*fr[8]-3.872983346207417*fr[2]+2.23606797749979*fr[0])*dfac_v; 
  }
#elif upwindType == QUAD 
double fupwind[4];
...
double alphaQuad;
  ...
  incr[8] = 0.7905694150420947*alpha[0]*fupwind[0]*dfac_v; 
