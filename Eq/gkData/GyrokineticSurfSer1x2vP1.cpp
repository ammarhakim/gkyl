...
double alphaQuad;
  ...
  incr[2] = 0.3535533905932737*(alpha[0]*fupwind[1]+fupwind[0]*alpha[1])*dfac_x; 
  incr[3] = 0.3535533905932737*(alpha[1]*fupwind[3]+alpha[0]*fupwind[2])*dfac_x; 
  incr[4] = -0.6123724356957944*(alpha[0]*fupwind[1]+fupwind[0]*alpha[1])*dfac_x; 
...
// w[NDIM]: Cell-center coordinates. dxv[NDIM]: Cell spacing. H/f: Input Hamiltonian/distribution function. out: Incremented output 
  ...
  double alpha[4]; 
  alpha[0] = (0.5*dfac_x*(Bmag[1]*(3.0*(BmagInv[1]*geoY[1]+BmagInv[0]*geoY[0])*dfac_v*dfac_x*m_*(Bmag[1]*wm+Phi[1]*q_)*wv+((-3.464101615137754*Gradpar[0]*dfac_v*q_)-3.0*Bmag[1]*(BmagInv[1]*geoY[1]+BmagInv[0]*geoY[0])*dfac_x*m_)*wm)+Phi[1]*((-3.0*Bmag[1]*(BmagInv[1]*geoY[1]+BmagInv[0]*geoY[0])*dfac_x*m_*q_)-3.464101615137754*Gradpar[0]*dfac_v*q2)))/(dfac_v*m_*q_); 
  alpha[1] = (0.5*dfac_x*(Bmag[1]*(3.0*(BmagInv[0]*geoY[1]+geoY[0]*BmagInv[1])*dfac_v*dfac_x*m_*(Bmag[1]*wm+Phi[1]*q_)*wv+((-3.464101615137754*Gradpar[1]*dfac_v*q_)-3.0*Bmag[1]*(BmagInv[0]*geoY[1]+geoY[0]*BmagInv[1])*dfac_x*m_)*wm)+Phi[1]*((-3.0*Bmag[1]*(BmagInv[0]*geoY[1]+geoY[0]*BmagInv[1])*dfac_x*m_*q_)-3.464101615137754*Gradpar[1]*dfac_v*q2)))/(dfac_v*m_*q_); 
  alpha[2] = (0.5*Bmag[1]*dfac_x*(dfac_v*(1.732050807568877*Bmag[1]*(BmagInv[1]*geoY[1]+BmagInv[0]*geoY[0])*dfac_x*m_*wv-2.0*Gradpar[0]*q_)-1.732050807568877*Bmag[1]*(BmagInv[1]*geoY[1]+BmagInv[0]*geoY[0])*dfac_x*m_))/(dfac_m*dfac_v*m_*q_); 
  alpha[3] = (0.5*Bmag[1]*dfac_x*(dfac_v*(1.732050807568877*Bmag[1]*(BmagInv[0]*geoY[1]+geoY[0]*BmagInv[1])*dfac_x*m_*wv-2.0*Gradpar[1]*q_)-1.732050807568877*Bmag[1]*(BmagInv[0]*geoY[1]+geoY[0]*BmagInv[1])*dfac_x*m_))/(dfac_m*dfac_v*m_*q_); 
#if upwindType == SURFAVG 
  if (alpha0>0) { 
