...
double EmGyrokineticGenGeoVol1x2vSerP2_Bvars_0(const double q_, const double m_, const double *w, const double *dxv, const double *Bmag, const double *jacobTotInv, const double *Gradpar, const double *geoX, const double *geoY, const double *geoZ, const double *Phi, const double *Apar, const double* dApardt, const double *f, double *out) 
{ 
...
{ 
...
#elif cflType == QUAD 
  // evaluate alpha at left surface quadrature points 
  alphaL = -(0.125*(0.6324555320336759*dApardt[2]-0.9486832980505137*dApardt[1]+0.7071067811865475*dApardt[0])*dfac_v*q_)/m_; 
  cflFreq += -0.5*(alphaL-std::abs(alphaL)); 
  alphaL = -(0.125*(0.7071067811865475*dApardt[0]-0.7905694150420947*dApardt[2])*dfac_v*q_)/m_; 
  cflFreq += -0.5*(alphaL-std::abs(alphaL)); 
  alphaL = -(0.125*(0.6324555320336759*dApardt[2]+0.9486832980505137*dApardt[1]+0.7071067811865475*dApardt[0])*dfac_v*q_)/m_; 
  cflFreq += -0.5*(alphaL-std::abs(alphaL)); 
  alphaL = -(0.125*(0.6324555320336759*dApardt[2]-0.9486832980505137*dApardt[1]+0.7071067811865475*dApardt[0])*dfac_v*q_)/m_; 
  ...
  cflFreq += -0.5*(alphaL-std::abs(alphaL)); 
  // evaluate alpha at right surface quadrature points 
  alphaR = -(0.125*(0.6324555320336759*dApardt[2]-0.9486832980505137*dApardt[1]+0.7071067811865475*dApardt[0])*dfac_v*q_)/m_; 
  cflFreq += 0.5*(alphaR+std::abs(alphaR)); 
  alphaR = -(0.125*(0.7071067811865475*dApardt[0]-0.7905694150420947*dApardt[2])*dfac_v*q_)/m_; 
  cflFreq += 0.5*(alphaR+std::abs(alphaR)); 
  alphaR = -(0.125*(0.6324555320336759*dApardt[2]+0.9486832980505137*dApardt[1]+0.7071067811865475*dApardt[0])*dfac_v*q_)/m_; 
  cflFreq += 0.5*(alphaR+std::abs(alphaR)); 
  alphaR = -(0.125*(0.6324555320336759*dApardt[2]-0.9486832980505137*dApardt[1]+0.7071067811865475*dApardt[0])*dfac_v*q_)/m_; 
...
double EmGyrokineticGenGeoVol1x2vSerP2_Bvars_1(const double q_, const double m_, const double *w, const double *dxv, const double *Bmag, const double *jacobTotInv, const double *Gradpar, const double *geoX, const double *geoY, const double *geoZ, const double *Phi, const double *Apar, const double* dApardt, const double *f, double *out) 
{ 
// w[NDIM]: Cell-center coordinates. dxv[NDIM]: Cell spacing. H/f: Input Hamiltonian/distribution function. out: Incremented output 
  double dfac_z = 2.0/dxv[0]; 
  double dfac_v = 2.0/dxv[1]; 
  ...
  double BstarZ_by_Bmag[20]; 
  BstarZ_by_Bmag[0] = 1.414213562373095*(Gradpar[2]*jacobTotInv[2]+Gradpar[1]*jacobTotInv[1]+Gradpar[0]*jacobTotInv[0]); 
  BstarZ_by_Bmag[1] = 0.2828427124746191*(4.47213595499958*(Gradpar[1]*jacobTotInv[2]+jacobTotInv[1]*Gradpar[2])+5.0*(Gradpar[0]*jacobTotInv[1]+jacobTotInv[0]*Gradpar[1])); 
  BstarZ_by_Bmag[7] = 0.04040610178208843*((22.3606797749979*Gradpar[2]+35.0*Gradpar[0])*jacobTotInv[2]+7.0*(5.0*jacobTotInv[0]*Gradpar[2]+4.47213595499958*Gradpar[1]*jacobTotInv[1])); 
