...
function _M.selectVol(basisNm, CDIM, VDIM, polyOrder)
   local funcSign = "(const double q_, const double m_, const double *w, const double *dxv, const double *bmag, const double *bmagInv, const double *cmag, const double *b_x, const double *b_y, const double *b_z, const double *phi, const double *f, double *out)"
...
function _M.selectSurf(basisNm, CDIM, VDIM, polyOrder)
   ...
   if CDIM == 1 and VDIM <= 2 then
      funcNm[1] = string.format("GyrokineticSurf%dx%dv%s_x_P%d", CDIM, VDIM, basisNmMap[basisNm], polyOrder) .. bvarString
      funcNm[2] = string.format("GyrokineticSurf%dx%dv%s_vpar_P%d", CDIM, VDIM, basisNmMap[basisNm], polyOrder) .. bvarString
