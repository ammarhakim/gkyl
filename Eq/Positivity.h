...
double patchFit(double r, double x, double CFL) {
   ...
   if (x > 0) {
      if (r<2.2) {
         val = std::max(0, std::min(1.0/CFL, std::exp(2*r*x/3)*(1+r*x/3)));
      } else {
         val = std::min(1.0/CFL, 6/(3-std::min(2.999, std::abs(r))));
      }
   } else {
      if r>-2.2 then
         val = std::max(0, std::min(1.0/CFL, std::exp(2*r*x/3)*(1+r*x/3)));
...
#elif extraType == EXP0
   return std::max(0, std::min(1.0/CFL, std::exp(2*r*x/3)*(1+r*x/3)));
