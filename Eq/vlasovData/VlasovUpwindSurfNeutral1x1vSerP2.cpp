...
__host__ __device__ double VlasovUpwindSurfNeutral1x1vSer_VX_P2(const double *wl, const double *wr, const double *dxvl, const double *dxvr, const double amax, const double *EM, const double *fl, const double *fr, double *outl, double *outr) 
{ 
  ...
  incr[4] = 0.01428571428571429*((22.3606797749979*alpha[2]+35.0*alpha[0])*fUp[2]+35.0*fUp[0]*alpha[2]+31.30495168499706*alpha[1]*fUp[1]); 
  incr[5] = 1.118033988749895*(alpha[2]*fUp[2]+alpha[1]*fUp[1]+alpha[0]*fUp[0]); 
  incr[6] = -0.01428571428571429*((38.72983346207417*alpha[2]+60.62177826491071*alpha[0])*fUp[2]+60.62177826491071*fUp[0]*alpha[2]+54.22176684690384*alpha[1]*fUp[1]); 
  incr[7] = 0.03333333333333333*(30.0*(alpha[1]*fUp[2]+fUp[1]*alpha[2])+33.54101966249684*(alpha[0]*fUp[1]+fUp[0]*alpha[1])); 
