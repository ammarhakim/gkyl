...
function _M.selectUpwindSurfElcMag(basisNm, CDIM, VDIM, polyOrder)
   local funcType = "double"
   local funcNm = {}
   for d = 1, VDIM do
      funcNm[d] = string.format("VlasovUpwindSurfElcMag%dx%dv%s_%s_P%d", CDIM, VDIM, basisNmMap[basisNm], vvars[d], polyOrder)
   end
   local funcSign = "(const double *wl, const double *wr, const double *dxvl, const double *dxvr, const double amax, const double *E, const double *fl, const double *fr, double *outl, double *outr)"
