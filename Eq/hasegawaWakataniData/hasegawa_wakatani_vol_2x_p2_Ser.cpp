...
double hasegawa_wakatani_vol_2x_p2_Ser(const double C_, const double kappa_, const double *xc, const double *dx, const double *phi, const double *fIn, const double *phiZonal, const double *nZonal, double *out) 
{ 
  ...
  // phiZonal[8]: zonally averaged (along y) electrostatic potential.
  // nZonal[8]: zonally averaged (along y) density.
  ...
  out[0] += -1.0*(fIn[8]+1.414213562373095*phiZonal[0]-1.0*phi[0]-1.414213562373095*nZonal[0])*C_; 
  out[1] += -0.5*((2.0*fIn[9]+2.828427124746191*phiZonal[1]-2.0*phi[1]-2.828427124746191*nZonal[1])*C_-1.732050807568877*(alpha_x[4]*fIn[4]+alpha_x[3]*fIn[3]+alpha_x[2]*fIn[2]+alpha_x[1]*fIn[1]+alpha_x[0]*fIn[0])); 
  ...
  out[4] += -0.1*((10.0*fIn[12]-10.0*phi[4]+14.14213562373095*phiZonal[2]-14.14213562373095*nZonal[2])*C_-17.32050807568877*alpha_x[3]*fIn[6]-17.32050807568877*(alpha_x[1]*fIn[4]+fIn[1]*alpha_x[4])-19.36491673103709*(alpha_x[2]*fIn[3]+fIn[2]*alpha_x[3]+alpha_x[0]*fIn[1]+fIn[0]*alpha_x[1])); 
  ...
  out[8] += -1.0*(fIn[8]+1.414213562373095*phiZonal[0]-1.0*phi[0]-1.414213562373095*nZonal[0])*C_; 
  out[9] += -0.5*((dx[0]*alpha_x[1]+3.464101615137754*alpha_x[0]*xc[0])*kappa_+(2.0*fIn[9]+2.828427124746191*phiZonal[1]-2.0*phi[1]-2.828427124746191*nZonal[1])*C_-1.732050807568877*(alpha_x[4]*fIn[12]+alpha_x[3]*fIn[11]+alpha_x[2]*fIn[10]+alpha_x[1]*fIn[9]+alpha_x[0]*fIn[8])); 
  ...
  out[12] += -0.1*((10.0*dx[0]*alpha_x[4]+38.72983346207418*xc[0]*alpha_x[1]+11.18033988749895*alpha_x[0]*dx[0])*kappa_+(10.0*fIn[12]-10.0*phi[4]+14.14213562373095*phiZonal[2]-14.14213562373095*nZonal[2])*C_-17.32050807568877*alpha_x[3]*fIn[14]-17.32050807568877*alpha_x[1]*fIn[12]-19.36491673103709*(alpha_x[2]*fIn[11]+alpha_x[3]*fIn[10])-17.32050807568877*alpha_x[4]*fIn[9]-19.36491673103709*(alpha_x[0]*fIn[9]+alpha_x[1]*fIn[8])); 
