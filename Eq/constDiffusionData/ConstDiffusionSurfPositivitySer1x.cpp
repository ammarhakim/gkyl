...
{ 
  ...
  if ((-0.408248290463863*fr[1])+0.408248290463863*fl[1]+0.3535533905932737*fr[0]+0.3535533905932737*fl[0] >= 0.0) {
    ...
    if (std::abs(g1) > 1.0e-15) {
      double g1Sq = g1*g1;
      incr1[0] = (-(0.25*fr[0]*g1Sq)/std::sinh(g1))-(0.25*fl[0]*g1Sq)/std::sinh(g1); 
      incr1[1] = (0.4330127018922193*fr[0]*g1Sq)/std::sinh(g1)+(0.4330127018922193*fl[0]*g1Sq)/std::sinh(g1); 
