...
template <int CDIM, int VDIM, int POLYORDER, int BASISTYPE>
__global__ void d_calcM2(GkylCartField_t *fIn, GkylCartField_t *out) {
  // In computing moments we will first assign whole configuration-space cells to a single block.
  // Then one must perform a reduction across a block for each conf-space basis coefficient.
  ...
  MomentModDecl<CDIM, VDIM, POLYORDER, BASISTYPE>::calcM2(cellxc, cellSize, distF, localSumPtr);
...
template <int CDIM, int VDIM, int POLYORDER, int BASISTYPE>
__global__ void d_calcM2ij(GkylCartField_t *fIn, GkylCartField_t *out) {
  // In computing moments we will first assign whole configuration-space cells to a single block.
  // Then one must perform a reduction across a block for each conf-space basis coefficient.
  ...
  MomentModDecl<CDIM, VDIM, POLYORDER, BASISTYPE>::calcM2ij(cellxc, cellSize, distF, localSumPtr);
...
template <int CDIM, int VDIM, int POLYORDER, int BASISTYPE>
__global__ void d_calcM3i(GkylCartField_t *fIn, GkylCartField_t *out) {
  // In computing moments we will first assign whole configuration-space cells to a single block.
  // Then one must perform a reduction across a block for each conf-space basis coefficient.
  ...
  MomentModDecl<CDIM, VDIM, POLYORDER, BASISTYPE>::calcM3i(cellxc, cellSize, distF, localSumPtr);
