...
#include <cstdio>
 ...
 __global__ void d_calcMom1x1vSer_M1i_P1(GkylCartField_t *fIn, GkylCartField_t *out) {
  // In computing moments we will first assign whole configuration-space cells to a single block.
  // Then one must perform a reduction across a block for each conf-space basis coefficient.
  ...
  MomentCalc1x1vSer_M1i_P1(cellCenter, cellSize, distF, localSumPtr);
 ...
 void cuda_MomentCalc1x1vSer_M1i_P1(GkDeviceProp *prop, int numBlocks, int numThreads, GkylCartField_t *fIn, GkylCartField_t *out) {
  ...
  d_calcMom1x1vSer_M1i_P1<<<numBlocks, numThreads, 2*(numThreads/warpSize)*sizeof(double)>>>(fIn, out);
}
 ...
 __global__ void d_calcMom1x1vSer_M1i_P2(GkylCartField_t *fIn, GkylCartField_t *out) {
  // In computing moments we will first assign whole configuration-space cells to a single block.
  // Then one must perform a reduction across a block for each conf-space basis coefficient.
  ...
  MomentCalc1x1vSer_M1i_P2(cellCenter, cellSize, distF, localSumPtr);
 ...
 void cuda_MomentCalc1x1vSer_M1i_P2(GkDeviceProp *prop, int numBlocks, int numThreads, GkylCartField_t *fIn, GkylCartField_t *out) {
  ...
  d_calcMom1x1vSer_M1i_P2<<<numBlocks, numThreads, 3*(numThreads/warpSize)*sizeof(double)>>>(fIn, out);
}
