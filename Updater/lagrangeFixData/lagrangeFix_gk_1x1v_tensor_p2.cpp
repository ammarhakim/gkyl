...
void lagrangeFix_gk_1x1v_tensor_p2(double *dm0, double *dm1, double *dm2, double *B, double mass, double *lo, double *L, double *Nv, double *vc, double *f) {
  ...
  lambda0[2] = pow(L[0],-6)*(360.0*Nv[0]*dm0[2]*pow(lo[0],4)+Nv[0]*(720.0*L[0]*dm0[2]-720.0*dm1[2])*pow(lo[0],3)+Nv[0]*(504.0*dm0[2]*pow(L[0],2)+360.0*dm2[2]-1080.0*L[0]*dm1[2])*pow(lo[0],2)+Nv[0]*(18.0*dm0[2]*pow(L[0],4)-72.0*dm1[2]*pow(L[0],3)+60.0*dm2[2]*pow(L[0],2))+Nv[0]*lo[0]*(144.0*dm0[2]*pow(L[0],3)-504.0*dm1[2]*pow(L[0],2)+360.0*L[0]*dm2[2]));
  ...
  lambda1[2] = -1.0*pow(L[0],-6)*(720.0*Nv[0]*dm0[2]*pow(lo[0],3)+Nv[0]*(1080.0*L[0]*dm0[2]-1440.0*dm1[2])*pow(lo[0],2)+Nv[0]*(72.0*dm0[2]*pow(L[0],3)-384.0*dm1[2]*pow(L[0],2)+360.0*L[0]*dm2[2])+Nv[0]*lo[0]*(504.0*dm0[2]*pow(L[0],2)+720.0*dm2[2]-1440.0*L[0]*dm1[2]));
  ...
  lambda2[2] = pow(L[0],-6)*(360.0*Nv[0]*dm0[2]*pow(lo[0],2)+Nv[0]*(60.0*dm0[2]*pow(L[0],2)+360.0*dm2[2]-360.0*L[0]*dm1[2])+Nv[0]*lo[0]*(360.0*L[0]*dm0[2]-720.0*dm1[2]));
  ...
  f[8] = f[8] + 0.05270462766947297*lambda2[2]*pow(L[0],3)*pow(Nv[0],-3);
}
