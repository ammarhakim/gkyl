...
function CrossPrimMoments:_advance(tCurr, inFld, outFld)
   ...
   local mSelf, nuSelf, m0Self, m1Self, uSelf, vtSqSelf, m1CorrectionSelf, m2CorrectionSelf
   local mOther, nuOther, m0Other, m1Other, uOther, vtSqOther, m1CorrectionOther, m2CorrectionOther
   mSelf, nuSelf                        = inFld[1], inFld[2]
   m0Self, m1Self                       = inFld[3][1], inFld[3][2]
   uSelf, vtSqSelf                      = inFld[4][1], inFld[4][2]
   m1CorrectionSelf, m2CorrectionSelf   = inFld[5][1], inFld[5][2]

   mOther, nuOther                      = inFld[7], inFld[8]
   m0Other, m1Other                     = inFld[9][1], inFld[9][2]
   uOther, vtSqOther                    = inFld[10][1], inFld[10][2]
   m1CorrectionOther, m2CorrectionOther = inFld[11][1], inFld[11][2]

   local m2Self, m2SelfIndexer, m2SelfItr
   local m2Other, m2OtherIndexer, m2OtherItr
   ...
   if self._polyOrder > 1 then
      m2Self         = inFld[3][3]
      m2Other        = inFld[9][3]
      m2SelfIndexer  = m2Self:genIndexer()
      m2OtherIndexer = m2Other:genIndexer()
   ...
   if self._polyOrder > 1 then
      ...
      for confIdx in confRangeDecomp:rowMajorIter(tId) do
         ...
         crossPrimMomentsCalc(self._binOpData, self._betaP1, mSelf, nuSelf, m0SelfItr:data(), m1SelfItr:data(), uSelfItr:data(), vtSqSelfItr:data(), m1CorrectionSelfItr:data(), m2CorrectionSelfItr:data(), m0StarSelfItr:data(), m1StarSelfItr:data(), m2StarSelfItr:data(), mOther, nuOther, m0OtherItr:data(), m1OtherItr:data(), uOtherItr:data(), vtSqOtherItr:data(), m1CorrectionOtherItr:data(), m2CorrectionOtherItr:data(), m0StarOtherItr:data(), m1StarOtherItr:data(), m2StarOtherItr:data(), uCrossSelfItr:data(), vtSqCrossSelfItr:data(), uCrossOtherItr:data(), vtSqCrossOtherItr:data())
      end
