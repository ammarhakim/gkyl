...
{ 
  ...
  if (m0B[0]<=0.0) {
    ...
    nu[0] = (m0B[0]*normNu*nuFrac)/sqrt(pow(vtSqMinA,3)+1.060660171779821*vtSqB[0]*pow(vtSqMinA,2)+0.375*pow(vtSqB[0],2)*vtSqMinA+0.0441941738241592*pow(vtSqB[0],3)); 
  } else if (vtSqB[0]<vtSqMinB) {
    nu[0] = (m0B[0]*normNu*nuFrac)/sqrt(pow(vtSqMinB,3)+1.060660171779821*vtSqA[0]*pow(vtSqMinB,2)+0.375*pow(vtSqA[0],2)*vtSqMinB+0.0441941738241592*pow(vtSqA[0],3)); 
  } else {
...
{ 
  ...
  if (vtSqA[0]<vtSqMinA) {
    ...
    vtSqA0 = 0.3535533905932738*vtSqA[0]; 
  }
  if (vtSqB[0]<vtSqMinB) {
    vtSqB0 = vtSqMinB; 
  } else {
    vtSqB0 = 0.3535533905932738*vtSqB[0]; 
  }
