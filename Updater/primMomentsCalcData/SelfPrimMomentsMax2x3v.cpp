...
void SelfPrimMoments2x3vMax_P1(const int pVdim, const double *m0, const double *m1, const double *m2, const double *m0S, const double *m1S, const double *cM, const double *cE, double *u, double *vtSq) 
{ 
  ...
  BigAEM(9,9) = 0.5*m0Sr[0]*pVdim-0.5*cE[0]; 
  BigAEM(9,10) = 0.5*m0Sr[1]*pVdim-0.5*cE[1]; 
  BigAEM(9,11) = 0.5*m0Sr[2]*pVdim-0.5*cE[2]; 
  BigAEM(10,9) = 0.5*m0Sr[1]*pVdim-0.5*cE[1]; 
  BigAEM(10,10) = 0.5*m0Sr[0]*pVdim-0.5*cE[0]; 
  BigAEM(11,9) = 0.5*m0Sr[2]*pVdim-0.5*cE[2]; 
  BigAEM(11,11) = 0.5*m0Sr[0]*pVdim-0.5*cE[0]; 
...
void SelfPrimMoments2x3vMax_P2(const int pVdim, const double *m0, const double *m1, const double *m2, const double *m0S, const double *m1S, const double *cM, const double *cE, double *u, double *vtSq) 
{ 
  // m0,m1,m2: moments of the distribution function. 
  // m0S,m1S:  star moments (only used for piecewise linear). 
  // cM, cE:   vtSq*cM and vtSq*cE are corrections to u and vtSq, respectively. 
  ...
  // ....... Block from correction to vtSq .......... // 
  BigAEM(18,18) = 0.5*m0r[0]*pVdim-0.5*cE[0]; 
  BigAEM(18,19) = 0.5*m0r[1]*pVdim-0.5*cE[1]; 
  BigAEM(18,20) = 0.5*m0r[2]*pVdim-0.5*cE[2]; 
  BigAEM(18,21) = 0.5*m0r[3]*pVdim-0.5*cE[3]; 
  BigAEM(18,22) = 0.5*m0r[4]*pVdim-0.5*cE[4]; 
  BigAEM(18,23) = 0.5*m0r[5]*pVdim-0.5*cE[5]; 
  BigAEM(19,18) = 0.5*m0r[1]*pVdim-0.5*cE[1]; 
  BigAEM(19,19) = 0.4472135954999579*m0r[4]*pVdim+0.5*m0r[0]*pVdim-0.4472135954999579*cE[4]-0.5*cE[0]; 
  BigAEM(19,20) = 0.5*m0r[3]*pVdim-0.5*cE[3]; 
  BigAEM(19,21) = 0.5*m0r[2]*pVdim-0.5*cE[2]; 
  BigAEM(19,22) = 0.4472135954999579*m0r[1]*pVdim-0.4472135954999579*cE[1]; 
  BigAEM(20,18) = 0.5*m0r[2]*pVdim-0.5*cE[2]; 
  BigAEM(20,19) = 0.5*m0r[3]*pVdim-0.5*cE[3]; 
  BigAEM(20,20) = 0.4472135954999579*m0r[5]*pVdim+0.5*m0r[0]*pVdim-0.4472135954999579*cE[5]-0.5*cE[0]; 
  BigAEM(20,21) = 0.5*m0r[1]*pVdim-0.5*cE[1]; 
  BigAEM(20,23) = 0.4472135954999579*m0r[2]*pVdim-0.4472135954999579*cE[2]; 
  BigAEM(21,18) = 0.5*m0r[3]*pVdim-0.5*cE[3]; 
  BigAEM(21,19) = 0.5*m0r[2]*pVdim-0.5*cE[2]; 
  BigAEM(21,20) = 0.5*m0r[1]*pVdim-0.5*cE[1]; 
  BigAEM(21,21) = 0.4472135954999579*m0r[5]*pVdim+0.4472135954999579*m0r[4]*pVdim+0.5*m0r[0]*pVdim-0.4472135954999579*cE[5]-0.4472135954999579*cE[4]-0.5*cE[0]; 
  BigAEM(21,22) = 0.4472135954999579*m0r[3]*pVdim-0.4472135954999579*cE[3]; 
  BigAEM(21,23) = 0.4472135954999579*m0r[3]*pVdim-0.4472135954999579*cE[3]; 
  BigAEM(22,18) = 0.5*m0r[4]*pVdim-0.5*cE[4]; 
  BigAEM(22,19) = 0.4472135954999579*m0r[1]*pVdim-0.4472135954999579*cE[1]; 
  BigAEM(22,21) = 0.4472135954999579*m0r[3]*pVdim-0.4472135954999579*cE[3]; 
  BigAEM(22,22) = 0.31943828249997*m0r[4]*pVdim+0.5*m0r[0]*pVdim-0.31943828249997*cE[4]-0.5*cE[0]; 
  BigAEM(23,18) = 0.5*m0r[5]*pVdim-0.5*cE[5]; 
  BigAEM(23,20) = 0.4472135954999579*m0r[2]*pVdim-0.4472135954999579*cE[2]; 
  BigAEM(23,21) = 0.4472135954999579*m0r[3]*pVdim-0.4472135954999579*cE[3]; 
  BigAEM(23,23) = 0.31943828249997*m0r[5]*pVdim+0.5*m0r[0]*pVdim-0.31943828249997*cE[5]-0.5*cE[0]; 
...
{ 
  ...
  const double dS = 0.25*dxvl[3]*dxvl[4]*intFac*(wr[2]-wl[2]); 
...
{ 
  ...
  const double dS = 0.25*dxvl[2]*dxvl[4]*intFac*(wr[3]-wl[3]); 
...
{ 
  ...
  const double dS = 0.25*dxvl[2]*dxvl[3]*intFac*(wr[4]-wl[4]); 
...
{ 
  ...
  out[0] += (2.449489742783178*fvmin[3]+2.449489742783178*fvmax[3]-1.414213562373095*fvmin[0]+1.414213562373095*fvmax[0])*dS; 
...
{ 
  ...
  out[0] += ((-3.16227766016838*fvmin[18])+3.16227766016838*fvmax[18]+2.449489742783178*fvmin[3]+2.449489742783178*fvmax[3]-1.414213562373095*fvmin[0]+1.414213562373095*fvmax[0])*dS; 
  out[1] += (2.449489742783178*fvmin[7]+2.449489742783178*fvmax[7]-1.414213562373095*fvmin[1]+1.414213562373095*fvmax[1])*dS; 
  out[2] += (2.449489742783178*fvmin[8]+2.449489742783178*fvmax[8]-1.414213562373095*fvmin[2]+1.414213562373095*fvmax[2])*dS; 
  out[3] += (1.414213562373095*fvmax[6]-1.414213562373095*fvmin[6])*dS; 
  out[4] += (1.414213562373095*fvmax[16]-1.414213562373095*fvmin[16])*dS; 
  out[5] += (1.414213562373095*fvmax[17]-1.414213562373095*fvmin[17])*dS; 
...
{ 
  ...
  out[1] += dS*((-1.414213562373095*fvmin[1]*vmin)+1.414213562373095*fvmax[1]*vmax-0.7071067811865475*fvmin[1]*dxv[2]-0.7071067811865475*fvmax[1]*dxv[2]); 
  out[2] += dS*((-1.414213562373095*fvmin[2]*vmin)+1.414213562373095*fvmax[2]*vmax-0.7071067811865475*dxv[2]*fvmin[2]-0.7071067811865475*dxv[2]*fvmax[2]); 
...
{ 
  ...
  out[0] += dS*((-3.16227766016838*fvmin[18]*vmin)+2.449489742783178*fvmin[3]*vmin-1.414213562373095*fvmin[0]*vmin+3.16227766016838*fvmax[18]*vmax+2.449489742783178*fvmax[3]*vmax+1.414213562373095*fvmax[0]*vmax); 
  out[1] += dS*(2.449489742783178*fvmin[7]*vmin-1.414213562373095*fvmin[1]*vmin+2.449489742783178*fvmax[7]*vmax+1.414213562373095*fvmax[1]*vmax); 
  out[2] += dS*(2.449489742783178*fvmin[8]*vmin-1.414213562373095*fvmin[2]*vmin+2.449489742783178*fvmax[8]*vmax+1.414213562373095*fvmax[2]*vmax); 
  out[3] += dS*(1.414213562373095*fvmax[6]*vmax-1.414213562373095*fvmin[6]*vmin); 
  out[4] += dS*(1.414213562373095*fvmax[16]*vmax-1.414213562373095*fvmin[16]*vmin); 
  out[5] += dS*(1.414213562373095*fvmax[17]*vmax-1.414213562373095*fvmin[17]*vmin); 
...
{ 
  ...
  const double dS = 0.25*dxv[2]*dxv[4]*intFac; 
 
  out[3] += (2.449489742783178*fvmin[4]+2.449489742783178*fvmax[4]-1.414213562373095*fvmin[0]+1.414213562373095*fvmax[0])*dS; 
  out[4] += (1.414213562373095*fvmax[1]-1.414213562373095*fvmin[1])*dS; 
  out[5] += (1.414213562373095*fvmax[2]-1.414213562373095*fvmin[2])*dS; 
...
{ 
  ...
  out[6] += ((-3.16227766016838*fvmin[19])+3.16227766016838*fvmax[19]+2.449489742783178*fvmin[4]+2.449489742783178*fvmax[4]-1.414213562373095*fvmin[0]+1.414213562373095*fvmax[0])*dS; 
  out[7] += (2.449489742783178*fvmin[9]+2.449489742783178*fvmax[9]-1.414213562373095*fvmin[1]+1.414213562373095*fvmax[1])*dS; 
  out[8] += (2.449489742783178*fvmin[10]+2.449489742783178*fvmax[10]-1.414213562373095*fvmin[2]+1.414213562373095*fvmax[2])*dS; 
  out[9] += (1.414213562373095*fvmax[6]-1.414213562373095*fvmin[6])*dS; 
  out[10] += (1.414213562373095*fvmax[16]-1.414213562373095*fvmin[16])*dS; 
  out[11] += (1.414213562373095*fvmax[17]-1.414213562373095*fvmin[17])*dS; 
...
{ 
  ...
  out[0] += dS*((-3.16227766016838*fvmin[19]*vmin)+2.449489742783178*fvmin[4]*vmin-1.414213562373095*fvmin[0]*vmin+3.16227766016838*fvmax[19]*vmax+2.449489742783178*fvmax[4]*vmax+1.414213562373095*fvmax[0]*vmax); 
  out[1] += dS*(2.449489742783178*fvmin[9]*vmin-1.414213562373095*fvmin[1]*vmin+2.449489742783178*fvmax[9]*vmax+1.414213562373095*fvmax[1]*vmax); 
  out[2] += dS*(2.449489742783178*fvmin[10]*vmin-1.414213562373095*fvmin[2]*vmin+2.449489742783178*fvmax[10]*vmax+1.414213562373095*fvmax[2]*vmax); 
  out[3] += dS*(1.414213562373095*fvmax[6]*vmax-1.414213562373095*fvmin[6]*vmin); 
  out[4] += dS*(1.414213562373095*fvmax[16]*vmax-1.414213562373095*fvmin[16]*vmin); 
  out[5] += dS*(1.414213562373095*fvmax[17]*vmax-1.414213562373095*fvmin[17]*vmin); 
...
{ 
  ...
  out[6] += (2.449489742783178*fvmin[5]+2.449489742783178*fvmax[5]-1.414213562373095*fvmin[0]+1.414213562373095*fvmax[0])*dS; 
  out[7] += (1.414213562373095*fvmax[1]-1.414213562373095*fvmin[1])*dS; 
  out[8] += (1.414213562373095*fvmax[2]-1.414213562373095*fvmin[2])*dS; 
...
{ 
  ...
  out[12] += ((-3.16227766016838*fvmin[20])+3.16227766016838*fvmax[20]+2.449489742783178*fvmin[5]+2.449489742783178*fvmax[5]-1.414213562373095*fvmin[0]+1.414213562373095*fvmax[0])*dS; 
  out[13] += (2.449489742783178*fvmin[12]+2.449489742783178*fvmax[12]-1.414213562373095*fvmin[1]+1.414213562373095*fvmax[1])*dS; 
  out[14] += (2.449489742783178*fvmin[13]+2.449489742783178*fvmax[13]-1.414213562373095*fvmin[2]+1.414213562373095*fvmax[2])*dS; 
  out[15] += (1.414213562373095*fvmax[6]-1.414213562373095*fvmin[6])*dS; 
  out[16] += (1.414213562373095*fvmax[16]-1.414213562373095*fvmin[16])*dS; 
  out[17] += (1.414213562373095*fvmax[17]-1.414213562373095*fvmin[17])*dS; 
...
{ 
  ...
  out[0] += dS*((-3.16227766016838*fvmin[20]*vmin)+2.449489742783178*fvmin[5]*vmin-1.414213562373095*fvmin[0]*vmin+3.16227766016838*fvmax[20]*vmax+2.449489742783178*fvmax[5]*vmax+1.414213562373095*fvmax[0]*vmax); 
  out[1] += dS*(2.449489742783178*fvmin[12]*vmin-1.414213562373095*fvmin[1]*vmin+2.449489742783178*fvmax[12]*vmax+1.414213562373095*fvmax[1]*vmax); 
  out[2] += dS*(2.449489742783178*fvmin[13]*vmin-1.414213562373095*fvmin[2]*vmin+2.449489742783178*fvmax[13]*vmax+1.414213562373095*fvmax[2]*vmax); 
  out[3] += dS*(1.414213562373095*fvmax[6]*vmax-1.414213562373095*fvmin[6]*vmin); 
  out[4] += dS*(1.414213562373095*fvmax[16]*vmax-1.414213562373095*fvmin[16]*vmin); 
  out[5] += dS*(1.414213562373095*fvmax[17]*vmax-1.414213562373095*fvmin[17]*vmin); 
