...
function BgkCollisions:init(tbl)
   ...
   self._N = tbl.numConfQuad and tbl.numConfQuad or self._confBasis:polyOrder() + 1 -- number of quadrature points in each direction
...
function BgkCollisions:_advance(tCurr, dt, inFld, outFld)
   ...
   for _, nm in ipairs(self._speciesList) do
      ...
      local ptclEnergyIn = assert(inFld[nm][3],
				  "BgkCollisions.advance: Must specify an input fluid moments field")
      local fOut = assert(outFld[nm][1],
			  "BgkCollisions.advance: Must specify an output field")
      
      local phaseLower, phaseUpper = qOut:getLower(), qOut:getUpper()
      local l, u = {}, {}
      for d = 1, numConfDims do
	 l[d], u[d] = phaseLower[d], phaseUpper[d]
      end
      local confRange = Range.Range(l, u)
      l, u = {}, {}
      local numVelDims = numPhaseDims - numConfDims
      for d = 1, numVelDims do
	 l[d], u[d] = phaseLower[d + numConfDims], phaseUpper[d + numConfDims]
      end
      ...
      local phaseIndexer = fOutIn:getIndexer()
      ...
      local maxwell = 0
      local nu = 0
      local maxwellMod = Lin.Vec(numPhaseBasis)
	 ...
	 for velIdx in velRange:colMajorIter() do
	    ...
	    for muIdx in phaseQuadRange:colMajorIter() do
	       ...
	       maxwell = numDensityOd[confMu]
	       for d = 1, numVelDims do
		  maxwell = maxwell / math.sqrt(2 * math.pi * velTherm2Ord[confMu]) *
		     mat.exp(-(self._phaseOrdinates[phaseMu][numConfDims + d] - velBulkOrd[confMu][d]) * 
				(self._phaseOrdinates[phaseMu][numConfDims + d] - velBulkOrd[confMu][d]) /
				(2 * velTherm2Ord[confMu]))
	       end
	       nu = self._collFreq(numDensityOd[confMu], velTherm2Ord[confMu])
	       for k = 1, numPhaseBasis do
		  fItr[k] = fItr[k] + self._cfl * dt * nu * (self._phaseWeights[phaseMu] * maxwell *
