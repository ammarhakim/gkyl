...
gkylFiveMomentSrcExact(FiveMomentSrcData_t *sd, FluidData_t *fd, double dt, double **ff, double *em, double *staticEm)
{
  ...
  double wp2_tot = 0;
  double Enorm = 1.;  // FIXME: nicer way?
  E0[0] = em[EX] / Enorm;
  E0[1] = em[EX] / Enorm;
  E0[2] = em[EX] / Enorm;
  for (unsigned n=0; n < nFluids; ++n)
  {
    ...
    qbym[n] = fd[n].charge / fd[n].mass;
    Wc[n] = qbym[n] * Bmag;
    local wp2 = f[RHO] * sq(qbym[n]) / epsilon0;
    local wp[n] = std::sqrt(wp2);
    wp2_tot += wp2;
    Pnorm[n] = Enorm * std::sqrt(f[RHO] * epsilon0);
    u0[n][0] = f[MX] / Pnorm[n];
    u0[n][1] = f[MY] / Pnorm[n];
    u0[n][2] = f[MZ] / Pnorm[n];
  }
  double wp_tot = std::sqrt(wp2_tot);

  // parallel
  // compute eigen vectors at time = 0
  Eigen::MatrixXd::Constant eig_par(nFluids + 1, nFluids + 1, 0.0);
  ...
  {
    eig_par(n + 1, n + 2) = 1. / wp[n];
    eig_par(nFluids, n + 2) = -1. / wp[nFluids-1];
  }

  // compute eigen coefficients from u0[n]

  // compute eigen vectors as function of time
  double cost = std::cos(wp_tot * dt * 0.5);
  double sint = std::sin(wp_tot * dt * 0.5);
  eig_par(0, 0) = wp_tot * cost;
  for (unsigned n = 0; n < nFluids; ++n)
  {
    eig_par(n + 1, 0) = wp[n] * sint;
  }
  eig_par(0, nFluids - 1) = wp_tot * sint;
  for (unsigned n = 0; n < nFluids; ++n)
  {
    eig_par(n + 1, 1) = wp[n] * cost;
  }

  // compute new values u1[n]
  for (unsigned n = 0; n < nFluids; ++n)
  {
  }
