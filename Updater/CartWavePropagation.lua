...
-- constructor
function CartWavePropagation:new(tbl)
   local self = setmetatable({}, CartWavePropagation)

   -- read data from input table
   self._onGrid = assert(tbl.onGrid, "Updater.CartWavePropagation: Must provide grid object using 'onGrid'")
   self._equation = assert(tbl.equation, "Updater.CartWavePropagation: Must provide equation object using 'equation'")
   self._limiterFunc = assert(limiterFunctions[tbl.limiter], "Updater.CartWavePropagation: Must specify limiter to use")

   -- set private data
   self._privData = new(typeof("WavePrivateData_t"))
   self._privData._ndim = self._onGrid:ndim()
   self._privData._cfl = assert(tbl.cfl, "Updater.CartWavePropagation: Must specify CFL number using 'cfl'")
...
setmetatable(CartWavePropagation, { __call = function (self, o) return self.new(self, o) end })
...
local function advance(self, tCurr, dt, inFld, outFld)
   ...
   local qIn = assert(inFld[1], "CartWavePropagation.advance: Must-specify an input field")
   local qOut = assert(outFld[1], "CartWavePropagation.advance: Must-specify an output field")
...
CartWavePropagation.__index = { advance = Base.advanceFuncWrap(advance) }

return {
   CartWavePropagation = CartWavePropagation
}
