...
{ 
  ...
  double vSqAv = pow(w[2],2.0)-1.414213562373095*u[2]*w[2]+0.5*pow(u[2],2.0)+pow(w[1],2.0)-1.414213562373095*u[0]*w[1]+0.5*pow(u[0],2.0); 
 
  fMax[0] = (0.4501581580785531*m0Av)/(exp(vSqAv/(2.0*vtSqAv))*abs(vtSqAv)); 
...
{ 
  ...
  double m0Av = 0.7071067811865476*m0[0]; 
  double vtSqAv = 0.7071067811865476*vtSq[0]; 
  double vSqAv = 0.5*pow(u[3],2.0)-1.414213562373095*w[2]*u[3]+pow(w[2],2.0)+pow(w[1],2.0)-1.414213562373095*u[0]*w[1]+0.5*pow(u[0],2.0); 
