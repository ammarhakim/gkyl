...
{ 
  ...
  tmp[0] = 0.25*(A[16]*B[16]+A[15]*B[15]+A[14]*B[14]+A[13]*B[13]+A[12]*B[12]+A[11]*B[11]+A[10]*B[10]+A[9]*B[9]+A[8]*B[8]+A[7]*B[7]+A[6]*B[6]+A[5]*B[5]+A[4]*B[4]+A[3]*B[3]+A[2]*B[2]+A[1]*B[1]+A[0]*B[0]); 
  tmp[1] = 0.25*(A[14]*B[15]+B[14]*A[15]+A[10]*B[13]+B[10]*A[13]+A[9]*B[12]+B[9]*A[12]+A[7]*B[11]+B[7]*A[11]+A[4]*B[8]+B[4]*A[8]+A[3]*B[6]+B[3]*A[6]+A[2]*B[5]+B[2]*A[5]+A[0]*B[1]+B[0]*A[1]); 
  tmp[2] = 0.25*(A[13]*B[15]+B[13]*A[15]+A[10]*B[14]+B[10]*A[14]+A[8]*B[12]+B[8]*A[12]+A[6]*B[11]+B[6]*A[11]+A[4]*B[9]+B[4]*A[9]+A[3]*B[7]+B[3]*A[7]+A[1]*B[5]+B[1]*A[5]+A[0]*B[2]+B[0]*A[2]); 
  tmp[3] = 0.05*(4.47213595499958*(A[3]*B[16]+B[3]*A[16])+5.0*(A[12]*B[15]+B[12]*A[15]+A[9]*B[14]+B[9]*A[14]+A[8]*B[13]+B[8]*A[13]+A[5]*B[11]+B[5]*A[11]+A[4]*B[10]+B[4]*A[10]+A[2]*B[7]+B[2]*A[7]+A[1]*B[6]+B[1]*A[6]+A[0]*B[3]+B[0]*A[3])); 
  tmp[4] = 0.25*(A[11]*B[15]+B[11]*A[15]+A[7]*B[14]+B[7]*A[14]+A[6]*B[13]+B[6]*A[13]+A[5]*B[12]+B[5]*A[12]+A[3]*B[10]+B[3]*A[10]+A[2]*B[9]+B[2]*A[9]+A[1]*B[8]+B[1]*A[8]+A[0]*B[4]+B[0]*A[4]); 
  tmp[5] = 0.25*(A[10]*B[15]+B[10]*A[15]+A[13]*B[14]+B[13]*A[14]+A[4]*B[12]+B[4]*A[12]+A[3]*B[11]+B[3]*A[11]+A[8]*B[9]+B[8]*A[9]+A[6]*B[7]+B[6]*A[7]+A[0]*B[5]+B[0]*A[5]+A[1]*B[2]+B[1]*A[2]); 
  tmp[6] = 0.05*(4.47213595499958*(A[6]*B[16]+B[6]*A[16])+5.0*(A[9]*B[15]+B[9]*A[15]+A[12]*B[14]+B[12]*A[14]+A[4]*B[13]+B[4]*A[13]+A[2]*B[11]+B[2]*A[11]+A[8]*B[10]+B[8]*A[10]+A[5]*B[7]+B[5]*A[7]+A[0]*B[6]+B[0]*A[6]+A[1]*B[3]+B[1]*A[3])); 
  tmp[7] = 0.05*(4.47213595499958*(A[7]*B[16]+B[7]*A[16])+5.0*(A[8]*B[15]+B[8]*A[15]+A[4]*B[14]+B[4]*A[14]+A[12]*B[13]+B[12]*A[13]+A[1]*B[11]+B[1]*A[11]+A[9]*B[10]+B[9]*A[10]+A[0]*B[7]+B[0]*A[7]+A[5]*B[6]+B[5]*A[6]+A[2]*B[3]+B[2]*A[3])); 
  tmp[8] = 0.25*(A[7]*B[15]+B[7]*A[15]+A[11]*B[14]+B[11]*A[14]+A[3]*B[13]+B[3]*A[13]+A[2]*B[12]+B[2]*A[12]+A[6]*B[10]+B[6]*A[10]+A[5]*B[9]+B[5]*A[9]+A[0]*B[8]+B[0]*A[8]+A[1]*B[4]+B[1]*A[4]); 
  tmp[9] = 0.25*(A[6]*B[15]+B[6]*A[15]+A[3]*B[14]+B[3]*A[14]+A[11]*B[13]+B[11]*A[13]+A[1]*B[12]+B[1]*A[12]+A[7]*B[10]+B[7]*A[10]+A[0]*B[9]+B[0]*A[9]+A[5]*B[8]+B[5]*A[8]+A[2]*B[4]+B[2]*A[4]); 
  tmp[10] = 0.05*(4.47213595499958*(A[10]*B[16]+B[10]*A[16])+5.0*(A[5]*B[15]+B[5]*A[15]+A[2]*B[14]+B[2]*A[14]+A[1]*B[13]+B[1]*A[13]+A[11]*B[12]+B[11]*A[12]+A[0]*B[10]+B[0]*A[10]+A[7]*B[9]+B[7]*A[9]+A[6]*B[8]+B[6]*A[8]+A[3]*B[4]+B[3]*A[4])); 
  tmp[11] = 0.05*(4.47213595499958*(A[11]*B[16]+B[11]*A[16])+5.0*(A[4]*B[15]+B[4]*A[15]+A[8]*B[14]+B[8]*A[14]+A[9]*B[13]+B[9]*A[13]+A[10]*B[12]+B[10]*A[12]+A[0]*B[11]+B[0]*A[11]+A[1]*B[7]+B[1]*A[7]+A[2]*B[6]+B[2]*A[6]+A[3]*B[5]+B[3]*A[5])); 
  tmp[12] = 0.25*(A[3]*B[15]+B[3]*A[15]+A[6]*B[14]+B[6]*A[14]+A[7]*B[13]+B[7]*A[13]+A[0]*B[12]+B[0]*A[12]+A[10]*B[11]+B[10]*A[11]+A[1]*B[9]+B[1]*A[9]+A[2]*B[8]+B[2]*A[8]+A[4]*B[5]+B[4]*A[5]); 
  tmp[13] = 0.05*(4.47213595499958*(A[13]*B[16]+B[13]*A[16])+5.0*(A[2]*B[15]+B[2]*A[15]+A[5]*B[14]+B[5]*A[14]+A[0]*B[13]+B[0]*A[13]+A[7]*B[12]+B[7]*A[12]+A[9]*B[11]+B[9]*A[11]+A[1]*B[10]+B[1]*A[10]+A[3]*B[8]+B[3]*A[8]+A[4]*B[6]+B[4]*A[6])); 
  tmp[14] = 0.05*(4.47213595499958*(A[14]*B[16]+B[14]*A[16])+5.0*(A[1]*B[15]+B[1]*A[15]+A[0]*B[14]+B[0]*A[14]+A[5]*B[13]+B[5]*A[13]+A[6]*B[12]+B[6]*A[12]+A[8]*B[11]+B[8]*A[11]+A[2]*B[10]+B[2]*A[10]+A[3]*B[9]+B[3]*A[9]+A[4]*B[7]+B[4]*A[7])); 
  tmp[15] = 0.05*(4.47213595499958*(A[15]*B[16]+B[15]*A[16])+5.0*(A[0]*B[15]+B[0]*A[15]+A[1]*B[14]+B[1]*A[14]+A[2]*B[13]+B[2]*A[13]+A[3]*B[12]+B[3]*A[12]+A[4]*B[11]+B[4]*A[11]+A[5]*B[10]+B[5]*A[10]+A[6]*B[9]+B[6]*A[9]+A[7]*B[8]+B[7]*A[8])); 
...
{ 
  ...
  // Fill AEM matrix. 
  data->AEM_D = Eigen::MatrixXd::Zero(17,17); 
  data->AEM_D(0,0) = 0.5*As[0]; 
  data->AEM_D(0,1) = 0.5*As[1]; 
  data->AEM_D(0,2) = 0.5*As[2]; 
  data->AEM_D(0,4) = 0.5*As[1]; 
  ...
  data->AEM_D(0,10) = 0.5*As[0]; 
  data->AEM_D(0,15) = 0.5*As[0]; 
  data->AEM_D(1,3) = 0.5*As[3]; 
  data->AEM_D(1,4) = 0.5*As[2]; 
  data->AEM_D(1,5) = 0.5*As[1]; 
  data->AEM_D(1,10) = 0.5*As[1]; 
  data->AEM_D(1,14) = 0.5*As[2]; 
  data->AEM_D(2,13) = 0.5*As[3]; 
