...
{ 
  ...
  tmp[5] = 0.7071067811865475*A[2]*B[8]+0.7071067811865475*A[1]*B[7]+0.7071067811865475*A[0]*B[5]; 
  tmp[6] = 0.4517539514526256*A[2]*B[6]+0.7071067811865475*A[0]*B[6]+0.632455532033676*A[1]*B[3]+0.7071067811865475*A[2]*B[2]; 
  tmp[7] = 0.632455532033676*A[1]*B[8]+0.6324555320336759*A[2]*B[7]+0.7071067811865475*A[0]*B[7]+0.7071067811865475*A[1]*B[5]; 
  tmp[8] = 0.4517539514526256*A[2]*B[8]+0.7071067811865475*A[0]*B[8]+0.632455532033676*A[1]*B[7]+0.7071067811865475*A[2]*B[5]; 
...
{ 
  ...
  tmp[2] = 0.7071067811865474*A[3]*B[11]+0.7071067811865475*A[2]*B[6]+0.7071067811865475*A[1]*B[3]+0.7071067811865475*A[0]*B[2]; 
  tmp[3] = 0.6210590034081187*A[2]*B[11]+0.6210590034081187*A[3]*B[6]+0.632455532033676*A[1]*B[6]+0.6324555320336759*A[2]*B[3]+0.7071067811865475*A[0]*B[3]+0.7071067811865475*A[1]*B[2]; 
  ...
  tmp[5] = 0.7071067811865474*A[3]*B[13]+0.7071067811865475*A[2]*B[10]+0.7071067811865475*A[1]*B[7]+0.7071067811865475*A[0]*B[5]; 
  tmp[6] = 0.4216370213557839*A[3]*B[11]+0.6210590034081187*A[1]*B[11]+0.4517539514526256*A[2]*B[6]+0.7071067811865475*A[0]*B[6]+0.6210590034081187*A[3]*B[3]+0.632455532033676*A[1]*B[3]+0.7071067811865475*A[2]*B[2]; 
  tmp[7] = 0.6210590034081186*A[2]*B[13]+0.6210590034081188*A[3]*B[10]+0.632455532033676*A[1]*B[10]+0.6324555320336759*A[2]*B[7]+0.7071067811865475*A[0]*B[7]+0.7071067811865475*A[1]*B[5]; 
  ...
  tmp[9] = 0.7071067811865475*A[3]*B[15]+0.7071067811865474*A[2]*B[14]+0.7071067811865474*A[1]*B[12]+0.7071067811865475*A[0]*B[9]; 
  tmp[10] = 0.421637021355784*A[3]*B[13]+0.6210590034081186*A[1]*B[13]+0.4517539514526256*A[2]*B[10]+0.7071067811865475*A[0]*B[10]+0.6210590034081187*A[3]*B[7]+0.632455532033676*A[1]*B[7]+0.7071067811865475*A[2]*B[5]; 
  tmp[11] = 0.421637021355784*A[2]*B[11]+0.7071067811865475*A[0]*B[11]+0.4216370213557839*A[3]*B[6]+0.6210590034081187*A[1]*B[6]+0.6210590034081187*A[2]*B[3]+0.7071067811865474*B[2]*A[3]; 
  tmp[12] = 0.6210590034081186*A[2]*B[15]+0.6210590034081186*A[3]*B[14]+0.6324555320336759*A[1]*B[14]+0.6324555320336759*A[2]*B[12]+0.7071067811865475*A[0]*B[12]+0.7071067811865474*A[1]*B[9]; 
  tmp[13] = 0.421637021355784*A[2]*B[13]+0.7071067811865475*A[0]*B[13]+0.421637021355784*A[3]*B[10]+0.6210590034081186*A[1]*B[10]+0.6210590034081186*A[2]*B[7]+0.7071067811865474*A[3]*B[5]; 
  tmp[14] = 0.421637021355784*A[3]*B[15]+0.6210590034081186*A[1]*B[15]+0.4517539514526256*A[2]*B[14]+0.7071067811865475*A[0]*B[14]+0.6210590034081186*A[3]*B[12]+0.6324555320336759*A[1]*B[12]+0.7071067811865474*A[2]*B[9]; 
  tmp[15] = 0.421637021355784*A[2]*B[15]+0.7071067811865475*A[0]*B[15]+0.421637021355784*A[3]*B[14]+0.6210590034081186*A[1]*B[14]+0.6210590034081187*A[2]*B[12]+0.7071067811865475*A[3]*B[9]; 
...
{ 
  ...
  // Declare Eigen Matrix with triple basis tensor dotted with B vector. 
  Eigen::MatrixXd AEM = Eigen::MatrixXd::Zero(9,9); 
  // Declare Eigen Vector with coefficients of B. 
  ...
  AEM(1,3) = 0.7071067811865475*As[2]; 
  AEM(1,4) = 0.6324555320336759*As[1]; 
  AEM(2,2) = 0.7071067811865475*As[2]; 
 
  // Fill BEV. 
  BEV << Bs[0],Bs[1],Bs[2],Bs[3],Bs[4],Bs[5],Bs[6],Bs[7],Bs[8]; 
 
  // Solve the system of equations. 
  u = AEM.colPivHouseholderQr().solve(BEV); 
 
  // Copy data from Eigen vector. 
  Eigen::Map<VectorXd>(out,9,1) = u; 
...
{ 
  ...
  AEM(0,15) = 0.6324555320336759*As[2]+0.7071067811865475*As[0]; 
  AEM(1,0) = 0.7071067811865475*As[2]; 
  AEM(1,1) = 0.6210590034081186*As[3]+0.6324555320336759*As[1]; 
  AEM(1,10) = 0.7071067811865475*As[2]; 
  AEM(1,11) = 0.6210590034081187*As[3]+0.632455532033676*As[1]; 
  AEM(2,0) = 0.7071067811865475*As[3]; 
  AEM(2,1) = 0.6210590034081186*As[2]; 
  AEM(2,14) = 0.7071067811865474*As[3]; 
  AEM(2,15) = 0.6210590034081187*As[2]; 
