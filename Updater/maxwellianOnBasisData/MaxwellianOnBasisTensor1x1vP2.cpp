...
void MaxwellianOnBasisGauss1x1vTensor_P2_phaseQuad(const double *flowUOrd, const double *vtSqOrd, const double *fMFacOrd, const double *bmagOrd, const double m_, const double *wc, const double *dxv, double *fMOut) {
  ...
  fMOut[6] = 0.1851851851851852*fMquad[8]-0.1851851851851852*fMquad[7]+0.1851851851851852*fMquad[5]-0.1851851851851852*fMquad[4]-0.3703703703703703*fMquad[2]+0.3703703703703703*fMquad[1]; 
  ...
  fMOut[8] = 0.1234567901234569*(fMquad[8]+fMquad[7])-0.2469135802469136*fMquad[6]+0.1234567901234569*(fMquad[5]+fMquad[4])-0.2469135802469136*fMquad[3]-0.2469135802469136*(fMquad[2]+fMquad[1])+0.4938271604938271*fMquad[0]; 
...
void GkMaxwellianOnBasisGauss1x1vTensor_P2_evAtConfOrd(const double *den, const double *flowU, const double *vtSq, const double *bmag, double *flowUOrd, double *vtSqOrd, double *fMFacOrd, double *bmagOrd) {
  ...
  if ((vtSqOrd[0] > 0.) && (m0Ord[0] > 0.))
    fMFacOrd[0] = (0.3989422804014326*m0Ord[0]*(0.7071067811865475*bmag[0]-0.7905694150420947*bmag[2]))/sqrt(vtSqOrd[0]); 
  else
    fMFacOrd[0] = 0.0;
  if ((vtSqOrd[1] > 0.) && (m0Ord[1] > 0.))
    fMFacOrd[1] = (0.3989422804014326*m0Ord[1]*(0.6324555320336759*bmag[2]-0.9486832980505137*bmag[1]+0.7071067811865475*bmag[0]))/sqrt(vtSqOrd[1]); 
  else
    fMFacOrd[1] = 0.0;
  if ((vtSqOrd[2] > 0.) && (m0Ord[2] > 0.))
    fMFacOrd[2] = (0.3989422804014326*(0.6324555320336759*bmag[2]+0.9486832980505137*bmag[1]+0.7071067811865475*bmag[0])*m0Ord[2])/sqrt(vtSqOrd[2]); 
  else
    fMFacOrd[2] = 0.0;
...
void GkMaxwellianOnBasisGauss1x1vTensor_P2_phaseQuad(const double *flowUOrd, const double *vtSqOrd, const double *fMFacOrd, const double *bmagOrd, const double m_, const double *wc, const double *dxv, double *fMOut) {
  ...
  double fMquad[9];
  if ((vtSqOrd[0] > 0.) && (fMFacOrd[0] > 0.)) {
    fMquad[0] = fMFacOrd[0]*exp(-(0.5*std::pow(wc[1]-1.0*flowUOrd[0],2.0))/vtSqOrd[0])+9.999999999999999e-41; 
    fMquad[1] = fMFacOrd[0]*exp(-(0.5*std::pow(wc[1]-0.3872983346207417*dxv[1]-1.0*flowUOrd[0],2.0))/vtSqOrd[0])+9.999999999999999e-41; 
    fMquad[2] = fMFacOrd[0]*exp(-(0.5*std::pow(wc[1]+0.3872983346207417*dxv[1]-1.0*flowUOrd[0],2.0))/vtSqOrd[0])+9.999999999999999e-41; 
  } else {
  ...
  if ((vtSqOrd[1] > 0.) && (fMFacOrd[1] > 0.)) {
    fMquad[3] = fMFacOrd[1]*exp(-(0.5*std::pow(wc[1]-1.0*flowUOrd[1],2.0))/vtSqOrd[1])+9.999999999999999e-41; 
    fMquad[4] = fMFacOrd[1]*exp(-(0.5*std::pow(wc[1]-1.0*flowUOrd[1]-0.3872983346207417*dxv[1],2.0))/vtSqOrd[1])+9.999999999999999e-41; 
    fMquad[5] = fMFacOrd[1]*exp(-(0.5*std::pow(wc[1]-1.0*flowUOrd[1]+0.3872983346207417*dxv[1],2.0))/vtSqOrd[1])+9.999999999999999e-41; 
  } else {
  ...
  if ((vtSqOrd[2] > 0.) && (fMFacOrd[2] > 0.)) {
    fMquad[6] = fMFacOrd[2]*exp(-(0.5*std::pow(wc[1]-1.0*flowUOrd[2],2.0))/vtSqOrd[2])+9.999999999999999e-41; 
    fMquad[7] = fMFacOrd[2]*exp(-(0.5*std::pow((-1.0*flowUOrd[2])+wc[1]-0.3872983346207417*dxv[1],2.0))/vtSqOrd[2])+9.999999999999999e-41; 
    fMquad[8] = fMFacOrd[2]*exp(-(0.5*std::pow((-1.0*flowUOrd[2])+wc[1]+0.3872983346207417*dxv[1],2.0))/vtSqOrd[2])+9.999999999999999e-41; 
  } else {
