...
void GkMaxwellianOnBasisGauss1x1vMax_P1_evAtConfOrd(const double *den, const double *flowU, const double *vtSq, const double *bmag, double *flowUOrd, double *vtSqOrd, double *fMFacOrd, double *bmagOrd) {
  ...
  if (vtSqOrd[0] <= 0.0)
    ...
    fMFacOrd[0] = (0.3989422804014326*(0.7071067811865475*bmag[0]-0.7071067811865474*bmag[1])*(0.7071067811865475*den[0]-0.7071067811865474*den[1]))/sqrt(vtSqOrd[0])+9.999999999999999e-41; 
  if (vtSqOrd[1] <= 0.0)
    fMFacOrd[1] = 9.999999999999999e-41;
  else
    fMFacOrd[1] = (0.3989422804014326*(0.7071067811865474*bmag[1]+0.7071067811865475*bmag[0])*(0.7071067811865474*den[1]+0.7071067811865475*den[0]))/sqrt(vtSqOrd[1])+9.999999999999999e-41; 
