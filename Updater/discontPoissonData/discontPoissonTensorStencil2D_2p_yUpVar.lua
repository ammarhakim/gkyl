...
local function stencilFn(dx, D, N, val)
  ...
  _M[1][5][8] = 0.0
  _M[1][5][9] = (965.9813662799093*N+261.6199533674754*D)/(360.0*dx[2]^2*N+40.0*dx[2]^2*D)
  ...
  _M[1][7][3] = 0.0
  _M[1][7][4] = 0.0
  _M[1][7][5] = -(1.0*(181.8653347947321*N-8.660254037844387*D))/(45.0*dx[2]^2*N+5.0*dx[2]^2*D)
  _M[1][7][6] = 0.0
  _M[1][7][7] = -(1.0*(687.0*N-57.0*D))/(144.0*dx[2]^2*N+16.0*dx[2]^2*D)
  _M[1][7][8] = 0.0
  _M[1][7][9] = -(1.0*(1591.796155291248*N-298.2197176579711*D))/(720.0*dx[2]^2*N+80.0*dx[2]^2*D)
  _M[1][8][1] = 0.0
  ...
  _M[1][9][3] = 0.0
  _M[1][9][4] = 0.0
  _M[1][9][5] = (80.49844718999243*N+20.12461179749811*D)/(18.0*dx[2]^2*N+2.0*dx[2]^2*D)
  _M[1][9][6] = 0.0
  _M[1][9][7] = (755.2317525104464*N+213.0140840414079*D)/(144.0*dx[2]^2*N+16.0*dx[2]^2*D)
  _M[1][9][8] = 0.0
  _M[1][9][9] = (333.0*N+129.0*D)/(144.0*dx[2]^2*N+16.0*dx[2]^2*D)
  _M[2] = Lin.Mat(9,9)
  ...
  _M[2][7][3] = 0.0
  _M[2][7][4] = 0.0
  _M[2][7][5] = (727.4613391789285*N-588.8972745734183*D)/(180.0*dx[2]^2*N+20.0*dx[2]^2*D)
  _M[2][7][6] = 0.0
  _M[2][7][7] = -(1.0*(2097.0*N+729.0*D))/(144.0*dx[2]^2*N+16.0*dx[2]^2*D)
  _M[2][7][8] = 0.0
  _M[2][7][9] = -(1.0*(848.1833528194243*N+4907.069899644797*D))/(720.0*dx[2]^2*N+80.0*dx[2]^2*D)
  _M[2][8][1] = 0.0
  ...
  _M[2][9][6] = 0.0
  _M[2][9][7] = (731.9938524332018*N-398.9172846593639*D)/(144.0*dx[2]^2*N+16.0*dx[2]^2*D)
  _M[2][9][8] = 0.0
  _M[2][9][9] = -(1.0*(6003.0*N+1599.0*D))/(144.0*dx[2]^2*N+16.0*dx[2]^2*D)
  _M[3] = Lin.Vec(9)
  _M[3][1] = 0.0 +(203.6467529817258/(72.0*dx[2]^2*N+8.0*dx[2]^2*D))*val[1] +(0.0)*val[2] +(0.0)*val[3]
  _M[3][2] = 0.0 +(0.0)*val[1] +(203.6467529817258/(72.0*dx[2]^2*N+8.0*dx[2]^2*D))*val[2] +(0.0)*val[3]
  _M[3][3] = 0.0 +(313.5346870762469/(72.0*dx[2]^2*N+8.0*dx[2]^2*D))*val[1] +(0.0)*val[2] +(0.0)*val[3]
  _M[3][4] = 0.0 +(0.0)*val[1] +(313.5346870762469/(72.0*dx[2]^2*N+8.0*dx[2]^2*D))*val[2] +(0.0)*val[3]
  _M[3][5] = 0.0 +(0.0)*val[1] +(0.0)*val[2] +(203.6467529817258/(72.0*dx[2]^2*N+8.0*dx[2]^2*D))*val[3]
  _M[3][6] = 0.0 +(151.7893276880823/(36.0*dx[2]^2*N+4.0*dx[2]^2*D))*val[1] +(0.0)*val[2] +(0.0)*val[3]
  _M[3][7] = 0.0 +(0.0)*val[1] +(0.0)*val[2] +(195.9591794226544/(45.0*dx[2]^2*N+5.0*dx[2]^2*D))*val[3]
  _M[3][8] = 0.0 +(0.0)*val[1] +(303.5786553761645/(72.0*dx[2]^2*N+8.0*dx[2]^2*D))*val[2] +(0.0)*val[3]
  _M[3][9] = 0.0 +(0.0)*val[1] +(0.0)*val[2] +(151.7893276880823/(36.0*dx[2]^2*N+4.0*dx[2]^2*D))*val[3]
  return(_M)
end
