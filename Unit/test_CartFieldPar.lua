...
function test_7(comm)
   ...
   for tI = 1, #testCuts do
      ...
      for idx in localExtRange:colMajorIter() do
         local fitr = field[tI]:get(indexer(idx))
         assert_equal(idx[1] + 2 * idx[2] + 3 * idx[3] + 1, fitr[1],
            string.format("Checking field value at (%d, %d, %d)", idx[1], idx[2], idx[3]))
      end
      Mpi.Barrier(comm)
...
function test_copyFieldGlobalFromLocal_2D(comm)
   ...
   local sz = Mpi.Comm_size(comm)
   if sz ~= 4 then
      log("Not running test_copyFieldGlobalFromLocal_2D as numProcs not exactly 4")
      return
   end
...
function test_copyFieldLocalFromGlobal_2D(comm)
   -- Initialize the global field and grid
   local sz = Mpi.Comm_size(comm)
   local defaultCommSize = 4
   if sz ~= defaultCommSize then
      log(string.format("Not running test_test_copyFieldLocalFromGlobal_2D as numProcs not exactly %d", defaultCommSize))
      return
   end
...
test_copyFieldLocalFromGlobal_2D(comm)
