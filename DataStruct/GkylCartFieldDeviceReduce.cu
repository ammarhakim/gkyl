...
__global__ void d_reduceCartField(int kComp, baseReduceOp *redOpIn, GkylCartField_t *fIn, double *redPerBlock) {
  ...
  while (linearIdx < numCellsTot) {
    ...
    myReduc = redOpIn->reduce(myReduc, fld[kComp]);
...
void reduceCartField(baseReduceOp *opIn, int numCellsTot, int currComp, int blocks, int threads, GkylCartField_t *fIn, double *blockRed) {
  ...
  if (isPow2(numCellsTot)) {
    switch (threads) {
      case 512:
        d_reduceCartField<512,true><<<blocks, threads, smemSize>>>(currComp, opIn, fIn, blockRed);
        break;
      case 256:
        d_reduceCartField<256,true><<<blocks, threads, smemSize>>>(currComp, opIn, fIn, blockRed);
        break;
      case 128:
        d_reduceCartField<128,true><<<blocks, threads, smemSize>>>(currComp, opIn, fIn, blockRed);
        break;
      case 64:
        d_reduceCartField<64,true><<<blocks, threads, smemSize>>>(currComp, opIn, fIn, blockRed);
        break;
      case 32:
        d_reduceCartField<32,true><<<blocks, threads, smemSize>>>(currComp, opIn, fIn, blockRed);
        break;
      case 16:
        d_reduceCartField<16,true><<<blocks, threads, smemSize>>>(currComp, opIn, fIn, blockRed);
        break;
      case 8:
        d_reduceCartField<8,true><<<blocks, threads, smemSize>>>(currComp, opIn, fIn, blockRed);
        break;
      case 4:
        d_reduceCartField<4,true><<<blocks, threads, smemSize>>>(currComp, opIn, fIn, blockRed);
        break;
      case 2:
        d_reduceCartField<2,true><<<blocks, threads, smemSize>>>(currComp, opIn, fIn, blockRed);
        break;
      case 1:
        d_reduceCartField<1,true><<<blocks, threads, smemSize>>>(currComp, opIn, fIn, blockRed);
        break;
    }
  } else {
    switch (threads) {
      case 512:
        d_reduceCartField<512,false><<<blocks, threads, smemSize>>>(currComp, opIn, fIn, blockRed);
        break;
      case 256:
        d_reduceCartField<256,false><<<blocks, threads, smemSize>>>(currComp, opIn, fIn, blockRed);
        break;
      case 128:
        d_reduceCartField<128,false><<<blocks, threads, smemSize>>>(currComp, opIn, fIn, blockRed);
        break;
      case 64:
        d_reduceCartField<64,false><<<blocks, threads, smemSize>>>(currComp, opIn, fIn, blockRed);
        break;
      case 32:
        d_reduceCartField<32,false><<<blocks, threads, smemSize>>>(currComp, opIn, fIn, blockRed);
        break;
      case 16:
        d_reduceCartField<16,false><<<blocks, threads, smemSize>>>(currComp, opIn, fIn, blockRed);
        break;
      case 8:
        d_reduceCartField<8,false><<<blocks, threads, smemSize>>>(currComp, opIn, fIn, blockRed);
        break;
      case 4:
        d_reduceCartField<4,false><<<blocks, threads, smemSize>>>(currComp, opIn, fIn, blockRed);
        break;
      case 2:
        d_reduceCartField<2,false><<<blocks, threads, smemSize>>>(currComp, opIn, fIn, blockRed);
        break;
      case 1:
        d_reduceCartField<1,false><<<blocks, threads, smemSize>>>(currComp, opIn, fIn, blockRed);
        break;
    }
  }
}
...
void gkylCartFieldDeviceReduce(baseReduceOp *redOp, int numCellsTot, int numComponents, int numBlocks, int numThreads, int maxBlocks, int maxThreads, GkDeviceProp *prop, GkylCartField_t *fIn, double *blockOut, double *intermediate, double *out) {
  ...
  for (int kComp=0; kComp<numComponents; kComp++) {
    reduceCartField(redOp, numCellsTot, kComp, numBlocks, numThreads, fIn, blockOut);
    ...
    checkCudaErrors(cudaMemcpy(out+kComp, blockOut, sizeof(double), cudaMemcpyDeviceToDevice));
  }
}
