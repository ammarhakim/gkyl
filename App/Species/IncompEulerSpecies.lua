...
local SP_BC_DIRICHLET = 4
local SP_BC_NEUMANN   = 5
...
IncompEulerSpecies.bcDirichlet = SP_BC_DIRICHLET    -- Specify the value.
IncompEulerSpecies.bcNeumann   = SP_BC_NEUMANN      -- Specify the derivative.
...
function IncompEulerSpecies:appendBoundaryConditions(dir, edge, bcType)
   ...
   elseif bcType == SP_BC_DIRICHLET then
      table.insert(self.boundaryConditions, self:makeBcUpdater(dir, edge, { {bcDirichletFunc}, 1 }, "pointwise"))
   elseif bcType == SP_BC_NEUMANN then
      table.insert(self.boundaryConditions, self:makeBcUpdater(dir, edge, { {bcNeumannFunc}, 2*math.pi-1 }, "pointwise"))
   else
