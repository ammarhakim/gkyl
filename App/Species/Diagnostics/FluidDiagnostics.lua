...
local contains=function(tbl, elem)
   for k, _ in pairs(tbl) do
      if elem == k then return true end
   end
   return false
end
...
function FluidDiags:fullInit(mySpecies, diagsImpl)
...
function FluidDiags:orgDiagnostics(diagsImpl, diagsTbl)
...
function FluidDiags:calcGridDiagnostics(tm, mySpecies)
...
function FluidDiags:writeGridDiagnostics(tm, fr)
   for _, diagNm in ipairs(self.diagGroups["grid"]) do 
      local diag = self.diags[diagNm]
...
function FluidDiags:writeRestartGridDiagnostics(tm, fr)
   for _, diagNm in ipairs(self.diagGroups["grid"]) do 
      local diag = self.diags[diagNm]
...
function FluidDiags:writeRestart(tm, gridFr, intFr)
