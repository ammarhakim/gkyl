...
plasmaApp = Plasma.App {
   ...
   funcField = Plasma.Geometry {
      ...
      bmag = function (t, xn)
         local x, z = xn[1], xn[3]
         --local dRdx, dZdx, dphidx = df(RZphi,1)(x, z, 0)
         --local dRdz, dZdz, dphidz = df(RZphi,2)(x, z, 0)
         --local gzz = dRdz*dRdz + dZdz*dZdz + dphidz*dphidz
         --local R, Z, phi = RZphi(x, z, 0)
         --local jacobian = (-dRdz*dZdx + dRdx*dZdz)*R
         --return math.sqrt(gzz)/jacobian
         local R, Z = RZ(x, z)
