...
r_bias = .25 -- Radius of the biased plate (m).
...
-- Wall potential used in sheath BCs.
local wallPotential = function(t, xn)
   local x, y = xn[1], xn[2]
   local r    = x
   if r >= r_s then
      return V_bias
...
plasmaApp = Plasma.App {
   ...
   lower       = {Rmin, -math.pi, -Lz/2}, -- Configuration space lower left.
   upper       = {Rmax,  math.pi,  Lz/2}, -- Configuration space upper right.
   mapc2p = function(xc)
       -- Field-aligned coordinates (r,theta,z)
       local r,theta,z = xc[1], xc[2], xc[3]
   ...
   periodicDirs = {2},                  -- Set y direction to peridodic
   ...
   maxWallTime = 96*3600, -- exits 5 min before 24h, after writing restart file
   parallelizeSpecies = true, -- for better GPU usage
 ...
 --        normNu      = {nuElc*((2*(vte^2))^(3/2))/n0, nuElcIon*((vte^2+vti^2)^(3/2))/n0},
      ...
      bcz = {Plasma.SheathBC{phiWall={timeDependence    = function(t) return 1. end, 
                                      spatialDependence = function(xn) return wallPotential(0.,xn) end,},
                             diagnostics={"M0","Energy","intM0","intEnergy"}},
             Plasma.SheathBC{phiWall={timeDependence    = function(t) return 1. end, 
                                      spatialDependence = function(xn) return wallPotential(0.,xn) end,},
                             diagnostics={"M0","Energy","intM0","intEnergy"}}, },
...
--         normNu      = {nuIon*((2*(vti^2))^(3/2))/n0, nuIonElc*((vti^2+vte^2)^(3/2))/n0},
   ...
   -- Field solver.
   field = Plasma.Field {
      -- Dirichlet in r, Periodic in theta, no bc in z.
