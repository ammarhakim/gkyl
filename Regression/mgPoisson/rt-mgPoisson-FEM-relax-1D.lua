-- Gkyl ------------------------------------------------------------------------
--
-- Test 1D FEM relaxation in the MGpoisson updater.
--
--    _______     ___
-- + 6 @ |||| # P ||| +
--------------------------------------------------------------------------------
...
for rlxI = 1, #relaxType do   -- Loop over types of relaxation.
   ...
   for iT = 1, #tests do   -- Loop over various boundary condition tests.
      ...
      phiFEM:write(string.format("phiFEM_%sP%i_Nx%i_%s_%s_relax%i.bp",basisName,pOrder,numCells[1],tests[iT]["bcStr"],relaxStr,0), 0.0)
      ...
      for nR = 1, numRelaxations do
         ...
         phiFEM:write(string.format("phiFEM_%sP%i_Nx%i_%s_%s_relax%i.bp",basisName,pOrder,numCells[1],tests[iT]["bcStr"],relaxStr,nR), nR)
      end
      ...
      phiFEM:write(string.format("phiFEManalyticSol_%sP%i_Nx%i_%s.bp",basisName,pOrder,numCells[1],tests[iT]["bcStr"]), 0.0)
