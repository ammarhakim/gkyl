-- Gkyl ------------------------------------------------------------------------
--
-- Test 2D translation between DG and FEM basis used in the MGpoisson updater.
--
--    _______     ___
-- + 6 @ |||| # P ||| +
--------------------------------------------------------------------------------
...
for iT = 1, #tests do
   ...
   -- ProjectOnBasis/evaluateOnNodes to initialize DG field.
   ...
   phiFEM:write(string.format("phiFEM_%sP%i_Nx%iNy%i_%s.bp",basisName,pOrder,numCells[1],numCells[2],tests[iT]["bcStr"]), 0.0)
   
   -- Transform the FEM field back to DG, and output the DG field.
   poissonSlv:translateFEMtoDG(phiFEM, phiDG)
   phiDG:write(string.format("phiDG_%sP%i_Nx%iNy%i_%s.bp",basisName,pOrder,numCells[1],numCells[2],tests[iT]["bcStr"]), 0.0)
