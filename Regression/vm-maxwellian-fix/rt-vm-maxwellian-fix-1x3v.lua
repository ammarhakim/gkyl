-- Gkyl ------------------------------------------------------------------------
--
-- Test for updater to fix maxwellian with iteration method, 1x3v.
--------------------------------------------------------------------------------
...
local n0 = 1.0
local u0 = {0.0, 0.75, 0.1}
local vt0 = 1.0   

-- Phase space mesh.
local lower = {-0.50, -6.0*vt0, -6.0*vt0, -6.0*vt0}
local upper = { 0.50,  6.0*vt0,  6.0*vt0,  6.0*vt0}
...
local uDriftFunc = function (t, xn) return u0[1]*(1.0+eps_u*math.cos(2.*math.pi*xn[1])), u0[2]*(1.0+eps_u*math.cos(2.*math.pi*xn[1])), u0[3]*(1.0+eps_u*math.cos(2.*math.pi*xn[1]))  end
...
local initialF = function(t, xn)
   ...
   local ux, uy, uz = uDriftFunc(t,xn)
   local vtsq = vtSqFunc(t,xn)

   return (den/(math.sqrt(2.*math.pi*vtsq)^vDim))*math.exp(-((vx-ux)^2+(vy-uy)^2+(vz-uz)^2)/(2.*vtsq))
end
