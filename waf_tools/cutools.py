...
class nvcc(Task.Task):
        run_str = '${NVCC} -x cu -c -dc -O3 -std=c++11 -lineinfo --maxrregcount=250 -arch=sm_70 --compiler-options="-fPIC" ${FRAMEWORKPATH_ST:FRAMEWORKPATH} ${CPPPATH_ST:INCPATHS} ${DEFINES_ST:DEFINES} ${CXX_SRC_F} ${SRC} -o ${OUT}/${TGT}'
        color   = 'GREEN'
        ext_in  = ['.h']
        vars    = ['CCDEPS']
        scan    = c_preproc.scan
